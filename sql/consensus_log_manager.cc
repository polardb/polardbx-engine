...
int ConsensusLogManager::init_service() {
  if (!opt_initialize) {
    ...
    uint64 mock_start_index = std::max(get_log_file_index()->get_first_index(), opt_consensus_start_index);
    ...
    if (!opt_consensus_force_recovery) {
      if (!is_learner) {
        ...
        consensus_ptr->init(cluster_str_config, 0, NULL, opt_consensus_io_thread_cnt,
