...
bool plugin_foreach_with_mask_without_binlog(THD *thd,
  ...
  */
  ...
  if (type == MYSQL_ANY_PLUGIN) {
    for (idx = 0; idx < total; idx++) {
      plugin = plugin_array->at(idx);
      plugins[idx] = (!(plugin->state & state_mask)) && strcmp(plugin->name.str, "binlog") ? plugin : NULL;
    }
  } else {
...
bool plugin_foreach_with_mask_without_binlog(THD *thd,
  ...
  return plugin_foreach_with_mask_without_binlog(thd, funcs, type, state_mask, arg);
}
