...
/**
  ...
  @param[in]    table       Table_ref object
...
bool PT_create_sequence_stmt::prepare_sequence_fields(const THD *thd) {
  ...
  assert(opt_table_element_list == NULL);
  /**
...
Sql_cmd *PT_create_sequence_stmt::make_cmd(THD *thd) {
  ...
  assert(opt_query_expression == NULL);
 ...
 public:
  Disable_reprepare_observer(THD *thd) : m_thd(thd) {
    ...
    if (m_saved) {
      ...
      /* One at most */
      assert(m_thd->get_reprepare_observer() == nullptr);
    }
  }
...
bool Sql_cmd_create_sequence::execute(THD *thd) {
  ...
  /**
    ...
    version of the table share is always different from of the Table_ref
    ...
    Actuallly, we can ignore the difference between the share and Table_ref
...
bool Open_sequence_table_ctx::open_table() {
  ...
  if (!m_inherit_table || !(m_inherit_table->table)) {
    /**
      Clone a Table_ref from table, then open and lock the table for the
      sequence dml.
    */
...
Insert_sequence_table_ctx::Insert_sequence_table_ctx(
    THD *thd, Table_ref *table_list, const Sequence_info *seq_info)
    : otx(thd, table_list),
      m_thd(thd),
      m_seq_info(seq_info),
...
bool Insert_sequence_table_ctx::write_record() {
  ...
  assert(table && table->in_use == m_thd);
