/* Copyright (c) 2009, 2022, Oracle and/or its affiliates. Copyright (c) 2023, 2024, Alibaba and/or its affiliates.
...
class Binlog_event_writer : public Basic_ostream {
  ...
  */
  Binlog_event_writer(MYSQL_BIN_LOG::Binlog_ofile *binlog_file)
    ...
    // revise position(), cache_log of consensus log manager is just a wapper of
    // io_cache
...
int MYSQL_BIN_LOG::gtid_end_transaction(THD *thd) {
  ...
  if (thd->owned_gtid.sidno > 0) {
    ...
    if (!opt_bin_log ||
      ...
      */
      const bool need_save_gtid = (!ConsensusLogManager::enable_consensus()
  ...
  */
  ...
  if (flags.finalized) {
    ...
    */
    ...
    /* correct the log_pos temporarily stored in IO_CACHE of
  ...
  */
  ...
  while (rit != filename_list.rend()) {
    ...
    switch (read_gtids_from_binlog(
        filename, nullptr, &binlog_previous_gtid_set, first_gtid,
        binlog_previous_gtid_set.get_sid_map(), opt_source_verify_checksum,
...
bool MYSQL_BIN_LOG::init_gtid_sets(Gtid_set *all_gtids, Gtid_set *lost_gtids,
  ...
  Checkable_rwlock *sid_lock = (is_relay_log && all_gtids != NULL)
  ...
  */
  if (is_server_starting && !is_relay_log && !is_xpaxos_log &&
      !filename_list.empty())
...
bool MYSQL_BIN_LOG::open_binlog(
  ...
  */
  ...
  // write previous consensus index event
  {
    ...
    if (opt_consensuslog_revise && is_relay_log)
      prev_consensus_index_ev.consensus_extra_time =
  ...
  */
  if (current_thd || is_xpaxos_log) {
    ...
    if (need_sid_lock) sid_lock->unlock();
    if (opt_consensuslog_revise && is_relay_log && is_xpaxos_log)
      prev_gtids_ev.consensus_extra_time =
          consensus_log_manager.get_event_timestamp();
  ...
  } else  // !(current_thd)
  {
    ...
    */
    if (is_relay_log) {
      ...
      if (!is_xpaxos_log) prev_gtids_ev.set_relay_log_event();
...
bool MYSQL_BIN_LOG::reset_logs(THD *thd, bool delete_only) {
  ...
  */
  ...
  if (!is_xpaxos_log || !is_relay_log) ha_reset_logs(thd);
...
int MYSQL_BIN_LOG::new_file_impl(
  ...
  */
  ...
  DBUG_EXECUTE_IF("crash_binlog_rotate_between_close_and_open",
                  DBUG_SUICIDE(););
...
bool MYSQL_BIN_LOG::write_incident(Incident_log_event *ev, THD *thd,
  ...
  */
  if (cache_mngr == nullptr ||
    ...
    if (!is_xpaxos_log) error = write_event_to_binlog(ev);
    /*
  ...
  } else  // (cache_mngr != NULL)
  {
    ...
    if (!is_xpaxos_log) {
      /*
        Write the incident event into stmt_cache, so that a GTID is generated
        and written for it prior to flushing the stmt_cache.
      */
...

  {
    ...
    */
    ...
    if ((ev = binlog_file_reader.read_event_object()) &&
      ...
      auto bl_recovery_ptr =
          xp::Recovery_manager::instance().create_recovery(binlog_file_reader);
...
std::pair<int, my_off_t> MYSQL_BIN_LOG::flush_thread_caches(THD *thd) {
  ...
  if (!error && bytes > 0) {
    ...
    */
    thd->set_trans_pos(log_file_name,
...
int MYSQL_BIN_LOG::process_flush_stage_queue(my_off_t *total_bytes_var,
  ...
  uint64 term = 0;
  if (!opt_initialize && consensus_ptr) {
    alisql::LogEntry log_entry;
    // use fake buf_size, tell consenesus layer, the entry is not noop
    BLConsensusLog::packLogEntry((uchar *)const_cast<char *>("any"), 3, term, 0,
  ...
  if (mysql_bin_log.is_rotating_caused_by_incident) {
    ...
    //will unlock in change_stage(Commit_stage_manager::SYNC_STAGE)
    mysql_mutex_lock(&LOCK_log);
  }
  ...
  for (THD *head = first_seen; head; head = head->next_to_commit) {
    ...
    // stmt or trx cache flush will generate a consensus index and set thd's
    // consensus_index
  ...
  *total_bytes_var = total_bytes;
  ...
  no_flushes = 0;
#endif
  // must unlock after fetch_queue_for() because we should not make stage queue
  // chaos
  mysql_mutex_unlock(consensus_log_manager.get_term_lock());
  //TODO::@yanhua remove later
  if (!mysql_bin_log.is_rotating_caused_by_incident)
...
int MYSQL_BIN_LOG::finish_commit(THD *thd) {
  ...
  */
  ...
  if (thd->session_tracker.get_tracker(SESSION_INDEX_TRACKER)->is_enabled())
    thd->session_tracker.get_tracker(SESSION_INDEX_TRACKER)
        ->mark_as_changed(thd, {});
...
int MYSQL_BIN_LOG::ordered_commit(THD *thd, bool all, bool skip_commit) {
  ...
  // variable used by applied_checker
  ...
  */
  ...
  if (change_stage(
          thd, Commit_stage_manager::BINLOG_FLUSH_STAGE, thd, nullptr,
  ...
  if (flush_error == 0 && total_bytes > 0) {
    if (!opt_initialize && alisql_server) { /* let the paxos send the log */
  ...
  */
  ...
  if (consensus_ptr && opt_enable_appliedindex_checker && sync_error == 0) {
  ...
  if (group_max_log_index > 0) {
    ...
    if (!opt_initialize && alisql_server) alisql_server->writeLogDone(group_max_log_index);
  }
...
int THD::binlog_update_row(TABLE *table, bool is_trans,
  ...
  /* Record in changeset */
  im::gChangesetManager.update_row_to_cache(this, table, before_record,
                                            after_record);
