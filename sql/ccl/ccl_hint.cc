...
*/
ulonglong Ccl_queue_value_hint::hash_value(THD *, const SELECT_LEX *) {
...
/**
  ...
  @param[in]      select      the top-level select context
*/
ulonglong Ccl_queue_field_hint::hash_value(THD *thd, const SELECT_LEX *select) {
  Item *cond;
  List_iterator_fast<Item> it(
      const_cast<SELECT_LEX *>(select)->ccl_queue_field_cond_list);
  while ((cond = it++)) {
    Item_func *func_cond = (Item_func *)(cond);
    Item **args = func_cond->arguments();
    DBUG_ASSERT(func_cond->argument_count() == 2);
