...
class Snapshot_noop_vision : public Snapshot_vision {
 public:
  ...
  virtual void store_int(uint64_t) override { DBUG_ASSERT(0); }

  virtual void after_activate() override { DBUG_ASSERT(0); }
...
class Table_snapshot {
 public:
  ...
  int activate(Snapshot_vision *vision, THD *thd) {
    int error;
    DBUG_ASSERT(vision == get(vision->type()));
...
extern void simulate_snapshot_clause(THD *thd, TABLE_LIST *all_tables);
