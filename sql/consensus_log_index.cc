...
* $Id:  consensus_log_index.cc,v 1.0 08/22/2016 09:16:32 AM droopy.hw(droopy.hw@alibaba-inc.com) $
*
************************************************************************/
...
int ConsensusLogIndex::add_to_index_list(uint64 consensus_index, ulong timestamp, std::string & log_name)
{
...
int ConsensusLogIndex::get_log_file_from_index(uint64 consensus_index, std::string & log_name)
{
  bool found = false;
  mysql_mutex_lock(&LOCK_consensuslog_index);
...
int ConsensusLogIndex::get_log_file_by_timestamp(ulong timestamp, std::string & log_name)
{
  bool found = false;
  mysql_mutex_lock(&LOCK_consensuslog_index);
  for (auto riter = index_list.rbegin(); riter != index_list.rend(); ++riter)
