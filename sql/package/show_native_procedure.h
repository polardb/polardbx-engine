...
class Sql_cmd_show_native_procedure : public Sql_cmd_admin_proc {
 public:
  explicit Sql_cmd_show_native_procedure(THD *thd, List<Item> *list,
                                         const Proc *proc)
 ...
 public:
  explicit Show_native_procedure_proc(PSI_memory_key key = 0) : Proc(key) {
    ...
    Column_element elements[COLUMN_LAST] = {
        {MYSQL_TYPE_VARCHAR, C_STRING_WITH_LEN("SCHEMA_NAME"), 128},
        {MYSQL_TYPE_VARCHAR, C_STRING_WITH_LEN("PROC_NAME"), 128},
        {MYSQL_TYPE_VARCHAR, C_STRING_WITH_LEN("PROC_TYPE"), 128},
        {MYSQL_TYPE_VARCHAR, C_STRING_WITH_LEN("PARAMETERS"), 1024}};
