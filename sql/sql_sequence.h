...
class PT_create_sequence_stmt : public PT_create_table_stmt {
  ...
  */
  ...
  /**
    ...
    @param[in]    thd         Connection context
    @param[in]    table       Table_ref

    @retval       false       success
    @retval       true        failure
  */
  bool prepare_sequence_engine(const THD *thd, const Table_ref *table);
  /**
...
class Open_sequence_table_ctx {
 public:
  /**
    Temporary Table_ref object that is used to hold opened table.
  */
  class Table_list_state {
   public:
    ...
    /** The cloned Table_ref will be free when statement end. */
    ~Table_list_state() {}

    Table_ref *cloned_table() const { return m_table; }

   private:
    Table_ref *m_table;
  };

  Open_sequence_table_ctx(THD *thd, Table_ref *table_list);
...
class Insert_sequence_table_ctx {
 public:
  Insert_sequence_table_ctx(THD *thd, Table_ref *table_list,
