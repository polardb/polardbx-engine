...
LEX_STRING RECYCLE_BIN_SCHEMA = {C_STRING_WITH_LEN("__recycle_bin__")};

LEX_STRING ORIGIN_SCHEMA = {C_STRING_WITH_LEN("__origin_schema__")};

LEX_STRING ORIGIN_TABLE = {C_STRING_WITH_LEN("__origin_table__")};
...
Recycle_error_handler::Recycle_error_handler(THD *thd, TABLE_LIST *table_list)
...
*/
static bool check_state(THD *thd, TABLE_LIST *table) {
  ...
  if (table_def == nullptr ||
      table_def->options().exists(ORIGIN_SCHEMA.str) ||
      table_def->options().exists(ORIGIN_TABLE.str)) {
...
/**
  ...
  @retval         true        failure
*/
static bool prepare_recycle_table(THD *thd, TABLE_LIST *table_list,
                                  char **new_table) {
  char buff[128];
...
recycle_base_table(THD *thd, std::set<handlerton *> *post_ddl_htons,
  ...
  DBUG_ASSERT(from_table_def != nullptr && to_table_def != nullptr);
  ...
  DBUG_ASSERT(!to_table_def->options().exists(ORIGIN_SCHEMA.str));
  DBUG_ASSERT(!to_table_def->options().exists(ORIGIN_TABLE.str));
  ...
  handler *file =
      (hton == NULL ? 0 : get_new_handler((TABLE_SHARE *)0,
...
*/
Recycle_result
recycle_truncate_table(THD *thd, const char *path, TABLE_LIST *table_list,
  ...
  if (res == Recycle_result::OK) {
    ...
    for (const dd::Partition *item:
      ...
      /* Reset sub partition */
      for (const dd::Partition *sub_item : item->sub_partitions()) {
