...
int ChangesetManager::start_track(const std::string &table_name, u_int64_t memory_limit) {
  ...
  if (memory_limit <= 0 || memory_limit > MAX_MEMORY_LIMIT) {
    my_error(ER_CHANGESET_COMMAND_ERROR, MYF(0), "changeset start failed, error memory limit param");
    return 1;
  }
  ...
  if (has_changeset(full_table_name)) {
    std::string err = std::string("changeset already in progress: ").append(table_name);
...
int ChangesetManager::stop_track(const std::string &table_name) {
  ...
  if ((err = open_table(table_name, &table, TL_WRITE))) {
    my_printf_error(ER_CHANGESET_COMMAND_ERROR, "lock table when stop track failed", 0);
    return err;
  }
...
int ChangesetManager::fetch_change(const std::string &table_name, bool delete_last_cs, 
...
int ChangesetManager::fetch_times(const std::string &table_name, Changeset::Stats &stats) {
  if (current_thd->db().str == nullptr) {
    my_error(ER_CHANGESET_COMMAND_ERROR, MYF(0), "please use database first");
    return 1;
  }
...
void ChangesetManager::get_primary_keys(uchar const *record, KEY *key_info, std::string &pk) {
  if (record == nullptr) {
...
void ChangesetManager::write_row_to_cache(THD *thd, TABLE *table, uchar const *record) {
  ...
  std::string table_name(table_meta->table_name.str, table_meta->table_name.length);
  DBTableName full_table_name(db_name, table_name);
  std::string pk;
...
void
ChangesetManager::update_row_to_cache(THD *thd, TABLE *table, const uchar *beforeRecord, const uchar *afterRecord) {
  if (!is_changeset_enable()) {
    return;
  }
...
void ChangesetManager::delete_row_to_cache(THD *thd, TABLE *table, uchar const *record) {
  if (!is_changeset_enable()) {
    return;
  }
...
int ChangesetManager::open_table(const std::string &table_name, TABLE **output, thr_lock_type lock_type) {
  THD *thd = current_thd;
...
inline ChangeSetCache *ChangesetManager::get_changeset_from_cache(THD *thd, const DBTableName &full_table_name) {
  if (thd->changeset_map.find(full_table_name) == thd->changeset_map.end()) {
    thd->changeset_map.emplace(full_table_name, std::move(std::unique_ptr<ChangeSetCache>(new ChangeSetCache())));
  }
...
inline Changeset *ChangesetManager::get_changeset_ptr(const DBTableName &full_table_name) {
...
bool inline ChangesetManager::is_changeset_stop(const DBTableName &full_table_name) {
...
inline void ChangesetManager::commit(const DBTableName &full_table_name, std::unique_ptr<ChangeSetCache> &cache) {
  polarx_rpc::CautoSpinRWLock lock(rw_lock, false, 2000);
  auto cs = get_changeset_ptr(full_table_name);
  if (cs == nullptr) {
    return;
...
void ChangesetManager::fetch_changeset(const DBTableName &full_table_name, bool delete_last_cs, 
...
Changeset::Stats ChangesetManager::fetch_changeset_stats(const DBTableName &full_table_name) {
  polarx_rpc::CautoSpinRWLock lock(rw_lock, false, 2000);
  auto cs = get_changeset_ptr(full_table_name);
  if (cs == nullptr) {
...
void ChangesetManager::init_changeset(const DBTableName &full_table_name, u_int64_t memory_limit) {
  polarx_rpc::CautoSpinRWLock lock(rw_lock, true, 2000);
  if (changeset_map.empty()) {
    changeset_start.store(true);
  }
  
  changeset_map.emplace(full_table_name, std::move(std::unique_ptr<Changeset>(new Changeset())));
