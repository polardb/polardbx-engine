...
public:
  ...
  int fetch_change(const std::string &table_name, bool delete_last_cs, std::map<std::string, ChangesetResult *> &changes, TABLE_SHARE *table_share);
   ...
   * cache map
   */
  ChangeSetCache *get_changeset_from_cache(THD *thd, const DBTableName &full_table_name);
  ...
  void update_row_to_cache(THD *thd, TABLE *table, const uchar *beforeRecord, const uchar *afterRecord);
  ...
  static void get_primary_keys(uchar const *record, KEY *key_info, std::string &pk);

  int open_table(const std::string &tableName, TABLE **output) {
    return open_table(tableName, output, TL_WRITE);
  }

  int open_table(const std::string &tableName, TABLE **output, thr_lock_type lock_type);

  static atomic_bool changeset_start;

  static inline bool is_changeset_enable() {
    return changeset_start.load() && enable_changeset;
  }
...
private:
  ...
  void commit(const DBTableName &full_table_name, std::unique_ptr<ChangeSetCache> &cache);

  bool has_changeset(const DBTableName &full_table_name);

  void erase_changeset(const DBTableName &full_table_name);

  void init_changeset(const DBTableName &full_table_name, u_int64_t memory_limit);

  Changeset *get_changeset_ptr(const DBTableName &full_table_name);

  void fetch_changeset(const DBTableName &full_table_name, bool delete_last_cs, std::map<std::string, ChangesetResult *> &res, TABLE_SHARE *table_share);
