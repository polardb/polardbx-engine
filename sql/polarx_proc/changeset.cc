...
const char *Changeset::get_next_file_name() {
  ...
  sprintf(file_name_buffer, "./%s/%s_%d.cs", full_table_name.db_name.c_str(), full_table_name.tb_name.c_str(), file_name_num);
  file_name_num++;
...
int Changeset::open_table(THD *thd, const std::string &db, const std::string &tb, TABLE **output) {
  TABLE_LIST tables(db.c_str(), db.length(), tb.c_str(), tb.length(), tb.c_str(), TL_READ);
