...
Sql_cmd *Changeset_proc_start::evoke_cmd(THD *thd, List<Item> *list) const {
...
void Sql_cmd_changeset_proc_start::send_result(THD *thd, bool error) {
  ...
  std::string table_name(table_name_item->val_str(nullptr)->ptr(), table_name_item->val_str(nullptr)->length());

  sql_print_information("changeset.start(%s)", table_name.data());

  u_int64_t memory_limit = memory_limit_item == nullptr ? MAX_MEMORY_SIZE : (u_int64_t) memory_limit_item->value;
  ...
  proto->start_row();
  proto->store(table_name.data(), &my_charset_utf8_bin);
  proto->end_row();
...
Sql_cmd *Changeset_proc_fetch::evoke_cmd(THD *thd, List<Item> *list) const {
  return new(thd->mem_root) Sql_cmd_type(thd, list, this);
}

bool Changeset_proc_fetch::my_send_result_metadata(THD *thd, Proc::Columns columns, std::list<Field *> fields) const {
  List<Item> field_list;
  Item *item;
  DBUG_ENTER("Proc::send_result_metadata");
//   DBUG_ASSERT(!columns.empty() && m_result_type == RESULT_SET);
  ...
  for (const auto &field: fields) {
    switch (field->type()) {
      ...
      case MYSQL_TYPE_INT24:
        if (!(item = new Item_return_int(field->field_name,
                        field->field_length,
                        field->type()))) {
          DBUG_RETURN(0);
        }
        item->unsigned_flag= (field->flags & UNSIGNED_FLAG);
        break;
      ...
      case MYSQL_TYPE_BLOB:
        if (!(item = new Item_blob(field->field_name,
                      field->field_length))) {
          DBUG_RETURN(0);
        }
        break;
      ...
      default:
        ...
        DBUG_ASSERT(field->type() == MYSQL_TYPE_STRING);
        if (!(item = new Item_return_int(field->field_name,
                        field->field_length,
                        field->type()))) {
          DBUG_RETURN(0);
    ...
    field_list.push_back(item);
    item->maybe_null= !(field->flags & NOT_NULL_FLAG);
  }
...
void Sql_cmd_changeset_proc_fetch::send_result(THD *thd, bool error) {
  ...
  bool delete_last_cs = fetch_type_item == nullptr || fetch_type_item->value == 0;
  if (gChangesetManager.fetch_change(table_name, delete_last_cs, changes, table->s)) {
  ...
  if (!changes.empty()) {
    ...
    if (((Changeset_proc_fetch *) m_proc)->my_send_result_metadata(thd, m_columns, begin.second->pk_field_list)) return;
  } else {
    // send result
    if (m_proc->send_result_metadata(thd)) return;
  }
  ...
  for (auto item: changes) {
    ...
    proto->start_row();
    proto->store(row->get_op_string().data(), &my_charset_utf8_bin);
...
Sql_cmd *Changeset_proc_finish::evoke_cmd(THD *thd, List<Item> *list) const {
  return new(thd->mem_root) Sql_cmd_type(thd, list, this);
}
...
Sql_cmd *Changeset_proc_stop::evoke_cmd(THD *thd, List<Item> *list) const {
  return new(thd->mem_root) Sql_cmd_type(thd, list, this);
}
...
void Sql_cmd_changeset_proc_stop::send_result(THD *thd, bool error) {
  ...
  std::string table_name(table_name_item->val_str(NULL)->ptr(), table_name_item->val_str(nullptr)->length());
...
Sql_cmd *Changeset_proc_stats::evoke_cmd(THD *thd, List<Item> *list) const {
  return new(thd->mem_root) Sql_cmd_type(thd, list, this);
}
...
void Sql_cmd_changeset_proc_stats::send_result(THD *thd, bool error) {
  ...
  for (auto &item : change_counts) {
    ...
    proto->start_row();
    proto->store(full_table_name.db_name.data(), &my_charset_utf8_bin);
    proto->store(full_table_name.tb_name.data(), &my_charset_utf8_bin);
...
Sql_cmd *Changeset_proc_times::evoke_cmd(THD *thd, List<Item> *list) const {
  return new(thd->mem_root) Sql_cmd_type(thd, list, this);
}
...
void Sql_cmd_changeset_proc_times::send_result(THD *thd, bool error) {
  ...
  if (gChangesetManager.fetch_times(table_name, stats)) {
    my_printf_error(ER_CHANGESET_COMMAND_ERROR, "call changeset times failed", 0);
    return;
  }
...
bool Sql_cmd_changeset_proc::check_access(THD *thd) {
  return false;
}
