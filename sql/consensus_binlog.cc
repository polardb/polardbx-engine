...
int large_event_flush(THD *thd, uchar *buffer, ulonglong total_size, Log_event *ev, ulonglong &total_batch_size)
{
  ...
  while(start_pos < ev->buf_len)
  {
    ...
    xp::info(ER_XP_COMMIT) << "large_event_flush batch_size " << blen
        ...
        << ", crc32 " << crc32;
    if (end_pos == ev->buf_len) {
      error = mysql_bin_log.write_consensus_log(flag, thd->consensus_term, ev->buf_len);
    } else {
  ...
  while(!error && (ev = consensus_log_event_istream.read_event_object(fd_ev, opt_source_verify_checksum, &default_alloc)) != NULL)
  {
        ...
        || batch_size + ev-> buf_len > opt_consensus_max_log_size || force_batch))
    {
      ...
      xp::info(ER_XP_COMMIT) << "large_trx_flush batch_size " << batch_size
          << ", buf_len " << ev->buf_len
          << ", max_log_size " << opt_consensus_max_log_size
...
int MYSQL_BIN_LOG::build_consensus_log_index() {
  ...
  if (!my_b_inited(&index_file)) {
    ...
    xp::error(ER_XP_COMMIT) << "build consenus log index failed, can't init index file";
    return 1;  
  }
  ...
  for (auto iter = consensuslog_file_name_vector.begin(); iter != consensuslog_file_name_vector.end(); ++iter) {
    ...
    if (!find_prev_consensus_log) {
      xp::error(ER_XP_COMMIT) << "log file " << iter->c_str() << " do not contain prev_consensus_log_ev";
      return 1;
    }
  }
...
int MYSQL_BIN_LOG::init_last_index_of_term(uint64 term)
{
  ...
  for (auto iter = consensuslog_file_name_vector.rbegin(); !found && iter != consensuslog_file_name_vector.rend(); ++iter)
  {
    ...
    if (!found)
    {
      xp::warn(ER_XP_COMMIT) << "log file " << iter->c_str()
        << " cannot found last log term index, term is " << term;
    }
  }
  xp::info(ER_XP_COMMIT) << "last log term is " << term << ", last log term index is " << consensus_log_manager.get_recovery_manager()->get_last_leader_term_index();
  return !found;
}
...
uint64 MYSQL_BIN_LOG::get_trx_end_index(uint64 firstIndex)
{
  ...
  {
    xp::error(ER_XP_RECOVERY) << "get_trx_end_index cannot find consensus index log " << firstIndex;
    return 0;
  }
...
int MYSQL_BIN_LOG::read_log_by_consensus_index(const char* file_name, uint64 consensus_index, uint64 *consensus_term, std::string& log_content, bool *outer, uint *flag, bool need_content)
{
  ...
  while (!stop_scan && (ev = binlog_file_reader.read_event_object()) != NULL)
  {
    ...
    case binary_log::CONSENSUS_LOG_EVENT:
      ...
      {
        xp::info(ER_XP_COMMIT) << "directly read log error, log size is error";
        abort();
      }
      break;
    case binary_log::FORMAT_DESCRIPTION_EVENT:
        ...
        if (end_pos - start_pos == consensus_log_length)
        {
          ...
          else if (need_content && (cflag & Consensus_log_event_flag::FLAG_BLOB_END))
          {
            ...
            if (found)
            {
              ...
              uint64 blob_end_pos = (blob_index_list.size() == 1 ? end_pos : (start_pos + split_len));
  ...
  xp::info(ER_XP_COMMIT) << "directly read log reached consensus index " << consensus_index;

  if (!found)
    xp::error(ER_XP_COMMIT) << "read log by consensus index failed";
...
int MYSQL_BIN_LOG::prefetch_logs_of_file(THD *thd, uint64 channel_id, const char* file_name, uint64 start_index)
{
        ...
        if (end_pos > start_pos && end_pos - start_pos == consensus_log_length)
        {
            ...
            for (size_t i=0; i<blob_index_list.size(); ++i)
            {
              if (blob_index_list[i] + prefetch_channel->get_window_size() >= start_index)
              {
                ...
                while ((result = prefetch_channel->add_log_to_prefetch_cache(blob_term_list[i],
                  ...
                  get_uchar_str(log_content), false, blob_flag_list[i], current_crc32)) == FULL)
                {
                  // wait condition already executed in add log to prefetch cache
                }
                if (result == INTERRUPT)
          ...
          else
          {
            if (current_index + prefetch_channel->get_window_size() >= start_index)
            {
              ...
              while ((result = prefetch_channel->add_log_to_prefetch_cache(current_term,
                current_index, log_content.size(),
                get_uchar_str(log_content), (rci_ev != NULL), current_flag, current_crc32)) == FULL)
              {
                // wait condition already executed in add log to prefetch cache
              }
              if (result == INTERRUPT || current_index == consensus_log_manager.get_sync_index())
  ...
  xp::info(ER_XP_COMMIT) << "channel_id " << channel_id
    ...
    << ", prefetch_window_size " << prefetch_channel->get_window_size()
    << ", sync_index " << consensus_log_manager.get_sync_index()
    << ", file_name " << file_name
    << ", get_error_type  " << binlog_file_reader.get_error_type();
...
int MYSQL_BIN_LOG::truncate_logs_from_index(std::vector<std::string> & files_list, std::string last_file)
{
  ...
  {
    xp::error(ER_XP_COMMIT) << "MYSQL_BIN_LOG::truncate_logs was called with file " << last_file.c_str()
  ...
  {
    xp::error(ER_XP_COMMIT) << "MYSQL_BIN_LOG::remove_logs_from_index failed to "
...
int MYSQL_BIN_LOG::truncate_files_after(std::string & file_name)
{
  ...
  if (error)
    xp::error(ER_XP_COMMIT) << "truncate_files_after failed";
  return error;
}


int MYSQL_BIN_LOG::truncate_single_file_by_consensus_index(const char* file_name, uint64 consensus_index, uint64 *offset)
{
  ...
  *offset = 0;
  ...
  {
    xp::error(ER_XP_COMMIT) << "Failed to find pos by consensus index " << consensus_index << " when truncate ";
    return -1;
  }
...
int MYSQL_BIN_LOG::consensus_truncate_log(uint64 consensus_index)
{
  ...
  if (find_log_by_consensus_index(consensus_index, file_name) ||
      ...
      truncate_single_file_by_consensus_index(file_name.c_str(), consensus_index, &offset) ||
      m_binlog_file->truncate(offset))
  {
    xp::error(ER_XP_COMMIT) << "Truncate cannot find consensus index log "
        << ", file_name " << file_name
        << ", consensus_index " << consensus_index
        << ", offset " << offset;
    abort();
  ...
  else
  {
    ...
    xp::info(ER_XP_COMMIT) << "succ to truncate binlog file " << file_name.c_str()
                                << ", consensus_index " << consensus_index
                                << ", offset " << offset
...
int MYSQL_BIN_LOG::consensus_get_log_position(uint64 consensus_index, char* log_name, uint64 *pos)
{
  ...
  {
    xp::error(ER_XP_COMMIT) << "Get log position cannot find consensus index log " << consensus_index;
    ret = 1;
  }
...
int MYSQL_BIN_LOG::consensus_get_log_entry(uint64 consensus_index, uint64 *consensus_term, std::string& log_content, bool *outer, uint *flag, bool need_content)
{
  std::string file_name;
  int ret = 0;
  // use another io_cache , so do not need lock LOCK_log
  if (find_log_by_consensus_index(consensus_index, file_name) ||
    read_log_by_consensus_index(file_name.c_str(), consensus_index, consensus_term, log_content, outer, flag, need_content))
  {
    xp::error(ER_XP_COMMIT) << "Get log entry cannot find consensus index log " << consensus_index;
    ret = 1;
  }
...
int MYSQL_BIN_LOG::consensus_prefetch_log_entries(THD *thd, uint64 channel_id, uint64 consensus_index)
{
  ...
  {
    xp::error(ER_XP_COMMIT) << "Prefetch cannot find consensus index log " << consensus_index;
    ret = 1;
  }
...
static int init_consensus_event_timestamp(uchar *buf, size_t len)
{
      ...
      consensus_log_manager.get_event_timestamp() == 0)
  {
    ...
    {
      xp::error(ER_XP_COMMIT) << "Found invalid event during init_consensus_event_timestamp.";
...
static int revise_entry_and_write(MYSQL_BIN_LOG::Binlog_ofile *binlog_file, uchar *buf, size_t len)
{
  ...
  if ((size_t)(header - buf) != len)
  {
    xp::error(ER_XP_COMMIT) << "Found invalid event during revise_entry_and_write.";
    return 1; // report error to abort
  }
  return binlog_file->write(buf, len);
}
...
{
  enum_binlog_checksum_alg alg = (enum_binlog_checksum_alg)binlog_checksum_options;
                                       ...
                                       uint64* index, bool* rotate_var, Relay_log_info *rli, bool with_check)
{
  ...
  else if (log.flag & Consensus_log_event_flag::FLAG_BLOB)
  {
    ...
    xp::info(ER_XP_COMMIT) << "Large event: cache the current log"
                               << ", size " << log.buf_size
                               << ", cache size " << my_b_tell(consensus_log_manager.get_cache());
  }
  ...
  *index = consensus_log_manager.get_current_index();
  if (*index != log.index) {
    ...
    if (log.index != 0) {
      xp::error(ER_XP_COMMIT) << "Consensus Index Mismatch, system current index is " << *index
                            << ", but the log index is " << log.index;
      error = 1;
    } else {
      log.checksum = opt_consensus_checksum ? checksum_crc32(0, log.buffer, log.buf_size) : 0;
    }
  }

  if (!error) {
    DBUG_EXECUTE_IF("generate_bad_log_entry", real_buffer[real_buf_size / 2] += 1;);
    error = verify_checksum(real_buffer, real_buf_size);
    if (error)
      xp::error(ER_XP_COMMIT) << "failed to verify checksum when accept consensus pkg, reject"
        << ", term " << log.term
        << ", index " << *index
  ...
  if (!error)
    error = consensus_log_manager.get_fifo_cache_manager()->add_log_to_cache(log.term,
      *index, log.buf_size, log.buffer, log.outer, log.flag, log.checksum);

  if (!error)
    error = init_consensus_event_timestamp(real_buffer, real_buf_size);

  if (!error)
    error = write_consensus_log(log.flag, log.term, real_buf_size);
...
int MYSQL_BIN_LOG::append_multi_consensus_logs(std::vector<ConsensusLogEntry> &logs, uint64* max_index, bool* rotate_var, Relay_log_info *rli)
{
  ...
  for (auto iter = logs.begin(); iter != logs.end(); iter++)
  {
    ...
    *max_index = consensus_log_manager.get_current_index();
    ...
    {
      xp::error(ER_XP_COMMIT) << "Consensus Index Mismatch, system current index is " << *max_index
    ...
    if (!error) {
      ...
      error = verify_checksum(real_buffer, real_buf_size);
      if (error)
        xp::warn(ER_XP_COMMIT) << "failed to verify checksum when accept consensus pkg, reject"
          << ", max_index " << *max_index
          << ", term " << iter->term
          << ", index " << iter->index
    ...
    if (!error)
      error = consensus_log_manager.get_fifo_cache_manager()->add_log_to_cache(iter->term,
        *max_index, iter->buf_size, iter->buffer, iter->outer, iter->flag, iter->checksum);
...
void MYSQL_BIN_LOG::consensus_before_commit(THD *thd) {
      ...
      (thd->consensus_index > consensus_log_manager.get_consensus_info()->get_start_apply_index())) )
  {
    ...
    xp::warn(ER_XP_COMMIT) << "Failed to commit ,because previous error or shutdown or leadership changed, system apply index:" 
  ...
  while (*is_running)
  {
    ...
    */
    ...
    if (binlog_file_reader.open(log_name.c_str())) {
      xp::error(ER_XP_COMMIT) << "Thread binlog_commit_pos_watcher fails to open the binlog file " << log_name.c_str();
      goto err;
    }
    skip = false;
    while (*is_running && !skip && (ev = binlog_file_reader.read_event_object()) != NULL)
    {
      ...
      case binary_log::CONSENSUS_LOG_EVENT:
        ...
        if (commitIndex <= consensus_log_ev->get_index())
        {
          if (commitIndex < consensus_log_ev->get_index())
          {
            xp::error(ER_XP_COMMIT) << "Thread binlog_commit_pos_watcher reports a unsafe commit position.";  // for defence
...
int flush_consensus_log(THD *thd, binlog_cache_data *,
  ...
  if (!is_large_trx) {
    ...
    if (!error) {
      ...
      thd->consensus_index = consensus_log_manager.get_current_index();
      error = mysql_bin_log.write_consensus_log(flag, thd->consensus_term, buf_size);
    }
