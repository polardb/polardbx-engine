...
int large_event_flush(THD *thd, uchar *buffer, ulonglong total_size, Log_event *ev, ulonglong &total_batch_size)
{
  ...
  while(start_pos < ev->buf_len)
  {
    ...
    raft::info(ER_RAFT_BLCOMMIT) << "Large event batch_size " << blen
...
int large_trx_flush(THD *thd, uchar *buffer, ulonglong total_size)
{
  ...
  while(!error && (ev = consensus_log_event_istream.read_event_object(fd_ev, opt_source_verify_checksum, &default_alloc)) != NULL)
  {
        ...
        || batch_size + ev-> buf_len > opt_consensus_max_log_size || force_batch))
    {
      ...
      thd->consensus_index = consensus_log_manager.get_current_index();
      raft::info(ER_RAFT_BLCOMMIT) << "Large event batch_size " << batch_size
          << ", total_batch_size " << total_batch_size
          << ", original buf_size " << total_size
          << ", consensus_index " << thd->consensus_index;
...
int MYSQL_BIN_LOG::build_consensus_log_index() {
  ...
  if (!my_b_inited(&index_file)) {
    ...
    raft::error(ER_RAFT_BLCOMMIT) << "build consenus log index failed, can't init index file";
    return 1;  
  }
  if ((error = find_log_pos(&log_info, NullS, false))) {
    if (error != LOG_INFO_EOF ) {
      raft::error(ER_RAFT_BLCOMMIT) << "find_log_pos() failed error: " << error;
      mysql_mutex_unlock(&LOCK_index);
      return error;
    }
  }
  ...
  for (auto iter = consensuslog_file_name_vector.begin(); iter != consensuslog_file_name_vector.end(); ++iter) {
    ...
    if (!find_prev_consensus_log) {
      raft::error(ER_RAFT_BLCOMMIT) << "log file " << iter->c_str() << " do not contain prev_consensus_log_ev";
      return 1;
    }
  }
...
int MYSQL_BIN_LOG::init_last_index_of_term(uint64 term)
{
  ...
  for (auto iter = consensuslog_file_name_vector.rbegin(); !found && iter != consensuslog_file_name_vector.rend(); ++iter)
  {
    ...
    if (!found)
    {
      raft::warn(ER_RAFT_BLCOMMIT) << "log file " << iter->c_str()
...
uint64 MYSQL_BIN_LOG::get_trx_end_index(uint64 firstIndex)
{
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "get_trx_end_index cannot find consensus index log " << firstIndex;
    return 0;
  }
...
int MYSQL_BIN_LOG::read_log_by_consensus_index(const char* file_name, uint64 consensus_index, uint64 *consensus_term, std::string& log_content, bool *outer, uint *flag, uint64 *checksum, bool need_content)
{
  ...
  while (!stop_scan && (ev = binlog_file_reader.read_event_object()) != NULL)
  {
    ...
    case binary_log::CONSENSUS_LOG_EVENT:
      ...
      {
        raft::info(ER_RAFT_BLCOMMIT) << "directly read log error, log size is error";
        abort();
      }
      break;
    case binary_log::FORMAT_DESCRIPTION_EVENT:
  ...
  if (end_pos > start_pos && !in_transaction)
  {
    if (need_content || rci_ev != NULL)
      // fetch_binlog_by_offset(&log, start_pos, end_pos, rci_ev, log_content);
      fetch_binlog_by_offset(binlog_file_reader, start_pos, end_pos, rci_ev, log_content);
    raft::info(ER_RAFT_BLCOMMIT) << "directly read last log size " << end_pos - start_pos;
    end_pos = start_pos = my_b_tell(binlog_file_reader.get_io_cache());
  }
  ...
  raft::info(ER_RAFT_BLCOMMIT) << "directly read log reached consensus index " << consensus_index;

  if (!found)
    raft::error(ER_RAFT_BLCOMMIT) << "read log by consensus index failed";
...
int MYSQL_BIN_LOG::prefetch_logs_of_file(THD *thd, uint64 channel_id, const char* file_name, uint64 start_index)
{
  ...
  raft::info(ER_RAFT_BLCOMMIT) << "channel_id " << channel_id
...
int MYSQL_BIN_LOG::truncate_logs_from_index(std::vector<std::string> & files_list, std::string last_file)
{
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "MYSQL_BIN_LOG::truncate_logs was called with file " << last_file.c_str()
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "MYSQL_BIN_LOG::remove_logs_from_index failed to "
...
err:
  raft::error(ER_RAFT_BLCOMMIT) << "truncate log from index failed";
  mysql_mutex_unlock(&LOCK_index);
  return 1;
}
...
int MYSQL_BIN_LOG::truncate_files_after(std::string & file_name)
{
  ...
  if (error)
    raft::error(ER_RAFT_BLCOMMIT) << "truncate_files_after failed";
  return error;
}
...
int MYSQL_BIN_LOG::truncate_single_file_by_consensus_index(const char* file_name, uint64 consensus_index)
{
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "Failed to find pos by consensus index " << consensus_index << " when truncate ";
    return -1;
  }
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "Failed to open the binlog file when truncate.";
    return -1;
  }
  if (my_chsize(file, offset, 0, MYF(MY_WME)))
  {
    raft::error(ER_RAFT_BLCOMMIT) << "Failed to resize binlog file when truncate.";
    mysql_file_close(file, MYF(MY_WME));
    return -1;
  }
  else
  {
    raft::info(ER_RAFT_BLCOMMIT) << "Truncate binlog file " << file_name
...
int MYSQL_BIN_LOG::consensus_truncate_log(uint64 consensus_index, Relay_log_info *rli)
{
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "Truncate cannot find consensus index log " << consensus_index;
    abort();
  }
  else
  {
    ...
    if (m_binlog_file->truncate(offset)) {
      raft::error(ER_RAFT_BLCOMMIT) << "Failed to truncate the binlog file " << offset;
      goto err;
    }
...
int MYSQL_BIN_LOG::consensus_get_log_position(uint64 consensus_index, char* log_name, uint64 *pos)
{
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "Get log position cannot find consensus index log " << consensus_index;
    ret = 1;
  }
...
int MYSQL_BIN_LOG::consensus_get_log_entry(uint64 consensus_index, uint64 *consensus_term, std::string& log_content, bool *outer, uint *flag, uint64 *checksum, bool need_content)
{
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "Get log entry cannot find consensus index log " << consensus_index;
    ret = 1;
  }
...
int MYSQL_BIN_LOG::consensus_prefetch_log_entries(THD *thd, uint64 channel_id, uint64 consensus_index)
{
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "Prefetch cannot find consensus index log " << consensus_index;
    ret = 1;
  }
...
static int init_consensus_event_timestamp(uchar *buf, size_t len)
{
      ...
      consensus_log_manager.get_event_timestamp() == 0)
  {
    ...
    {
      raft::error(ER_RAFT_BLCOMMIT) << "Found invalid event during init_consensus_event_timestamp.";
...
static int revise_entry_and_write(MYSQL_BIN_LOG::Binlog_ofile *binlog_file, uchar *buf, size_t len)
{
  ...
  if ((size_t)(header - buf) != len)
  {
    raft::error(ER_RAFT_BLCOMMIT) << "Found invalid event during revise_entry_and_write.";
    return 1; // report error to abort
  }
  return binlog_file->write(buf, len);
}
                                       ...
                                       uint64* index, bool* rotate_var, Relay_log_info *rli, bool with_check)
{
  ...
  else if (log.flag & Consensus_log_event_flag::FLAG_BLOB)
  {
    ...
    raft::info(ER_RAFT_BLCOMMIT) << "Large event: cache the current log, size " << log.buf_size;
  ...
  else if (log.flag & Consensus_log_event_flag::FLAG_BLOB_END)
  {
    ...
    my_b_write(consensus_log_manager.get_cache(), log.buffer, log.buf_size);
    real_buf_size = consensus_log_manager.serialize_cache(&real_buffer);
    raft::info(ER_RAFT_BLCOMMIT) << "Large event: cache the whole log, size " << real_buf_size;
  }
  else
  {
  ...
  *index = consensus_log_manager.get_current_index();
  ...
  {
    raft::error(ER_RAFT_BLCOMMIT) << "Consensus Index Mismatch, system current index is " << *index
...
int MYSQL_BIN_LOG::append_multi_consensus_logs(std::vector<ConsensusLogEntry> &logs, uint64* max_index, bool* rotate_var, Relay_log_info *rli)
{
  ...
  for (auto iter = logs.begin(); iter != logs.end(); iter++)
  {
    ...
    *max_index = consensus_log_manager.get_current_index();
    ...
    {
      raft::error(ER_RAFT_BLCOMMIT) << "Consensus Index Mismatch, system current index is " << *max_index
    ...
    else if (iter->flag & Consensus_log_event_flag::FLAG_BLOB)
    {
      ...
      real_buf_size = empty_log.length();
      /* save real data to cache */
      raft::info(ER_RAFT_BLCOMMIT) << "Large event: cache the current log, size " << iter->buf_size;
...
void MYSQL_BIN_LOG::consensus_before_commit(THD *thd) {
      ...
      (thd->consensus_index > consensus_log_manager.get_consensus_info()->get_start_apply_index())) )
  {
    ...
    raft::warn(ER_RAFT_BLCOMMIT) << "Failed to commit ,because previous error or shutdown or leadership changed, system apply index:" << consensus_log_manager.get_consensus_info()->get_start_apply_index()
                            ...
                            my_off_t *valid_pos)
{
  ...
  mem_root_unordered_map<my_xid, my_commit_gcn> xids(&mem_root);
  Consensus_cluster_info_log_event *rci_ev = NULL;
  ...
  */
    ...
    && ev->is_valid())
  {
    if (ev->get_type_code() == binary_log::CONSENSUS_LOG_EVENT)
    {
      ...
      {
        raft::error(ER_RAFT_BLCOMMIT) << "ConsensusLogManager::recover_intergrity_for_normandy error, Log index out of order";
        abort();
      }
      if (end_pos < start_pos)
      {
        raft::error(ER_RAFT_BLCOMMIT) << "ConsensusLogManager::recover_intergrity_for_normandy error, Log struct broken";
        abort();
      }
      ...
      if (begin_consensus && end_pos > start_pos && end_pos - start_pos == current_length)
      {
        ...
        else if (current_flag & Consensus_log_event_flag::FLAG_BLOB_END)
        {
          ...
          for (size_t i=0; i<blob_index_list.size(); ++i)
          {
            ...
            consensus_log_manager.get_fifo_cache_manager()->add_log_to_cache(blob_term_list[i], blob_index_list[i],
                log_content.size(), (uchar*)const_cast<char*>(log_content.c_str()),  (rci_ev != NULL), blob_flag_list[i], blob_crc32_list[i]);
            blob_start_pos = blob_end_pos;
            blob_end_pos = blob_end_pos + split_len > end_pos? end_pos: blob_end_pos + split_len;
          }
          blob_index_list.clear();
        ...
        else
        {
          ...
          consensus_log_manager.get_fifo_cache_manager()->add_log_to_cache(current_term, current_index,
              log_content.size(), (uchar*)const_cast<char*>(log_content.c_str()),  (rci_ev != NULL), current_flag, current_crc32);
          begin_consensus = FALSE;
          valid_index = current_index;
  ...
  if (begin_consensus)
  {
    raft::warn(ER_RAFT_BLCOMMIT) << "last consensus log is not intergrated, sync index should set to " << valid_index
        <<" instead of " << current_index;
  }
  ...
  */
  ...
  raft::warn(ER_RAFT_BLCOMMIT) << "Recover consensus index is " << current_index;
...
err2:
  ...
  // my_hash_free(&xids);
err1:
  raft::error(ER_RAFT_BLCOMMIT) << "Crash recovery failed. Either correct the problem "
  ...
  while (*is_running)
  {
    ...
    */
    ...
    if (binlog_file_reader.open(log_name.c_str())) {
      raft::error(ER_RAFT_BLCOMMIT) << "Thread binlog_commit_pos_watcher fails to open the binlog file " << log_name.c_str();
      goto err;
    }
    skip = false;
    ...
    while (*is_running && !skip && (ev = binlog_file_reader.read_event_object()) != NULL)
    {
      ...
      case binary_log::CONSENSUS_LOG_EVENT:
        ...
        if (commitIndex <= consensus_log_ev->get_index())
        {
          if (commitIndex < consensus_log_ev->get_index())
          {
            raft::error(ER_RAFT_BLCOMMIT) << "Thread binlog_commit_pos_watcher reports a unsafe commit position.";  // for defence
...
err:
    ...
    if (binlog_file_reader.has_fatal_error())
    {
      ...
      {
        retry++;
        raft::warn(ER_RAFT_BLCOMMIT) << "Fail to find commit position. "
