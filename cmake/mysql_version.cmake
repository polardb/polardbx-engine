...
IF(NOT POLARDB_VERSION)
  SET(POLARDB_VERSION "PolarDB V2.0")
ENDIF()
IF(NOT POLARDBX_PRODUCT_VERSION)
  SET(POLARDBX_PRODUCT_VERSION 1.0.0)
ENDIF()
IF(NOT POLARDBX_RELEASE_DATE)
  SET(POLARDBX_RELEASE_DATE 19700101)
ENDIF()
IF(NOT POLARDBX_ENGINE_VERSION)
  SET(POLARDBX_ENGINE_VERSION 1.0.0)
ENDIF()
IF(NOT POLARDBX_VERSION_EXTRA)
  SET(POLARDBX_VERSION_EXTRA X-Cluster)
ENDIF()
...
MACRO(GET_MYSQL_VERSION)
  ...
  SET(POLARDB_VERSION
    "${POLARDB_VERSION}_${POLARDBX_PRODUCT_VERSION}_${POLARDBX_ENGINE_VERSION}-${POLARDBX_RELEASE_DATE} (Distributed Edition)")
  MESSAGE(STATUS "POLARDB_VERSION ${POLARDB_VERSION}")

  SET(VERSION
    "${MAJOR_VERSION}.${MINOR_VERSION}.${PATCH_VERSION}${EXTRA_VERSION}-${POLARDBX_VERSION_EXTRA}-${POLARDBX_ENGINE_VERSION}-${POLARDBX_RELEASE_DATE}")
...
EXECUTE_PROCESS(COMMAND sh -c "cd ${CMAKE_SOURCE_DIR} && git rev-parse --abbrev-ref HEAD"
  OUTPUT_VARIABLE BUILD_BRANCH
  OUTPUT_STRIP_TRAILING_WHITESPACE
  RESULT_VARIABLE BRANCH_RESULT)
IF(BRANCH_RESULT)
  SET(BUILD_BRANCH "" CACHE INTERNAL "MySQL Build Branch")
ENDIF()
MESSAGE(STATUS "BUILD_BRANCH: ${BUILD_BRANCH}")

EXECUTE_PROCESS(COMMAND sh -c "cd ${CMAKE_SOURCE_DIR} && git rev-parse --short HEAD"
  OUTPUT_VARIABLE BUILD_COMMIT
  OUTPUT_STRIP_TRAILING_WHITESPACE
  RESULT_VARIABLE COMMIT_RESULT)
IF(COMMIT_RESULT)
  SET(BUILD_COMMIT "00000000" CACHE INTERNAL "MySQL Build Commit")
ENDIF()
MESSAGE(STATUS "BUILD_COMMIT: ${BUILD_COMMIT}")

IF (NOT CMAKE_BUILD_TYPE)
  set(BUILD_TYPE "default" CACHE INTERNAL "build type")
ELSE()
  set(BUILD_TYPE "${CMAKE_BUILD_TYPE}" CACHE INTERNAL "build type")
ENDIF()
