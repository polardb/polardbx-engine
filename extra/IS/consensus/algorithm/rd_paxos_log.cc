 ...
 * $Id:  rd_paxos_log.cc,v 1.0 08/15/2016 02:15:50 PM hangfeng.fj(hangfeng.fj@alibaba-inc.com) $
 *
 ************************************************************************/
...
const std::string firstIndexTag = "@PAXOS_LOG_FIRSTINDEX@"; /* for truncate forward */

namespace alisql {

RDPaxosLog::RDPaxosLog(const std::string &dataDir, bool compress, size_t writeBufferSize, bool sync)
 : db_(NULL), lastLogIndex_(UINT64_MAX), lastLogTerm_(0), firstLogIndex_(0), async_(false), sync_(sync), purgeLogFilter_(nullptr)
{
...
{
  std::string value;
  rocksdb::Status status= db_->Get(rocksdb::ReadOptions(), lastIndexTag, &value);
  ...
  if (status.ok() && !value.empty()) {
      ...
      /* set to 0 */
      status= db_->Put(rocksdb::WriteOptions(), firstIndexTag, intToString(firstLogIndex_.load()));
      assert(status.ok());
  }
}
...
uint64_t RDPaxosLog::append(const LogEntry &entry)
{
  ...
  return (async_ && entry.optype() != kNormal && entry.optype() != kConfigureChange) ? 0 : index;
}

uint64_t RDPaxosLog::append(const ::google::protobuf::RepeatedPtrField<LogEntry> &entries)
{
  std::string buf;
  std::string curIndex;
...
uint64_t RDPaxosLog::getFirstLogIndex()
{
    return firstLogIndex_.load();
}
...
void RDPaxosLog::truncateBackward(uint64_t firstIndex)
{
...

  {
    ...
    rocksdb::WriteOptions write_options;
    write_options.sync= sync_;
    rocksdb::Status status= db_->Put(write_options, lastIndexTag, intToString(lastLogIndex_));
    assert(status.ok());
    if (lastLogIndex_ > 0)
    {
      LogEntry logEntry;
