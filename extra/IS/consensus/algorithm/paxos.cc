...
void Paxos::commitDepResetLog(commitDepArgType* arg)
{
  ...
  while (log->getLastLogIndex() < arg->lastLogIndex)
  {
    ...
    /* no lock protection: X-Cluster log module ensure that you cannot append a log if you are a follower */
...
uint64_t Paxos::replicateLog_(LogEntry &entry, const bool needLock)
{
  ...
  /* Traditional Path: write checksum before wirteLog, which is different from X-Cluster */
