 ...
 * $Id:  consensus-replicate-with-cache-log-t.cc,v 1.0 03/14/2019 11:06:00 AM aili.xp(aili.xp@alibaba-inc.com) $
 *
 ************************************************************************/
...
class FilePaxosLogTest : public FilePaxosLog {
  public:
    FilePaxosLogTest(const std::string &dataDir, LogTypeT type= LTMem):FilePaxosLog(dataDir, type) { }
...
TEST(consensus, replicate_with_cache_log)
{
  ...
  std::shared_ptr<PaxosLog> rlog1= std::make_shared<FilePaxosLogTest>(dir1, FilePaxosLog::LogType::LTMem);
  Paxos *paxos1= new Paxos(timeout, rlog1);
  paxos1->init(strConfig, 1);
  std::string dir2 = std::string("paxosLogTestDir")+strConfig[1];
  std::shared_ptr<PaxosLog> rlog2= std::make_shared<FilePaxosLogTest>(dir2, FilePaxosLog::LogType::LTMem);
  Paxos *paxos2= new Paxos(timeout, rlog2);
  paxos2->init(strConfig, 2);
  std::string dir3 = std::string("paxosLogTestDir")+strConfig[2];
  std::shared_ptr<PaxosLog> rlog3= std::make_shared<FilePaxosLogTest>(dir3, FilePaxosLog::LogType::LTMem);
  ...
  sleep(2);
  EXPECT_TRUE(rlog2->getLastLogIndex() == base && rlog3->getLastLogIndex() == base);

  ((FilePaxosLogTest *)rlog1.get())->setLastCachedLogIndex(rlog1->getLastLogIndex());
  paxos1->appendLog(false);
  sleep(2);
  EXPECT_TRUE(rlog2->getLastLogIndex() == base + 1 && rlog3->getLastLogIndex() == base + 1);
  ...
  sleep(2);
  EXPECT_TRUE(rlog2->getLastLogIndex() == base + 2 && rlog3->getLastLogIndex() == base + 2);
