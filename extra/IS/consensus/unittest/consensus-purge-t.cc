 ...
 * $Id:  consensus-purge-t.cc,v 1.0 Sep 3, 2019 1:34:10 PM jarry.zj(jarry.zj@alibaba-inc.com) $
 *
 ************************************************************************/
...
TEST(consensus, paxos_purgeLog_normal) {
  ...
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir31", true, 4 * 1024 * 1024);
  Paxos *paxos1= new Paxos(timeout, rlog, 3000);
  paxos1->init(strConfig, 1, NULL);
  paxos1->initAutoPurgeLog(true);
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir32", true, 4 * 1024 * 1024);
  Paxos *paxos2= new Paxos(timeout, rlog, 3000);
  paxos2->init(strConfig, 2, NULL);
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir33", true, 4 * 1024 * 1024);
...
TEST(consensus, paxos_purgeLog_nopurge_forcepurge) {
  ...
  uint64_t timeout= 12500;
  std::shared_ptr<PaxosLog> rlog;
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir41", true, 4 * 1024 * 1024);
  Paxos *paxos1= new Paxos(timeout, rlog, 3000);
  paxos1->init(strConfig, 1, NULL);
  paxos1->initAutoPurgeLog(false, true, NULL); // disable autoPurge
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir42", true, 4 * 1024 * 1024);
  Paxos *paxos2= new Paxos(timeout, rlog, 3000);
  paxos2->init(strConfig, 2, NULL);
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir43", true, 4 * 1024 * 1024);
  Paxos *paxos3= new Paxos(timeout, rlog, 3000);
  paxos3->init(strConfig, 3, NULL);
...
TEST(consensus, paxos_purgeLog_disable_appliedIndex) {
  ...
  uint64_t timeout= 12500;
  std::shared_ptr<PaxosLog> rlog;
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir51", true, 4 * 1024 * 1024);
  Paxos *paxos1= new Paxos(timeout, rlog, 3000);
  paxos1->init(strConfig, 1, NULL);
  paxos1->initAutoPurgeLog(true, false, NULL); // disable appliedIndex use commitIndex
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir52", true, 4 * 1024 * 1024);
  Paxos *paxos2= new Paxos(timeout, rlog, 3000);
  paxos2->init(strConfig, 2, NULL);
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir53", true, 4 * 1024 * 1024);
  Paxos *paxos3= new Paxos(timeout, rlog, 3000);
  paxos3->init(strConfig, 3, NULL);
...
TEST(consensus, paxos_purgeLog_local) {
  ...
  rlog= std::make_shared<RDPaxosLog>("paxosLogTestDir44", true, 4 * 1024 * 1024);
  ...
  sleep(1);
  EXPECT_EQ(paxos1->getConfig()->learnersToString(), std::string("127.0.0.1:11004$01"));
  ...
  paxos2->forcePurgeLog(true);
  msleep(1000);
  EXPECT_EQ(paxos2->getLog()->getLength(), 2); // still 2 because learner is shutdown
