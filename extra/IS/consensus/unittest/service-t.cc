...
void foo2()
{
  std::cout << "Execute foo2!!!"<< std::endl << std::flush;
}
...
TEST(Service, ThreadTimer)
{
  ...
  ThreadTimer *tt1= new ThreadTimer(tts, 0.2, ThreadTimer::Repeatable, foo1, 2);
  tt1->start();
  /* There is no need to delete tt2 or the new timer. */
  /*ThreadTimer *tt2= */new ThreadTimer(tts, 1.0, ThreadTimer::Oneshot, foo2);

  new ThreadTimer(tts, 0.5, ThreadTimer::Oneshot, [=](uint64_t i){std::cout<< "Execute index:"<< i<< std::endl << std::flush;}, 123);

  ThreadTimer *tt3= new ThreadTimer(tts, 2.0, ThreadTimer::Stage, []{std::cout<< "foo3"<< std::endl << std::flush;tt3Called.fetch_add(1);});
  tt3->start();
  std::cout << "StageExtraTime:" << tt3->getStageExtraTime() << std::endl << std::flush;
...
TEST(Service, ThreadTimerRepeatable)
{
  ...
  ThreadTimer *tmr= new ThreadTimer(tts, 0.3, ThreadTimer::Repeatable, ttCallback, &str);
  tmr->start();
...
TEST(Service, ThreadTimerRepeatableStop)
{
  ...
  std::string str("aaaaaaa");
  ThreadTimer *tmr= new ThreadTimer(tts, 2.0, ThreadTimer::Repeatable, ttCallback, &str);
  tmr->start();
...
TEST(Service, ThreadTimer_weight_repeat1)
{
  ...
  ThreadTimer *tt3= new ThreadTimer(tts, 10.0, ThreadTimer::Stage, []{std::cout<< "foo3"<< std::endl << std::flush;tt3Called.fetch_add(1);});
  tt3->setRandWeight(6);
  tt3->start();
