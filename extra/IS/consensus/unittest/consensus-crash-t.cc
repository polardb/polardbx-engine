 ...
 * $Id:  consensus-crash-t.cc,v 1.0 02/22/2017 02:11:11 PM yingqiang.zyq(yingqiang.zyq@alibaba-inc.com) $
 *
 ************************************************************************/
...
{
  /*
   * paxos1 has more log in term 0 (index:1-3), paxos2/paxos3 only have index:1 in term 0;
   * But paxos2 became leader in term 1.
   * This case used to check if the paxos1 can truncate the log(2-3).
   * We limit MaxPacketSize to 1 to let leader send on log for each time.
   */
  ...
  rlog1= rlog= std::make_shared<FilePaxosLog>("paxosLogTestDir1", FilePaxosLog::LTSync);
  EXPECT_EQ(rlog1->getLastLogIndex(), 0);
  Paxos *paxos1= new Paxos(timeout, rlog);
  rlog2= rlog= std::make_shared<FilePaxosLog>("paxosLogTestDir2", FilePaxosLog::LTSync);
  Paxos *paxos2= new Paxos(timeout, rlog);
  rlog3= rlog= std::make_shared<FilePaxosLog>("paxosLogTestDir3", FilePaxosLog::LTSync);
...
TEST(consensus, leader_has_more_log)
{
  /*
   * paxos1 has more log in term 0 (index:1-3), paxos2/paxos3 only have index:1 in term 0;
   * But paxos1 became leader in term 1.
   * We limit MaxPacketSize to 1 to let leader send on log for each time.
   * We should not push matchIndex/commitIndex when we not send this term's log entry to the follower.
   */
  std::vector<std::string> strConfig;
  strConfig.emplace_back("127.0.0.1:11001");
  strConfig.emplace_back("127.0.0.1:11002");
...
{
   //
   // paxos1 has more log in term 0 (index:1-3), paxos2/paxos3 only have index:1 in term 0, 2-3 in term 1;
   // This case used to check if the paxos2 can send 2-3 to paxos1, and the paxos1 can truncate the log(2-3).
   // We limit MaxPacketSize to 1 to let leader send on log for each time.
   //
  std::vector<std::string> strConfig;
  strConfig.emplace_back("127.0.0.1:11001");
...
TEST(consensus, follower_lose_log)
{
  ...
  const uint64_t timeout= 2000;
  std::shared_ptr<PaxosLog> rlog, rlog1, rlog2, rlog3;
  rlog1= rlog= std::make_shared<FilePaxosLog>("paxosLogTestDir1", FilePaxosLog::LTMem);
  EXPECT_EQ(rlog1->getLastLogIndex(), 0);
  Paxos *paxos1= new Paxos(timeout, rlog);
  rlog2= rlog= std::make_shared<FilePaxosLog>("paxosLogTestDir2", FilePaxosLog::LTMem);
  Paxos *paxos2= new Paxos(timeout, rlog);
  rlog3= rlog= std::make_shared<FilePaxosLog>("paxosLogTestDir3", FilePaxosLog::LTMem);
  Paxos *paxos3= new Paxos(timeout, rlog);
...
TEST(consensus, leader_truncate_configure_change)
{
  ...
  EXPECT_EQ(paxos1->changeLearners(Paxos::CCAddNode, config), PaxosErrorCode::PE_NONE);
  EXPECT_EQ(paxos1->changeLearners(Paxos::CCDelNode, config), PaxosErrorCode::PE_NONE);
