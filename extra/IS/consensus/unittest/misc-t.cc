 ...
 * $Id:  misc-t.cc,v 1.0 01/11/2017 10:59:25 AM yingqiang.zyq(yingqiang.zyq@alibaba-inc.com) $
 *
 ************************************************************************/
...
TEST(misc, SingleProcessQueue)
{
  ...
  std::thread th1(&SingleProcessQueue<uint64_t>::process, &testQueue, processCb);

  testQueue.push(new uint64_t(10));
  std::thread th2(&SingleProcessQueue<uint64_t>::process, &testQueue, processCb);
