 ...
 * $Id:  single_leader.cc,v 1.0 04/12/2017 3:40:23 PM jarry.zj(jarry.zj@alibaba-inc.com) $
 *
 ************************************************************************/
...
:commitIndex_(0), heartbeatTimeout_(1000), clusterId_(clusterId), mockLastIndex_(0), mockLastTerm_(0)
{
}

void SingleLeader::init(int port, std::string learnerAddr,
    uint64_t lastLogIndex, uint64_t lastTerm, uint64_t cacheSize, uint64_t ioThreadCnt,
    uint64_t workThreadCnt)
{
  mockLastTerm_ = lastTerm;
  mockLastIndex_ = lastLogIndex;
  easy_warn_log("single_leader : init mockLastIndex_ %llu, mockLastTerm_ %llu", mockLastIndex_, mockLastTerm_);
...
uint64_t SingleLeader::replicateLog(LogEntry &entry)
{
  ...
  easy_warn_log("single_leader : replicateLog write done logTerm(%ld), logIndex(%ld)\n", entry.term(), logIndex);
...
uint64_t SingleLeader::appendLogFillForEach(PaxosMsg *msg, LearnerServer *server)
{
  ...
  {
    easy_warn_log("single_leader: server %d 's prevLogIndex %d larger than lastLogIndex %d. Just ignore.\n", server->serverId, prevLogIndex, lastLogIndex);
  ...
  if (prevLogIndex > 0)
  {
    ...
    {
      easy_warn_log("single_leader: Fail to AppendLog to server %d, prevLogIndex(%ld) is less than mockLastIndex_(%ld).\n", msg->serverid(), prevLogIndex, mockLastIndex_);
      return size;
    }
    ...
    else
    {
      ...
      if (ret != 0)
      {
        easy_warn_log("single_leader: getEntry fail for prevLogIndex(%ld) in Fill AppendLog to server %d, miss log entries.\n", prevLogIndex, msg->serverid());
        return size;
      }
      else
      {
  ...
  if (lastLogIndex >= server->nextIndex)
  {
    ...
    for (uint64_t i = server->nextIndex; i <= lastLogIndex; ++i)
    {
      ...
      {
        easy_warn_log("single_leader: getEntry fail for entries(i:%ld) in Fill AppendLog to server %d, miss log entries\n", i, msg->serverid());
...
int SingleLeader::onAppendLogResponce(PaxosMsg *msg)
{
  ...
  {
    easy_error_log("single_leader: onAppendLogResponce receive a msg msgId(%llu) from server %llu which is a wrong learner.\n", msg->msgid(), msg->serverid());
    return -1;
  }
  learnerServer_->waitForReply = 0;
  learnerServer_->appliedIndex = msg->appliedindex(); /* we do not care about appliedIndex here, just update */
  ...
  if (msg->issuccess())
  {
    ...
    {
      easy_warn_log("single_leader: onAppendLogResponce this response of AppendLog to server %d may be a resend msg that we have already received, msg index(%ld) is smaller than nextIndex(%ld)\n", msg->serverid(), msg->lastlogindex(), learnerServer_->nextIndex.load());
    }
    else
    {
    ...
    easy_warn_log("single_leader: msgId(%llu) AppendLog to server %d success, matchIndex(old:%llu,new:%llu) and nextIndex(old:%llu,new:%llu) have changed\n", msg->msgid(), msg->serverid(), oldMatchIndex, learnerServer_->matchIndex.load(), oldNextIndex, learnerServer_->nextIndex.load());
  ...
  else
  {
    ...
    learnerServer_->matchIndex = msg->lastlogindex();
    learnerServer_->nextIndex = learnerServer_->matchIndex + 1;
    if (oldNextIndex != learnerServer_->nextIndex || oldMatchIndex != learnerServer_->matchIndex)
    {
      /* clear the mempaxoslog before learnerServer's matchIndex, release the cachesize */
      log_->truncateForward(learnerServer_->matchIndex);
      /* send the remaining log */
      appendLogToLearner();
      easy_error_log("single_leader: Learner(%d) change its local log position! We reset server(learner)'s matchIndex(old:%llu,new:%llu) and nextIndex(old:%llu,new:%llu).", msg->serverid(), oldMatchIndex, learnerServer_->matchIndex.load(), oldNextIndex, learnerServer_->nextIndex.load());
    }
  }
...
uint64_t SingleLeader::getLastLogIndex()
{
  return log_->getLastLogIndex();
}

void SingleLeader::stop()
{
  srv_->stop();
}
