 ...
 * $Id:  learner_server.cc,v 1.0 05/08/2017 3:40:11 PM jarry.zj(jarry.zj@alibaba-inc.com) $
 *
 ************************************************************************/
 ...
 */
LearnerServer::LearnerServer(uint64_t serverId): RemoteServer(serverId), singleLeader(NULL), connectTimeout(1000)
{
}
...
{
  if (addr.port == 0)
    addr = srv->createConnection(strAddr, getSharedThis(), connectTimeout, serverId);
}
...
void LearnerServer::onConnectCb()
{
  ...
  if (ni == singleLeader->mockLastIndex_)
    ni++;
  nextIndex.store(ni);
  easy_warn_log("LearnerServer : update server %d 's nextIndex(new:%llu) when onConnect\n", serverId, nextIndex.load());
...
void LearnerServer::sendMsg(void * ptr)
{
  ...
  if (waitForReply == 1)
  {
    easy_warn_log("Try to send msg to server %ld, now we are waiting for response.", serverId);
    return;
  }
