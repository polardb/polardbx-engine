...
static int easy_kfc_add_node(easy_kfc_t *kfc, easy_pool_t *pool,
                             easy_hash_t *table, char *host, int port, uint64_t group_id, int role);
...
void *easy_kfc_decode(easy_message_t *m)
{
    ...
    // alloc packet
    if ((packet = (easy_kfc_packet_t *)easy_pool_calloc(m->pool,
                  sizeof(easy_kfc_packet_t))) == NULL) {
        m->status = EASY_ERROR;
        return NULL;
    }
...
error_exit:
    ...
    if (array) easy_array_destroy(array);
...
int easy_kfc_recv_message(easy_kfc_agent_t *agent, char *data, int len)
{
    ...
    if (resp) {
        ...
        if(ret > 0) memcpy(data, resp->data, ret);
...
static int easy_kfc_ping(easy_kfc_agent_t *agent)
{
    ...
    if (cnt > 0) {
        ...
        easy_list_for_each_entry_safe(s, s2, &agent->wobj.session_list, session_list_node) {
            ...
            if (packet && packet->group_id == agent->group_id) cnt ++;
...
void easy_kfc_destroy(easy_kfc_t *kfc)
{
    ...
    if (kfc->pool) easy_pool_destroy(kfc->pool);
...
// 增加node
static int easy_kfc_add_node(easy_kfc_t *kfc,
                             easy_pool_t *pool, easy_hash_t *table,
                             char *host, int port, uint64_t group_id, int role)
{
    easy_addr_t             addr;
    easy_kfc_group_t        *group = NULL;
    ...
    easy_debug_log("host=%s port=%d group=%.*s role=%s", host, port, sizeof(uint64_t),
                   &group_id, ((role == EASY_KFC_ROLE_SERVER) ? "server" : "client"));

    addr = easy_inet_str_to_addr(host, port);

    if (addr.family == 0) {
        easy_debug_log("ERROR: host=%s port=%d group=%.*s role=%s", host, port, sizeof(uint64_t),
                       &group_id, ((role == EASY_KFC_ROLE_SERVER) ? "server" : "client"));
        return EASY_ERROR;
    }
    ...
    if ((node = (easy_kfc_node_t *)easy_client_list_find(node_table, &addr)) != NULL) {
        easy_debug_log("DUPLICATE ERROR: host=%s port=%d group=%.*s role=%s", host, port, sizeof(uint64_t),
                       &group_id, ((role == EASY_KFC_ROLE_SERVER) ? "server" : "client"));
        return EASY_ERROR;
    }
...
static void easy_kfc_local_group(easy_kfc_t *kfc, easy_hash_t *group_list)
{
    ...
    for(n = 0; n < group_list->size; n++) {
        ...
        while(node) {
            ...
            // old group
            if (kfc->group_list && (ogroup = (easy_kfc_group_t *)easy_hash_find(kfc->group_list,
                                             hgroup->group_id)) != NULL) {
...
static void easy_kfc_do_connect(easy_kfc_agent_t *agent)
{
    ...
    if (kfc->version != agent->version) {
        ...
        if (oldcnt > 0) memcpy(addr, agent->slist.addr, sizeof(easy_kfc_node_t *) * oldcnt);
...
static void easy_kfc_clear_node(easy_hash_t *group_list)
{
    ...
    easy_hash_for_each(n, hnode, group_list) {
        ...
        if (group->server_list) easy_hash_clear(group->server_list);

        if (group->client_list) easy_hash_clear(group->client_list);
    }
}
