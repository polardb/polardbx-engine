...
public class TextStoreCommandUnitTest extends BaseTextCommandUnitTest {
	...
	public void testTairSetEncode() {
		Command command = this.commandFactory.createTairSetCommand(key, key
				.getBytes(), exp, value, false, namespace, version, transcoder);
		assertNull(command.getIoBuffer());
		command.encode();
		checkByteBufferEquals(command, "tair_set test 0 0 2 0 0\r\n10\r\n");
		command = this.commandFactory.createTairSetCommand(key, key.getBytes(),
				exp, value, true, namespace, version, transcoder);
		command.encode();
		checkByteBufferEquals(command, "tair_set test 0 0 2 0 0 noreply\r\n10\r\n");
	}
