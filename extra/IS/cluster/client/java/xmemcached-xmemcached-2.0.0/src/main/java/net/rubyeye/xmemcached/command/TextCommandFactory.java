...
import net.rubyeye.xmemcached.command.text.*;
...
public class TextCommandFactory implements CommandFactory {
	...
	@SuppressWarnings("unchecked")
	public final Command createTairSetCommand(final String key, final byte[] keyBytes,
										final int exp, final Object value, boolean noreply,
										final int namespace, final int version,
										Transcoder transcoder) {
		return this.createTairStoreCommand(key, keyBytes, exp, value, CommandType.TAIR_SET,
				noreply, namespace, version, transcoder);
	}
	...
	@SuppressWarnings("unchecked")
	final Command createTairStoreCommand(String key, byte[] keyBytes, int exp,
									     Object value, CommandType cmdType, boolean noreply,
										 int namespace, int version, Transcoder transcoder) {
		return new TairTextStoreCommand(key, keyBytes, cmdType, new CountDownLatch(
				1), exp, -1, value, noreply, namespace, version, transcoder);
	}
