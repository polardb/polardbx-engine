...
DECLARE_string(paxoslog_dir);
DECLARE_bool(data_compress);
DECLARE_bool(paxoslog_sync);
...
void StateMachine::applyLogThread()
{
  while (!stop_)
  {
    while (!stop_ && paxos_->getCommitIndex() <= lastAppliedIndex_)
    {
      paxos_->waitCommitIndexUpdate(lastAppliedIndex_);
    }
    ...
    uint64_t commitIndex= paxos_->getCommitIndex();
    
    for (uint64_t i= lastAppliedIdx + 1; i <= commitIndex; i++)
    {
      LogEntry logEntry;
      int ret= paxosLog_->getEntry(i, logEntry);
      if (ret != 0)
      {
        LOG_INFO("Fail to get entry idx: %ld from paxos log db!\n", i);
        abort();
      }
      ...
      switch (logEntry.optype())
      {
        ...
        case kNop:
          {
            // Before transfering to leader, paxos layer will always send
            ...
            if (paxos_->getState() == Paxos::State::LEADER && 
                leaderTerm_ == paxos_->getTerm())
            {
              LOG_INFO("Transfer to leader");
              state_.store(Paxos::State::LEADER);
            }
            break;
          }
        default:
...
int StateMachine::init(StateMachineService *srv)
{
  ...
  if (ret == -1) 
  {
    LOG_INFO("Init paxos log failed!\n");
    return ret;
  }
...
void StateMachine::stateChangeCb(enum Paxos::State paxosState)
{
  // Step down if we are no longer leader
  if (state_.load() == Paxos::LEADER)
  {
    if (paxosState != Paxos::LEADER)
    {
      LOG_INFO("Change from state %d to %d", state_.load(), paxosState);
      state_.store(paxosState);
    }
  }
  else
  {
    if (paxosState == Paxos::LEADER) 
    {
      ...
      // in applythread when we apply the empty log generated by paxos layer
      leaderTerm_= paxos_->getTerm();
...
{
  paxos_= std::shared_ptr<Paxos>(new Paxos(5000, paxosLog_));
  paxos_->setStateChangeCb(std::bind(&StateMachine::stateChangeCb, this, std::placeholders::_1));
  return paxos_->init(rpcMembers_, FLAGS_server_id, NULL);
}
...
int StateMachine::get(easy_request_t *r, TextRequest *cmd)
{
  ...
  if (cmd->getCommand() == TextCommand::GETS && 
      paxos_->getState() != Paxos::State::LEADER)
  {
    response->setResult(TEXT_ERROR);
    return srv_->sendResponse(r, response);
  }
  
  termBeforeGets= paxos_->getTerm();
...
int StateMachine::showStats(easy_request_t *r, TextRequest *cmd)
{
  ...
  if (cmd->getStats() == stats_t::CLUSTER)
  {
    if (state_.load() == Paxos::LEADER)
    {
      ...
      Paxos::ClusterInfoType cis[clusterSize];
      paxos_->getClusterInfo(cis, clusterSize);
...
{
  uint64_t curIndex= paxos_->replicateLog(logEntry);
  if (curIndex == -1)
  {
    response->setResult(TEXT_ERROR);
    return srv_->sendResponse(r, response);
