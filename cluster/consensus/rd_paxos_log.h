 ...
 * $Id:  rd_paxos_log.h,v 1.0 08/15/2016 02:46:50 PM hangfeng.fj(hangfeng.fj@alibaba-inc.com) $
 *
 ************************************************************************/
...
class RDPaxosLog : public PaxosLog {
  public:
    RDPaxosLog(const std::string &dataDir, bool compress, size_t writeBufferSize, bool sync=true);
    virtual ~RDPaxosLog();
    ...
    void setPurgeLogFilter(std::function<bool(const LogEntry &le)> cb) {purgeLogFilter_ = cb;}
    void debugSetLastLogIndex(uint i) {std::lock_guard<std::mutex> lg(lock_);firstLogIndex_= lastLogIndex_= i;}
  ...
  private:
    ...
    std::function<bool(const LogEntry &le)> purgeLogFilter_; /* act as a filter for purging log */
