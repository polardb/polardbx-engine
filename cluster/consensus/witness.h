 ...
 * $Id:  witness.h,v 1.0 12/20/2016 07:15:45 PM jarry.zj(jarry.zj@alibaba-inc.com) $
 *
 ************************************************************************/
...
public:
  Witness(std::string addr, std::shared_ptr<MemPaxosLog> log, uint64_t clusterId = 0,
  ...
  virtual int onAppendLogSendFail(PaxosMsg *msg, uint64_t *newId= nullptr) {return 0;}
  ...
  const uint64_t getCommitIndex() {return commitIndex_;}
  ...
  virtual int setClusterId(uint64_t ci) {clusterId_.store(ci); return 0;}
