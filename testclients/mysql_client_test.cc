...
static void test_bug17667() {
  ...
  for (statement_cursor = statements; statement_cursor->buffer != NULL;
    ...
    do {
      ...
      } while (strstr(line_buffer, statement_cursor->buffer) == NULL);
      hits++;
...
static void test_bug13001491() {
  ...
  DIE_UNLESS(mysql_real_connect(
      c, opt_host, "mysqltest_u1", NULL, current_db, opt_port, opt_unix_socket,
...
#define BIN_LOG_HEADER_SIZE 4U
  ...
  rc = test_mysql_binlog_perform(mysql1, 0, BIN_LOG_HEADER_SIZE, 0, 0, 0, 0);
  DIE_UNLESS(rc == 0);
  rc = test_mysql_binlog_perform(mysql1, "", BIN_LOG_HEADER_SIZE, 0, 0, 0, 0);
  DIE_UNLESS(rc == 0);
...
static void test_wl11772() {
  ...
  for (int i = 0; i < 3; ++i) {
    fprintf(stdout, "%d . Connecting to user %s@%s...", i, userhostpass[i].user,
            userhostpass[i].host);
    mysql_con1 = mysql_client_init(NULL);
...
static void *ccl_thread(void *param) {
  ...
  DIE_UNLESS(ccl_con);
  if (!mysql_real_connect(ccl_con, opt_host, opt_user, opt_password, NULL, opt_port, opt_unix_socket, 0))
  {
    myquery2(ccl_con, 1);
  }
  if(strlen(s->m_init_sql) != 0)
  {
    if(mysql_query(ccl_con, s->m_init_sql))
      myquery2(ccl_con, 1);
  }
  begintime = time(NULL);
