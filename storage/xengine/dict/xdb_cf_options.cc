...
#include "./xdb_compact_filter.h"

namespace myx {

Xdb_pk_comparator Xdb_cf_options::s_pk_comparator;
Xdb_rev_comparator Xdb_cf_options::s_rev_pk_comparator;

bool Xdb_cf_options::init(
    const xengine::table::BlockBasedTableOptions &table_options,
    std::shared_ptr<xengine::table::TablePropertiesCollectorFactory> prop_coll_factory,
  ...
  m_default_cf_opts.compaction_filter_factory.reset(
      new Xdb_compact_filter_factory);
...
void Xdb_cf_options::get(const std::string &cf_name,
                         xengine::common::ColumnFamilyOptions *const opts) {
  DBUG_ASSERT(opts != nullptr);

  // set defaults
  //xengine::common::GetColumnFamilyOptionsFromString(*opts, m_default_config, opts);

  // set per-cf config if we have one
  Name_to_config_t::iterator it = m_name_map.find(cf_name);
  if (it != m_name_map.end()) {
    xengine::common::GetColumnFamilyOptionsFromString(*opts, it->second, opts);
  }
}

bool Xdb_cf_options::set_default(const std::string &default_config) {
  xengine::common::ColumnFamilyOptions options;

  if (!default_config.empty() &&
      !xengine::common::GetColumnFamilyOptionsFromString(options, default_config,
                                                 &options)
           .ok()) {
...
void Xdb_cf_options::skip_spaces(const std::string &input, size_t *const pos) {
...
bool Xdb_cf_options::find_column_family(const std::string &input,
                                        size_t *const pos,
...
// braces.
bool Xdb_cf_options::find_options(const std::string &input, size_t *const pos,
...
bool Xdb_cf_options::find_cf_options_pair(const std::string &input,
                                          size_t *const pos,
                                          std::string *const cf,
...
bool Xdb_cf_options::set_override(const std::string &override_config) {
  ...
  while (pos < override_config.size()) {
    ...
    if (!xengine::common::GetColumnFamilyOptionsFromString(options, opt_str, &options)
             .ok()) {
      // NO_LINT_DEBUG
      sql_print_warning(
...
const xengine::util::Comparator *
Xdb_cf_options::get_cf_comparator(const std::string &cf_name) {
  if (Xdb_cf_manager::is_cf_name_reverse(cf_name.c_str())) {
...
void Xdb_cf_options::get_cf_options(const std::string &cf_name,
                                    xengine::common::ColumnFamilyOptions *const opts) {
  // DBUG_ASSERT(opts != nullptr);
