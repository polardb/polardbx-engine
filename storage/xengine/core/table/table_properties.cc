...
void AppendProperty(std::string& props, const std::string& key,
                    ...
                    const std::string& kv_delim) {
  AppendProperty(props, key, util::ToString(value), prop_delim, kv_delim);
}
...
std::string TableProperties::ToString(const std::string& prop_delim,
  ...
  AppendProperty(result, "# deletes", num_deletes, prop_delim, kv_delim);
...
const std::string TablePropertiesNames::kNumDeletes = "rocksdb.num.deletes";
