...
class PlainTableKeyEncoder {
 public:
  ...
  common::Status AppendKey(const common::Slice& key,
                           util::WritableFileWriter* file, uint64_t* offset,
...
class PlainTableFileReader {
 public:
  ...
  bool Read(uint32_t file_offset, uint32_t len, common::Slice* out) {
    if (file_info_->is_mmap_mode) {
      assert(file_offset + len <= file_info_->data_end_offset);
      *out = common::Slice(file_info_->file_data.data() + file_offset, len);
      return true;
    } else {
  ...
  // If return false, status code is stored in status_.
  bool ReadNonMmap(uint32_t file_offset, uint32_t len, common::Slice* output);

  // *bytes_read = 0 means eof. false means failure and status is saved
  // in status_. Not directly returning common::Status to save copying status
 ...
 private:
  ...
  common::Slice GetFromBuffer(Buffer* buf, uint32_t file_offset, uint32_t len);
};
...
class PlainTableKeyDecoder {
 public:
  ...
  //           indexes. Output.
  common::Status NextKey(uint32_t start_offset,
                         db::ParsedInternalKey* parsed_key,
                         common::Slice* internal_key, common::Slice* value,
                         uint32_t* bytes_read, bool* seekable = nullptr);

  common::Status NextKeyNoValue(uint32_t start_offset,
                                db::ParsedInternalKey* parsed_key,
                                common::Slice* internal_key,
                                uint32_t* bytes_read, bool* seekable = nullptr);
  ...
  uint32_t fixed_user_key_len_;
  common::Slice saved_user_key_;
  db::IterKey cur_key_;
  const common::SliceTransform* prefix_extractor_;
  bool in_prefix_;

 private:
  common::Status NextPlainEncodingKey(uint32_t start_offset,
                                      db::ParsedInternalKey* parsed_key,
                                      common::Slice* internal_key,
                                      uint32_t* bytes_read,
                                      bool* seekable = nullptr);
  common::Status NextPrefixEncodingKey(uint32_t start_offset,
                                       db::ParsedInternalKey* parsed_key,
                                       common::Slice* internal_key,
                                       uint32_t* bytes_read,
                                       bool* seekable = nullptr);
  common::Status ReadInternalKey(uint32_t file_offset, uint32_t user_key_size,
                                 db::ParsedInternalKey* parsed_key,
                                 uint32_t* bytes_read, bool* internal_key_valid,
                                 common::Slice* internal_key);
  inline common::Status DecodeSize(uint32_t start_offset,
                                   PlainTableEntryType* entry_type,
                                   uint32_t* key_size, uint32_t* bytes_read);
};
