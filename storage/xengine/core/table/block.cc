...
void BlockIter::seek_end_key()
{
  if (!need_seek_end_key_ || 0 == end_ikey_.size()) {
    ...
    Slice value = Slice(data_ + offset, 0);
    IterKey key;
    key.Clear();
    uint32_t current = 0;
...
bool BlockIter::parse_next_key(uint32_t& current,
                               uint32_t& restart_index,
                               IterKey& key,
                               Slice& value,
                               bool& key_pinned) {
  current = next_entry_offset(value);
  const char* p = data_ + current;
  const char* limit = data_ + restarts_;  // Restarts come right after data
  if (p >= limit) {
    // No more entries to return.  Mark as invalid.
    current = restarts_;
  ...
  if (p == nullptr || key.Size() < shared) {
    ...
    if (global_seqno_ != kDisableGlobalSequenceNumber) {
      ...
      // expect that all encoded sequence numbers are zeros and all value
      // types are kTypeValue
      assert(GetInternalKeySeqno(key.GetInternalKey()) == 0);
      assert(ExtractValueType(key.GetInternalKey()) == ValueType::kTypeValue);
      ...
      key.UpdateInternalKey(global_seqno_, ValueType::kTypeValue);
    }

    value = Slice(p + non_shared, value_length);
    while (restart_index + 1 < num_restarts_ &&
...
Block::Block(BlockContents&& contents, SequenceNumber _global_seqno,
  ...
  if (read_amp_bytes_per_bit != 0 && size_ != 0) {
//    read_amp_bitmap_.reset(new BlockReadAmpBitmap(restart_offset_, read_amp_bytes_per_bit));
    read_amp_bitmap_.reset(MOD_NEW_OBJECT(
        memory::ModId::kDefaultMod, BlockReadAmpBitmap, restart_offset_, read_amp_bytes_per_bit));
  }
}
