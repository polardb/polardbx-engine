...
class PlainTableIndex {
 public:
  ...
  explicit PlainTableIndex(common::Slice data) { InitFromRawData(data); }
  ...
  common::Status InitFromRawData(common::Slice data);
  ...
  const char* GetSubIndexBasePtrAndUpperBound(uint32_t offset,
    ...
    return util::GetVarint32Ptr(index_ptr, index_ptr + 4, upper_bound);
  }
...
class PlainTableIndexBuilder {
 public:
  PlainTableIndexBuilder(util::Arena* arena,
  ...
  void AddKeyPrefix(common::Slice key_prefix_slice, uint32_t key_offset);

  common::Slice Finish();

  uint32_t GetTotalSize() const {
    return util::VarintLength(index_size_) + util::VarintLength(num_prefixes_) +
 ...
 private:
  ...
  // data structures.
  common::Slice FillIndexes(const std::vector<IndexRecord*>& hash_to_offsets,
                            const std::vector<uint32_t>& entries_per_bucket);

  util::Arena* arena_;
  const common::ImmutableCFOptions ioptions_;
  monitor::HistogramImpl keys_per_prefix_hist_;
