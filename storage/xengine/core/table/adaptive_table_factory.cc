...
common::Status AdaptiveTableFactory::NewTableReader(
  ...
  if (footer.table_magic_number() == kPlainTableMagicNumber ||
    ...
    return plain_table_factory_->NewTableReader(
        table_reader_options, file, file_size, table, arena);
    ...
    return common::Status::NotSupported("Unidentified table format");
  }
}
