...
InternalIterator* CuckooTableReader::NewIterator(
    const ReadOptions& read_options, memory::SimpleAllocator* arena,
    bool skip_filters, const uint64_t scan_add_blocks_limit) {
  UNUSED(scan_add_blocks_limit);
  if (!status().ok()) {
...
//    iter = new CuckooTableIterator(this);
    iter = MOD_NEW_OBJECT(memory::ModId::kDbIter, CuckooTableIterator, this);
  } else {
    auto iter_mem = arena->alloc(sizeof(CuckooTableIterator));
