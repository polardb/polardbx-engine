...
class BlockPrefixIndex::Builder {
 public:
  ...
  BlockPrefixIndex* Finish() {
    ...
    uint32_t* block_array_buffer = (uint32_t *)memory::base_malloc(total_block_array_entries * sizeof(uint32_t));
//        new uint32_t[total_block_array_entries];
    uint32_t* buckets = (uint32_t *)memory::base_malloc(num_buckets * sizeof(uint32_t));
//        new uint32_t[num_buckets];
    uint32_t offset = 0;
    for (uint32_t i = 0; i < num_buckets; i++) {
      uint32_t num_blocks = num_blocks_per_bucket[i];
      if (num_blocks == 0) {
