...
void TableReaderBenchmark(Options& opts, EnvOptions& env_options,
  ...
  std::string file_name = test::TmpDir() + "/rocksdb_table_reader_benchmark";
...
//    unique_ptr<WritableFile> file;
    ...
    file_writer = MOD_NEW_OBJECT(memory::ModId::kDefaultMod, WritableFileWriter, file, env_options);
    storage::LayerPosition output_layer_position;
    tb = opts.table_factory->NewTableBuilder(
        TableBuilderOptions(
            ioptions, ikc, &int_tbl_prop_collector_factories,
            CompressionType::kNoCompression, CompressionOptions(),
            nullptr /* compression_dict */, false /* skip_filters */,
            kDefaultColumnFamilyName, output_layer_position),
...
//        new RandomAccessFileReader(raf));
    RandomAccessFileReader *file_reader = MOD_NEW_OBJECT(memory::ModId::kDefaultMod, RandomAccessFileReader, raf);
    s = opts.table_factory->NewTableReader(
        TableReaderOptions(ioptions, env_options, ikc), file_reader,
...
int main(int argc, char** argv) {
  ...
  if (FLAGS_prefix_len < 16) {
    options.prefix_extractor.reset(NewFixedPrefixTransform(FLAGS_prefix_len));
  }
  ReadOptions ro;
  ...
  if (FLAGS_table_factory == "cuckoo_hash") {
    ...
    tf.reset(NewCuckooTableFactory(table_options));
#else
    ...
    tf.reset(new PlainTableFactory(plain_table_options));
    options.prefix_extractor.reset(NewFixedPrefixTransform(FLAGS_prefix_len));
#else
    ...
    tf.reset(new BlockBasedTableFactory());
  } else {
