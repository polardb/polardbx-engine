...
class HashIndexBuilder : public IndexBuilder {
 public:
  explicit HashIndexBuilder(const db::InternalKeyComparator* comparator,
                            const common::SliceTransform* hash_key_extractor,
                            int index_block_restart_interval)
      : IndexBuilder(comparator),
        primary_index_builder_(comparator, index_block_restart_interval),
        hash_key_extractor_(hash_key_extractor) {}

  using IndexBuilder::AddIndexEntry;
  virtual void AddIndexEntry(std::string* last_key_in_current_block,
                             const common::Slice* first_key_in_next_block,
                             const BlockHandle& block_handle) override {
  ...
  virtual void OnKeyAdded(const common::Slice& key) override {
 ...
 private:
  void FlushPendingPrefix() {
    ...
    util::PutVarint32Varint32Varint32(
        &prefix_meta_block_,
  ...
  const common::SliceTransform* hash_key_extractor_;
 ...
 * continiously until common::Status::OK() is returned.
 *
...
class PartitionedIndexBuilder : public IndexBuilder {
 public:
  ...
  explicit PartitionedIndexBuilder(const db::InternalKeyComparator* comparator,
                                   const BlockBasedTableOptions& table_opt);
