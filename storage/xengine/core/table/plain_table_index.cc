...
Slice PlainTableIndexBuilder::Finish() {
  ...
  keys_per_prefix_hist_.Add(num_keys_per_prefix_);
  __XENGINE_LOG(INFO, "Number of Keys per prefix Histogram: %s", keys_per_prefix_hist_.ToString().c_str());
...
Slice PlainTableIndexBuilder::FillIndexes(
  ...
  __XENGINE_LOG(DEBUG, "Reserving %" PRIu32 " bytes for plain table's sub_index", sub_index_size_);
  auto total_allocate_size = GetTotalSize();
  char* allocated = arena_->AllocateAligned(
      total_allocate_size, huge_page_tlb_size_);
  ...
  __XENGINE_LOG(DEBUG, "hash table size: %d, suffix_map length %" ROCKSDB_PRIszt, index_size_, sub_index_size_);
