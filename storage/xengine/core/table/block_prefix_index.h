...
class BlockPrefixIndex {
 public:
  ...
  // not exist.
  uint32_t GetBlocks(const common::Slice& key, uint32_t** blocks);
  ...
  static common::Status Create(const common::SliceTransform* hash_key_extractor,
                               const common::Slice& prefixes,
                               const common::Slice& prefix_meta,
...
//    delete[] block_array_buffer_;
    ...
    memory::base_free(block_array_buffer_);
  }
 ...
 private:
  ...
  BlockPrefixIndex(const common::SliceTransform* internal_prefix_extractor,
  ...
  const common::SliceTransform* internal_prefix_extractor_;
