...
PlainTableReader::PlainTableReader(const ImmutableCFOptions& ioptions,
    ...
    : internal_comparator_(icomparator),
      ...
      arena_(Arena::kMinBlockSize, 0, memory::ModId::kTableCache),
      ioptions_(ioptions),
...
//    return new PlainTableIterator(this, prefix_extractor_ != nullptr);
    return MOD_NEW_OBJECT(memory::ModId::kDbIter, PlainTableIterator, this, prefix_extractor_ != nullptr);
  } else {
//    auto mem = arena->AllocateAligned(sizeof(PlainTableIterator));
    auto mem = arena->alloc(sizeof(PlainTableIterator));
...
Status PlainTableReader::PopulateIndex(TableProperties* props,
  ...
  if ((ioptions_.prefix_extractor == nullptr) && (hash_table_ratio != 0)) {
...
bool PlainTableReader::MatchBloom(uint32_t hash) const {
  ...
  return bloom_.MayContainHash(hash);
}
...
uint64_t PlainTableReader::ApproximateOffsetOf(const Slice& key) { return 0; }
