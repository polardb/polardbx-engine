...
Cache::Handle* GetEntryFromCache(Cache* block_cache, const Slice& key,
  ...
  auto cache_handle = block_cache->Lookup(key, nullptr);
...
class PartitionIndexReader : public IndexReader, public Cleanable {
 public:
  ...
  virtual void set_mod_id(const size_t mod_id) const override {
    assert(index_block_);
    memory::update_mod_id(index_block_->data(), mod_id);
  }
...
Status BlockBasedTable::Open(const ImmutableCFOptions& ioptions,
  ...
  if (!s.ok()) {
    ...
    s = table::ReadMetaBlock(
        rep->file.get(), file_size, kBlockBasedTableMagicNumber, rep->ioptions,
...
void BlockBasedTable::set_mod_id(const size_t mod_id) const {
  if (nullptr != rep_ && rep_->index_reader) {
  ...
  if (s.ok()) {
    ...
    if (block_cache != nullptr && block->value->cachable() &&
      ...
      s = block_cache->Insert(block_cache_key, block->value,
                              ...
                              Cache::Priority::LOW, block->value->data());
      if (s.ok()) {
...
Status BlockBasedTable::PutDataBlockToCache(
  ...
  if (block_cache_compressed != nullptr && raw_block != nullptr &&
      raw_block->cachable()) {
    const void* raw_ptr = raw_block->data();
    s = block_cache_compressed->Insert(
        compressed_block_cache_key, raw_block, raw_block->usable_size(),
        &DeleteCachedEntry<Block>, nullptr, Cache::Priority::LOW, raw_ptr);
    if (s.ok()) {
...
BlockBasedTable::CachableEntry<FilterBlockReader> BlockBasedTable::GetFilter(
  ...
  auto cache_handle =
      GetEntryFromCache(block_cache, key, CountPoint::BLOCK_CACHE_FILTER_MISS,
  ...
  if (cache_handle != nullptr) {
    ...
    if (filter != nullptr) {
      ...
      if (s.ok()) {
        ...
        QUERY_COUNT_ADD(CountPoint::BLOCK_CACHE_FILTER_BYTES_INSERT,
                        filter->size());
        QUERY_COUNT_ADD(CountPoint::BLOCK_CACHE_BYTES_WRITE, filter->size());
      } else {
        QUERY_COUNT(CountPoint::BLOCK_CACHE_ADD_FAILURES);
        delete filter;
        return CachableEntry<FilterBlockReader>();
...
InternalIterator* BlockBasedTable::NewIndexIterator(
  ...
  auto cache_handle =
      GetEntryFromCache(block_cache, key, CountPoint::BLOCK_CACHE_INDEX_MISS,
...
InternalIterator* BlockBasedTable::NewIterator(const ReadOptions& read_options,
  ...
  return NewTwoLevelIterator(
      MOD_NEW_OBJECT(memory::ModId::kDbIter, BlockEntryIteratorState, this, read_options, skip_filters),
