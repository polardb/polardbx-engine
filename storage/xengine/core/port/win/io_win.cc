...
SSIZE_T pwrite(HANDLE hFile, const char* src, size_t numBytes,
  ...
  if (FALSE == WriteFile(hFile, src, static_cast<DWORD>(numBytes),
...
Status WinMmapFile::Truncate(uint64_t size) { return Status::OK(); }
...
Status WinMmapFile::Allocate(uint64_t offset, uint64_t len) {
  Status status;
  TEST_KILL_RANDOM("WinMmapFile::Allocate", xengine_kill_odds);
...
Status WinSequentialFile::PositionedRead(uint64_t offset, size_t n,
...
SSIZE_T WinRandomAccessImpl::ReadIntoBuffer(uint64_t user_offset,
  ...
  SSIZE_T read = PositionedReadInternal(buffer.Destination(), bytes_to_read,
                                        first_page_start);
...
SSIZE_T WinRandomAccessImpl::PositionedReadInternal(char* src, size_t numBytes,
                                                    uint64_t offset) const {
...
inline Status WinRandomAccessImpl::ReadImpl(uint64_t offset, size_t n,
                                            Slice* result,
                                            char* scratch) const {
  Status s;
  SSIZE_T r = -1;
  ...
  if (file_base_->use_direct_io()) {
    ...
    if (!read_ahead_ && random_access_max_buffer_size_ == 0) {
      CalculateReadParameters(buffer_.Alignment(), offset, bytes_requested,
                              actual_bytes_toread, first_page_start);

      assert(actual_bytes_toread > 0);

      r = ReadIntoOneShotBuffer(offset, first_page_start, actual_bytes_toread,
                                left, dest);
    } else {
...
inline void WinRandomAccessImpl::HintImpl(
    RandomAccessFile::AccessPattern pattern) {
...
void WinRandomAccessFile::EnableReadAhead() { HintImpl(SEQUENTIAL); }

bool WinRandomAccessFile::ShouldForwardRawRequest() const { return true; }

void WinRandomAccessFile::Hint(AccessPattern pattern) { HintImpl(pattern); }
...
inline Status WinWritableImpl::PreallocateInternal(uint64_t spaceToReserve) {
...
Status WinWritableImpl::AppendImpl(const Slice& data) {
  ...
  if (file_data_->use_direct_io()) {
    ...
    SSIZE_T ret = pwrite(file_data_->GetFileHandle(), data.data(), data.size(),
                         filesize_);
...
Status WinWritableImpl::PositionedAppendImpl(const Slice& data,
                                             uint64_t offset) {
  if (file_data_->use_direct_io()) {
  ...
  SSIZE_T ret =
      pwrite(file_data_->GetFileHandle(), data.data(), data.size(), offset);
...
inline Status WinWritableImpl::TruncateImpl(uint64_t size) {
  Status s =
...
Status WinWritableImpl::CloseImpl() {
  ...
  if (fsync(hFile) < 0) {
    auto lastError = GetLastError();
    s = IOErrorFromWindowsError(
        "fsync failed at Close() for: " + file_data_->GetName(), lastError);
  }
...
Status WinWritableImpl::AllocateImpl(uint64_t offset, uint64_t len) {
  Status status;
  TEST_KILL_RANDOM("WinWritableFile::Allocate", xengine_kill_odds);
...
Status WinWritableFile::Append(const Slice& data) { return AppendImpl(data); }
...
// Need to implement this so the file is truncated correctly
// when buffered and unbuffered mode
Status WinWritableFile::Truncate(uint64_t size) { return TruncateImpl(size); }

Status WinWritableFile::Close() { return CloseImpl(); }

// write out the cached data to the OS cache
// This is now taken care of the WritableFileWriter
Status WinWritableFile::Flush() { return Status::OK(); }

Status WinWritableFile::Sync() { return SyncImpl(); }

Status WinWritableFile::Fsync() { return SyncImpl(); }

uint64_t WinWritableFile::GetFileSize() { return GetFileSizeImpl(); }
...
bool WinRandomRWFile::ShouldForwardRawRequest() const { return true; }
...
Status WinRandomRWFile::Flush() { return Status::OK(); }

Status WinRandomRWFile::Sync() { return SyncImpl(); }

Status WinRandomRWFile::Close() { return CloseImpl(); }
