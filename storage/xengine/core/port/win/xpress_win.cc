...
PVOID CompressorAlloc(PVOID, SIZE_T size) { return je_malloc(size); }
...
char* Decompress(const char* input_data, size_t input_length,
  ...
  // use base_malloc relace new
  char* ptr = static_cast<char*>(
      util::base_malloc(decompressedBufferSize, util::ModId::kUncompressBlock));
  assert(ptr);
  outputBuffer.reset(ptr);
