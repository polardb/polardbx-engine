...
static void RunConcurrentRead(int run) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    util::Env::Default()->Schedule(ConcurrentReader, &state);
...
static void RunConcurrentInsert(int run, int write_parallelism = 4) {
  util::Env::Default()->SetBackgroundThreads(1 + write_parallelism,
                                             util::Env::Priority::LOW);
  const int seed = test::RandomSeed() + (run * 100);
  Random rnd(seed);
  const int N = 1000;
  const int kSize = 1000;
  for (int i = 0; i < N; i++) {
    ...
    for (int k = 0; k < kSize; k += write_parallelism) {
      ...
      for (int p = 0; p < write_parallelism; ++p) {
        util::Env::Default()->Schedule(ConcurrentWriter, &state);
      }
