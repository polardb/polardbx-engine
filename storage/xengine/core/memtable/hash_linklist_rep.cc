...
class HashLinkListRep : public MemTableRep {
 ...
 private:
  ...
  class FullListIterator : public MemTableRep::Iterator {
   public:
    explicit FullListIterator(MemtableSkipList* list, memory::Allocator* allocator)
...
void HashLinkListRep::Insert(KeyHandle handle) {
  ...
  if (bucket_entries_logging_threshold_ > 0 &&
    ...
    __XENGINE_LOG(INFO, "HashLinkedList bucket %" ROCKSDB_PRIszt
                        " has more than %d entries. Key to insert: %s",
...
MemTableRep::Iterator* HashLinkListRep::GetIterator(Arena* alloc_arena) {
  ...
  if (if_log_bucket_dist_when_flash_) {
    __XENGINE_LOG(INFO, "hashLinkedList Entry distribution among buckets: %s",
