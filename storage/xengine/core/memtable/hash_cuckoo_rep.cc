...
using namespace db;
using namespace memtable::stl_wrappers;
namespace xengine {
namespace memtable {
namespace {
...
void HashCuckooRep::Get(const db::LookupKey& key, void* callback_args,
...
void HashCuckooRep::Insert(KeyHandle handle) {
  ...
  if (FindCuckooPath(key, user_key, cuckoo_path_, &cuckoo_path_length,
    ...
    if (backup_table_.get() == nullptr) {
      ...
      backup_table_.reset(
          factory.CreateMemTableRep(compare_, allocator_, nullptr));
