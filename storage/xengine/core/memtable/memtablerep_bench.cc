...
#include <cstdio>
int main() {
  fprintf(stderr, "Please install gflags to run this benchmark tools\n");
  return 1;
}
#else
...
int main(int argc, char** argv) {
  ...
  InternalKeyComparator internal_key_comp(BytewiseComparator());
  MemTable::KeyComparator key_comp(internal_key_comp);
  Arena arena;
  WriteBufferManager wb(FLAGS_write_buffer_size);
  memtable::MemTableAllocator memtable_allocator(&arena, &wb);
  uint64_t sequence;
  ...
  while (benchmarks != nullptr) {
    ...
    if (name == Slice("fillseq")) {
      ...
      key_gen.reset(new KeyGenerator(&rng, SEQUENTIAL, FLAGS_num_operations));
      benchmark.reset(
          new FillBenchmark(memtablerep.get(), key_gen.get(), &sequence));
    } else if (name == Slice("fillrandom")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(
          new KeyGenerator(&rng, UNIQUE_RANDOM, FLAGS_num_operations));
      benchmark.reset(
          new FillBenchmark(memtablerep.get(), key_gen.get(), &sequence));
    } else if (name == Slice("readrandom")) {
      key_gen.reset(new KeyGenerator(&rng, RANDOM, FLAGS_num_operations));
      benchmark.reset(
          new ReadBenchmark(memtablerep.get(), key_gen.get(), &sequence));
    } else if (name == Slice("readseq")) {
      key_gen.reset(new KeyGenerator(&rng, SEQUENTIAL, FLAGS_num_operations));
      benchmark.reset(new SeqReadBenchmark(memtablerep.get(), &sequence));
    } else if (name == Slice("readwrite")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new KeyGenerator(&rng, RANDOM, FLAGS_num_operations));
      benchmark.reset(new ReadWriteBenchmark<ConcurrentReadBenchmarkThread>(
          memtablerep.get(), key_gen.get(), &sequence));
    } else if (name == Slice("seqreadwrite")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new KeyGenerator(&rng, RANDOM, FLAGS_num_operations));
      benchmark.reset(
          new ::ReadWriteBenchmark<SeqConcurrentReadBenchmarkThread>(
              memtablerep.get(), key_gen.get(), &sequence));
    } else {
