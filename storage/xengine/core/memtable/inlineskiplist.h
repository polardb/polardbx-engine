...
class InlineSkipList {
 ...
 public:
  ...
  // skiplist object.
  explicit InlineSkipList(Comparator cmp, memory::Allocator* allocator,
  ...
  // Return estimated number of entries bewteen start_key and end_key
  uint64_t EstimateCount(const char* start_key, const char* end_key) const;
 ...
 private:
  ...
  memory::Allocator* const allocator_;  // Allocator used for allocations of nodes
  ...
  bool KeyIsAfterNode(const common::Slice& key, Node* n) const;

  // Returns the earliest node with a key >= key.
  // Return nullptr if there is no such node.
  Node* FindGreaterOrEqual(const char* key) const;
  Node* FindGreaterOrEqual(const common::Slice& key) const;
...
int InlineSkipList<Comparator>::RandomHeight() {
  auto rnd = util::Random::GetTLSInstance();
...
template <class Comparator>
bool InlineSkipList<Comparator>::KeyIsAfterNode(const common::Slice& key,
                                                Node* n) const {
  // nullptr n is considered infinite
  assert(n != head_);
  return (n != nullptr) && (compare_(n->Key(), key) < 0);
...
template <class Comparator>
typename InlineSkipList<Comparator>::Node*
InlineSkipList<Comparator>::FindGreaterOrEqual(const common::Slice& key) const {
  // Note: It looks like we could reduce duplication by implementing
  // this function as FindLessThan(key)->Next(0), but we wouldn't be able
  // to exit early on equality and the result wouldn't even be correct.
  // A concurrent insert might occur after FindLessThan(key) but before
...
template <class Comparator>
uint64_t InlineSkipList<Comparator>::EstimateCount(const char* start_key,
                                                   const char* end_key) const {
  uint64_t count = 0;
  bool found = false;

  assert(compare_(start_key, end_key) < 0);

  const char* key = start_key;
  Node* x = head_;
  int level = GetMaxHeight() - 1;
  while (true) {
    assert(x == head_ || compare_(x->Key(), key) < 0);
    ...
    if (next == nullptr || compare_(next->Key(), key) >= 0) {
      if (!found) {
        if (next != nullptr) {
          // next_key >= key, set key with end_key
          if (compare_(next->Key(), end_key) <= 0) {
            key = end_key;
            found = true;
          }
        }
        ...
        if (level <= 1) {
          // don't down to level0 for perf optimize
          return count;
        } else {
          // Switch to next list
          count *= kBranching_;
...
InlineSkipList<Comparator>::InlineSkipList(const Comparator cmp,
    ...
    : kMaxHeight_(max_height),
      ...
      kScaledInverseBranching_((util::Random::kMaxNext + 1) / kBranching_),
      compare_(cmp),
