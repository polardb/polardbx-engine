...
TEST_F(ThreadLocalTest, UniqueIdTest) {
  port::Mutex mu;
  port::CondVar cv(&mu);
  uint64_t init_val = IDChecker::PeekId();
  ASSERT_EQ(IDChecker::PeekId(), init_val);
  // New ThreadLocal instance bumps id by 1
  {
    // Id used 0
    Params p1(&mu, &cv, nullptr, 1u);
    ASSERT_EQ(IDChecker::PeekId(), init_val + 1u);
    // Id used 1
    Params p2(&mu, &cv, nullptr, init_val + 1u);
    ASSERT_EQ(IDChecker::PeekId(), init_val + 2u);
    // Id used 2
    Params p3(&mu, &cv, nullptr, init_val + 1u);
    ASSERT_EQ(IDChecker::PeekId(), init_val + 3u);
    // Id used 3
    Params p4(&mu, &cv, nullptr, init_val + 1u);
    ASSERT_EQ(IDChecker::PeekId(), init_val + 4u);
  }
  // id 3, 2, 1, 0 are in the free queue in order
  ASSERT_EQ(IDChecker::PeekId(), init_val);

  // pick up 0
  Params p1(&mu, &cv, nullptr, init_val + 1u);
  ASSERT_EQ(IDChecker::PeekId(), init_val + 1u);
  // pick up 1
  Params* p2 = new Params(&mu, &cv, nullptr, init_val + 1u);
  ASSERT_EQ(IDChecker::PeekId(), init_val + 2u);
  // pick up 2
  Params p3(&mu, &cv, nullptr, init_val + 1u);
  ASSERT_EQ(IDChecker::PeekId(), init_val + 3u);
  ...
  ASSERT_EQ(IDChecker::PeekId(), init_val + 3u);
  // pick up 3
  Params p5(&mu, &cv, nullptr, init_val + 1u);
  // next new id
  ASSERT_EQ(IDChecker::PeekId(), init_val + 4u);
...
class TestA {
public:
  static std::atomic<int> dtor_counter_;
  TestA() : x_(999) {}
  ~TestA() { dtor_counter_.fetch_add(1); }
  int get() {return x_;}
  void set(int x) {x_ = x;}
private:
  int x_;
...
std::atomic<int> TestA::dtor_counter_;

TestA *new_testa() {
  return new TestA();
}

void delete_testa(void *obj) {
  auto testa = static_cast<TestA*>(obj);
  delete testa;
}

thread_local TestA *tls_testa = nullptr;

TEST_F(ThreadLocalTest, ClassThreadLocalObj) {
  tls_testa = new_testa();
  auto& th_main = ThreadLocalHelper::instance();
  th_main.register_deleter(delete_testa, tls_testa);
  std::vector<std::thread> threads;
  for (int i = 0; i != 10; ++i) {
    threads.push_back(std::thread([](){
      if (nullptr == tls_testa) {
        tls_testa = new_testa();
        auto& th = ThreadLocalHelper::instance();
        th.register_deleter(delete_testa, tls_testa);
      }
    }));
  }
  ...
  ASSERT_EQ(TestA::dtor_counter_.load(), 10);
}
...
TEST_F(ThreadLocalTest, DISABLED_MainThreadDiesFirst) {
  ...
  try {
#endif  // ROCKSDB_LITE
    port::Thread th(&AccessThreadLocal, nullptr);
    th.detach();
