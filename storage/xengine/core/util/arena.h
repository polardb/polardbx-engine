...
class Arena : public memory::Allocator {
 public:
  // No copying allowed
 ...
 private:
  ...
  typedef std::vector<char*, memory::stl_adapt_allocator<char *, memory::ModId::kArenaVector>> Blocks;
  Blocks blocks_;
...
inline char* Arena::Allocate(size_t bytes) {
  ...
  char *ptr = AllocateFallback(bytes, false /* unaligned */);
  if (nullptr != ptr && memory::ModId::kMemtable != mod_id_ && blocks_.size() > 0) {
    hold_size_ += bytes;
    memory::update_hold_size(bytes, mod_id_);
  }
  return ptr;
