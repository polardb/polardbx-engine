...
Status Truncate(Env* env, const std::string& filename, uint64_t length) {
  unique_ptr<SequentialFile, memory::ptr_destruct_delete<SequentialFile>> orig_file;
  SequentialFile *orig_file_ptr = nullptr;
  const EnvOptions options;
  Status s = env->NewSequentialFile(filename, orig_file_ptr, options);
  orig_file.reset(orig_file_ptr);
  if (!s.ok()) {
  ...
  if (s.ok()) {
    ...
    unique_ptr<WritableFile, memory::ptr_destruct_delete<WritableFile>> tmp_file;
    WritableFile *tmp_file_ptr = nullptr;
    s = env->NewWritableFile(tmp_name, tmp_file_ptr, options);
    tmp_file.reset(tmp_file_ptr);
    if (s.ok()) {
...
Status FaultInjectionTestEnv::NewDirectory(const std::string& name,
  ...
  Status s = target()->NewDirectory(name, ptr);
  r.reset(ptr);
  assert(s.ok());
  if (!s.ok()) {
    return s;
  }
  result = new TestDirectory(this, TrimDirname(name), r.release());
  return Status::OK();
}
...
Status FaultInjectionTestEnv::NewWritableFile(const std::string& fname,
  ...
  unique_ptr<WritableFile> ures;
  s = target()->NewWritableFile(fname, result, soptions);
  ures.reset(result);
  if (s.ok()) {
    result = new TestWritableFile(fname, std::move(ures), this);
