...
class DeleteScheduler {
 public:
  ...
  common::Status DeleteFile(const std::string& fname);
  ...
  std::map<std::string, common::Status> GetBackgroundErrors();

 private:
  common::Status MoveToTrash(const std::string& file_path,
                             std::string* path_in_trash);

  common::Status DeleteTrashFile(const std::string& path_in_trash,
  ...
  std::map<std::string, common::Status> bg_errors_;
  ...
  monitor::InstrumentedCondVar cv_;
  ...
  monitor::InstrumentedMutex file_move_mu_;
  Logger* info_log_;
