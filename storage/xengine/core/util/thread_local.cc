...
pthread_key_t ThreadLocalHelper::tls_key_;
thread_local ThreadLocalHelper::DeleterVector *ThreadLocalHelper::deleters_ = nullptr;

ThreadLocalHelper &ThreadLocalHelper::instance() {
  static ThreadLocalHelper th;
  static pthread_once_t key_once = PTHREAD_ONCE_INIT;
  int err = pthread_once(&key_once, make_key);
  if (err) {
    abort();
  }
  return th;
...
ThreadLocalHelper::~ThreadLocalHelper() {
  // main thread cleanup
  delete_entities(this);
}

int ThreadLocalHelper::register_deleter(UnrefHandler handler, void *ptr) {
  auto deleters = get_deleters();
  deleters->emplace_back(handler, ptr);
  return pthread_setspecific(tls_key_, this);
}

ThreadLocalHelper::DeleterVector *ThreadLocalHelper::get_deleters() {
  if (nullptr == deleters_) {
    deleters_ = new ThreadLocalHelper::DeleterVector(); // fail -> throw -> abort
  }
  return deleters_;
}

void ThreadLocalHelper::delete_entities(void *ctx) {
  auto deleters = get_deleters();
  for (auto iter = deleters->rbegin(); iter != deleters->rend(); ++iter) {
    UnrefHandler deleter = iter->first;
    void *object = iter->second;
    deleter(object);
  }
  // Thread is exiting, deleters will no longger be used.
  delete deleters_;
  deleters_ = nullptr;
}
