...
int SequentialFileReader::prefetch(size_t n, AIOHandle *aio_handle) {
  ...
  if (ISNULL(aio_handle)) {
    XENGINE_LOG(WARN, "aio handle is null");
    ret = Status::kInvalidArgument;
  } else {
    size_t offset = prefetch_offset_.fetch_add(n);
    AIOInfo aio_info;
    if (FAILED(file_->fill_aio_info(offset, n, aio_info))) {
      XENGINE_LOG(WARN, "failed to fill aio info", K(ret), K(offset), K(n));
    } else if (FAILED(aio_handle->prefetch(aio_info))) {
      XENGINE_LOG(WARN, "aio handle prefetch failed", K(ret), K(aio_info));
    }
    if (FAILED(ret)) {
...
int SequentialFileReader::read(size_t n, common::Slice* result,
  ...
  if (ISNULL(aio_handle)) {
    ...
    AIOInfo aio_info;
    if (FAILED(file_->fill_aio_info(offset, n, aio_info))) {
      XENGINE_LOG(WARN, "failed to fill io info", K(ret), K(offset), K(n));
    }  else if (FAILED(aio_handle->read_by_actual_size(aio_info.offset_,
                                            aio_info.size_, result, scratch))) {
      XENGINE_LOG(WARN, "aio handle read failed", K(ret), K(aio_info));
    }
  }
  IOSTATS_ADD(bytes_read, result->size());
  return ret;
                                 ...
                                 AIOHandle *aio_handle)
{
  ...
  if (nullptr != aio_handle) {
    ...
    if (FAILED(file_->fill_aio_info(offset, size, aio_info))) {
      XENGINE_LOG(WARN, "failed to fill io info", K(ret), K(offset), K(size));
    } else if (FAILED(aio_handle->read(aio_info.offset_, aio_info.size_, result, scratch))) {
      XENGINE_LOG(WARN, "aio handle read failed", K(ret), K(offset), K(size), K(aio_info));
                                     ...
                                     AIOHandle *aio_handle)
{
  ...
  if (ISNULL(aio_handle)) {
    ...
    if (FAILED(ret)) {
      ...
      ret = Status::kOk;
      aio_handle->aio_req_->status_ = Status::kErrorUnexpected;
      XENGINE_LOG(INFO, "aio prefetch failed, will try sync IO!");
    }
  }
  return ret;
}
