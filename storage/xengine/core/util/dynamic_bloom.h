...
class DynamicBloom {
 public:
  ...
  explicit DynamicBloom(
      memory::Allocator* allocator, uint32_t total_bits, uint32_t locality = 0,
      uint32_t num_probes = 6,
      uint32_t (*hash_func)(const common::Slice& key) = nullptr,
  ...
  void Add(const common::Slice& key);

  // Like Add, but may be called concurrent with other functions.
  void AddConcurrently(const common::Slice& key);
  ...
  bool MayContain(const common::Slice& key) const;
  ...
  common::Slice GetRawData() const {
    return common::Slice(reinterpret_cast<char*>(data_), GetTotalBits() / 8);
  }
 ...
 private:
  ...
  uint32_t (*hash_func_)(const common::Slice& key);
...
inline void DynamicBloom::Add(const common::Slice& key) {
  AddHash(hash_func_(key));
}

inline void DynamicBloom::AddConcurrently(const common::Slice& key) {
...
inline bool DynamicBloom::MayContain(const common::Slice& key) const {
