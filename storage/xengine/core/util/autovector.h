...
template <class T, size_t kSize = 8, int Mod = memory::ModId::kAutoVector, typename AllocatorT = memory::WrapAllocator>
class autovector {
 public:
  ...
  autovector() = default;
  autovector(AllocatorT &alloc_)
      : vect_(memory::stl_adapt_allocator<T, Mod>(alloc_)) {
  }
 ...
 private:
  ...
  std::vector<T, memory::stl_adapt_allocator<T, Mod>> vect_;
};

template <class T, size_t kSize, int Mod, class AllocatorT>
autovector<T, kSize, Mod, AllocatorT>& autovector<T, kSize, Mod, AllocatorT>::assign(const autovector& other) {
