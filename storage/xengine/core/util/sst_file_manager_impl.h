...
class SstFileManagerImpl : public SstFileManager {
 public:
  ...
  common::Status OnAddFile(const std::string& file_path);

  // DB will call OnDeleteFile whenever an sst file is deleted.
  common::Status OnDeleteFile(const std::string& file_path);

  // DB will call OnMoveFile whenever an sst file is move to a new path.
  common::Status OnMoveFile(const std::string& old_path,
  ...
  virtual common::Status ScheduleFileDeletion(const std::string& file_path);
