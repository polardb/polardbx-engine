...
bool CheckMemoryAllocated(size_t allocated, size_t expected) {
  ...
  // max_expected = expected * 1.25 to detect critical overallocation.
  size_t max_expected = expected + expected / 10 * 2.5;
...
static void ApproximateMemoryUsageTest(size_t huge_page_size) {
  const size_t kBlockSize = 4096 - memory::AllocMgr::kBlockMeta;
  const size_t kOrigBlockSize = 4096;
  const size_t kEntrySize = kOrigBlockSize / 8;
  const size_t kZero = 0;
  Arena arena(kOrigBlockSize, huge_page_size);
  ...
  const size_t kLeftSize = kBlockSize - kEntrySize * num_blocks;
  ...
  arena.AllocateAligned(kLeftSize);
  ASSERT_EQ(mem_usage, arena.MemoryAllocatedBytes());
  ASSERT_EQ(arena.ApproximateMemoryUsage(), usage + kLeftSize);
  usage = arena.ApproximateMemoryUsage();
...
static void SimpleTest(size_t huge_page_size) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (i > N / 10) {
      ASSERT_LE(arena.ApproximateMemoryUsage(),
                bytes * 1.250 + memory::AllocMgr::kBlockMeta * i);
    }
  }
