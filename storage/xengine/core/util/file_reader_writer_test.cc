...
//  unique_ptr<FakeWF> wf(new FakeWF);
  ...
  unique_ptr<WritableFileWriter> writer(
      new WritableFileWriter(wf, env_options));
  Random r(301);
...
//  unique_ptr<FakeWF> wf(new FakeWF());
  ...
  unique_ptr<WritableFileWriter> writer(
      new WritableFileWriter(wf, EnvOptions()));
...
//        test::GetWritableFileWriter(new test::StringSink(&control_contents_)));
    auto sink = MOD_NEW_OBJECT(memory::ModId::kDefaultMod, test::StringSink, &control_contents_);
    auto write_holder = std::unique_ptr<WritableFileWriter, memory::ptr_destruct_delete<WritableFileWriter>>(
        test::GetWritableFileWriter(sink));
    write_holder->Append(Slice(str));
    write_holder->Flush();
    auto read_holder = std::unique_ptr<RandomAccessFile, memory::ptr_destruct_delete<RandomAccessFile>>(
//        new test::StringSource(control_contents_));
        MOD_NEW_OBJECT(memory::ModId::kDefaultMod, test::StringSource, control_contents_));
    test_read_holder_ =
 ...
 private:
  ...
  std::unique_ptr<RandomAccessFile, memory::ptr_destruct_delete<RandomAccessFile>> test_read_holder_;
