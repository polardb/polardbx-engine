...
class FullFilterBitsBuilder : public FilterBitsBuilder {
 public:
  ...
  virtual Slice Finish(
      std::unique_ptr<char[], memory::ptr_delete<char>>* buf) override {
    ...
    util::EncodeFixed32(data + total_bits / 8 + 1,
...
char* FullFilterBitsBuilder::ReserveSpace(const int num_entry,
  ...
  data = reinterpret_cast<char*>(base_malloc(sz, memory::ModId::kFilterBlockCache));
  memset(data, 0, sz);
  return data;
}
...
class FixedSizeFullFilterBitsBuilder : public FullFilterBitsBuilder {
 public:
  ...
  virtual void AddKey(const Slice& key) override {
    AddHash(util::BloomHash(key), data_, num_lines_, total_bits_);
  }
  ...
  virtual Slice Finish(std::unique_ptr<char[],
    ...
    util::EncodeFixed32(data_ + total_bits_ / 8 + 1, num_lines_);
    buf->reset(data_);
    return Slice(data_, total_bits_ / 8 + 5);
  }
...
class FullFilterBitsReader : public FilterBitsReader {
 public:
  ...
  virtual bool MayMatch(const Slice& entry) override {
    ...
    uint32_t hash = util::BloomHash(entry);
...
void FullFilterBitsReader::GetFilterMeta(const Slice& filter,
  ...
  *num_lines = util::DecodeFixed32(filter.data() + len - 4);
}
...
class BloomFilterPolicy : public FilterPolicy {
 public:
  ...
  virtual const char* Name() const override {
    return "xengine.BuiltinBloomFilter";
  }
  ...
  virtual FilterBitsBuilder* GetFixedBitsBuilder(
      const size_t num_entries) const override {
    auto* filter = new FixedSizeFullFilterBitsBuilder(bits_per_key_,
                                                      num_probes_, num_entries);
    filter->Initialize();
    return filter;
  }
