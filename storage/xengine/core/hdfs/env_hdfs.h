...
class HdfsEnv : public Env {
 public:
  ...
  virtual common::Status NewSequentialFile(
  ...
  virtual common::Status FileExists(const std::string& fname);

  virtual common::Status GetChildren(const std::string& path,
                                     std::vector<std::string>* result);

  virtual common::Status DeleteFile(const std::string& fname);

  virtual common::Status CreateDir(const std::string& name);

  virtual common::Status CreateDirIfMissing(const std::string& name);

  virtual common::Status DeleteDir(const std::string& name);

  virtual common::Status GetFileSize(const std::string& fname, uint64_t* size);
  ...
  virtual common::Status LockFile(const std::string& fname, FileLock** lock);

  virtual common::Status UnlockFile(FileLock* lock);
  ...
  virtual common::Status GetTestDirectory(std::string* path) {
  ...
  virtual common::Status GetHostName(char* name, uint64_t len) {
    return posixEnv->GetHostName(name, len);
  }

  virtual common::Status GetCurrentTime(int64_t* unix_time) {
...
static const common::Status notsup;
...
class HdfsEnv : public Env {
 public:
  ...
  virtual common::Status GetChildren(
      const std::string& path, std::vector<std::string>* result) override {
    return notsup;
  }
  ...
  virtual common::Status CreateDir(const std::string& name) override {
    return notsup;
  }

  virtual common::Status CreateDirIfMissing(const std::string& name) override {
    return notsup;
  }

  virtual common::Status DeleteDir(const std::string& name) override {
    return notsup;
  }
  ...
  virtual common::Status UnlockFile(FileLock* lock) override { return notsup; }
  ...
  virtual common::Status GetHostName(char* name, uint64_t len) override {
    return notsup;
  }
