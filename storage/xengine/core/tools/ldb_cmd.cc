...
void PrintBucketCounts(const std::vector<uint64_t>& bucket_counts,
  ...
  fprintf(stdout, "Keys in range %s to %s : %lu\n",
          ReadableTime(time_point).c_str(), ReadableTime(ttl_end).c_str(),
...
void InternalDumpCommand::DoCommand() {
  ...
  if (print_stats_) {
    std::string stats;
    if (db_->GetProperty(GetCfHandle(), "xengine.stats", &stats)) {
      fprintf(stdout, "%s\n", stats.c_str());
    }
  }
  ...
  if (!db_) {
    ...
    switch (type) {
      ...
      case kDescriptorFile:
        //ManifestDumpCommand::dump_manifest_file(path_, /* verbose_ */ false, is_key_hex_,
        //                 /*  json_ */ false);
        break;
      default:
        exec_state_ = LDBCommandExecuteResult::Failed(
...
void DBDumperCommand::DoDumpCommand() {
  ...
  if (print_stats_) {
    std::string stats;
    if (db_->GetProperty("xengine.stats", &stats)) {
      fprintf(stdout, "%s\n", stats.c_str());
    }
  }
...
void ChangeCompactionStyleCommand::DoCommand() {
  ...
  for (int i = 0; i < db_->NumberLevels(GetCfHandle()); i++) {
    db_->GetProperty(GetCfHandle(),
                     "xengine.num-files-at-level" + NumberToString(i),
                     &property);
