...
#include <cstdio>
int main() {
  fprintf(stderr, "Please install gflags to run xengine tools\n");
  return 1;
}
#else
...
using namespace xengine::memtable;
...
DEFINE_uint64(db_write_buffer_size, Options().db_write_buffer_size,
...
DEFINE_int32(level1_extents_major_compaction_trigger,
             ...
             "Level1 major compaction start trigger");
...
namespace {
enum CompressionType StringToCompressionType(const char* ctype) {
  assert(ctype);
...
static enum CompressionType FLAGS_compression_type_e = kSnappyCompression;
...
class StressTest {
  ...
  bool BuildOptionsTable() {
    ...
    std::unordered_map<std::string, std::vector<std::string> > options_tbl = {
        ...
        {"level0_layer_num_compaction_trigger",
         {
             ToString(FLAGS_level0_layer_num_compaction_trigger),
             ToString(FLAGS_level0_layer_num_compaction_trigger + 2),
             ToString(FLAGS_level0_layer_num_compaction_trigger + 4),
         }},
        {"level1_extents_major_compaction_trigger",
         {
             ToString(FLAGS_level1_extents_major_compaction_trigger),
             ToString(FLAGS_level1_extents_major_compaction_trigger + 2000),
             ToString(FLAGS_level1_extents_major_compaction_trigger + 4000),
         }
        },
        {"level2_usage_percent",
         {
             ToString(FLAGS_level2_usage_percent),
             ToString(FLAGS_level2_usage_percent + 5),
 ...
 private:
  ...
  Status SetOptions(ThreadState* thread) {
    ...
    if (name == "soft_rate_limit" || name == "hard_rate_limit") {
      ...
      opts["level0_layer_num_compaction_trigger"] =
          options_table_["level0_layer_num_compaction_trigger"][value_idx];
      opts["level1_extents_major_compaction_trigger"] =
      ...
      if (!FLAGS_test_batches_snapshots &&
        ...
        if (thread->rand.OneIn(FLAGS_clear_column_family_one_in)) {
          ...
          std::string new_name = ToString(new_column_family_name_.fetch_add(1));
          {
            ...
            fprintf(
                stdout,
                "[CF %d] Dropping and recreating sub table. new name: %s\n",
                cf, new_name.c_str());
          }
          ...
          if (!s.ok()) {
            fprintf(stderr, "dropping sub table error: %s\n",
                    s.ToString().c_str());
            std::terminate();
          }
          //TODO:yuanfeng
  ...
  void VerificationAbort(SharedState* shared, std::string msg, int cf,
                         int64_t key) const {
    printf("Verification failed for sub table %d key %" PRIi64 ": %s\n", cf,
           key, msg.c_str());
  ...
  void PrintEnv() const {
    fprintf(stdout, "XEngine version           : %d.%d\n", kMajorVersion,
            kMinorVersion);
...
int main(int argc, char** argv) {
  ...
  xengine::tools::StressTest stress;
  if (stress.Run()) {
    return 0;
  } else {
    return 1;
