...
#include "xengine/utilities/sim_cache.h"
...
DEFINE_string(
    options_file, "",
    "The path to a XENGINE options file.  If specified, then db_bench will "
    "run with the XENGINE options in the default sub table of the "
    "specified options file. "
    "Note that with this setting, db_bench will ONLY accept the following "
    "XENGINE options related command-line arguments, all other arguments "
    "that are related to XENGINE options will be ignored:\n"
    "\t--use_existing_db\n"
    "\t--statistics\n"
    "\t--row_cache_size\n"
...
DEFINE_uint64(
    ...
    "If non-zero, db_bench will rate-limit the writes going into XENGINE. This "
    "is the global rate in bytes/second.");

DEFINE_uint64(
    benchmark_read_rate_limit, 0,
    "If non-zero, db_bench will rate-limit the reads from XENGINE. This "
          ...
          if (id_ == 0 && FLAGS_stats_per_interval) {
            ...
            if (db_with_cfh && db_with_cfh->num_created.load()) {
              for (size_t i = 0; i < db_with_cfh->num_created.load(); ++i) {
                if (db->GetProperty(db_with_cfh->cfh[i], "xengine.cfstats",
                                    &stats))
                  fprintf(stderr, "%s\n", stats.c_str());
                if (FLAGS_show_table_properties) {
                  for (int level = 0; level < FLAGS_num_levels; ++level) {
                    if (db->GetProperty(
                            db_with_cfh->cfh[i],
                            "xengine.aggregated-table-properties-at-level" +
                                ToString(level),
                            &stats)) {
              ...
              if (db->GetProperty("xengine.stats", &stats)) {
                fprintf(stderr, "%s\n", stats.c_str());
              }
              if (FLAGS_show_table_properties) {
                for (int level = 0; level < FLAGS_num_levels; ++level) {
...
// Current the following isn't equivalent to OS_LINUX.
  ...
  void PrintEnvironment() {
    fprintf(stderr, "XENGINE:    version %d.%d\n", kMajorVersion,
            kMinorVersion);
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      if (name == "fillseqdeterministic" ||
        ...
        PrintStats("xengine.stats");
      } else if (name == "resetstats") {
        ResetStats();
      } else if (name == "verify") {
        VerifyDBFromDB(FLAGS_truth_db);
      } else if (name == "levelstats") {
        PrintStats("xengine.levelstats");
      } else if (name == "sstables") {
        PrintStats("xengine.sstables");
      ...
      if (FLAGS_flush_write_buffer_after_test) {//we can flush for blockcache or rowcache test
        ...
        for (ColumnFamilyHandle* handle : db_.cfh) {
          ...
          db_impl->Flush(xengine::common::FlushOptions(), handle);
          index = index + 1;
        }
 ...
 private:
...
#ifndef ROCKSDB_LITE
    printf("Initializing XENGINE Options from the specified file\n");
    DBOptions db_opts;
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    printf("Initializing XENGINE Options from command-line flags\n");
...
//    xengine::fpga::FPGACompactionJob::get_instance().set_data_dir(FLAGS_db);
  ...
  void InitializeOptionsGeneral(Options* opts) {
    ...
    if (FLAGS_row_cache_size) {
      if (FLAGS_cache_numshardbits >= 1) {
        options.row_cache =
            NewRowCache(FLAGS_row_cache_size, FLAGS_cache_numshardbits, false,
                        0, 0, ModId::kRowCache, true);
      } else {
        options.row_cache = NewRowCache(FLAGS_row_cache_size, -1, false, 0, 0,
                                        ModId::kRowCache, true);
      }
    }
...
// optimization, so skip the check of the max level.
    ...
    return Status::OK();
#else
    fprintf(stderr, "XENGINE Lite doesn't support filldeterministic\n");
    return Status::NotSupported(
        "XENGINE Lite doesn't support filldeterministic");
#endif  // ROCKSDB_LITE
  }
  ...
  bool stress_check_external(const Status& s, const Slice& key,
    ...
    if (is_deleted) {
      ...
      XENGINE_LOG(ERROR, "key lost", K(thread_flag), K_(stress_cf_id), K(key_id));
      error_func(key_id);
      return true;  // Skip if this key is not in db.
    } else {
...
#pragma GCC pop_options
  ...
  void do_random_manual_compaction(ColumnFamilyHandle* cfh, DB* db) {
    ...
    if (0 == subtable_id) {
      db->CompactRange(
          xengine::common::CompactRangeOptions(), nullptr, nullptr, task_type);
    } else {
      db->CompactRange(
          xengine::common::CompactRangeOptions(), cfh, nullptr, nullptr, task_type);
    }
  }
