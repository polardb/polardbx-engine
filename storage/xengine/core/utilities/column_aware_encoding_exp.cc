...
//      unique_ptr<WritableFile> encoded_out_file;
      WritableFile *encoded_out_file = nullptr;
      ...
      if (FLAGS_format == "col") {
        reader.EncodeBlocks(kvp_cd, encoded_out_file, compression_type,
                            kv_pair_blocks, &encoded_blocks, FLAGS_stat);
      } else {  // row format
        reader.EncodeBlocksToRowFormat(encoded_out_file, compression_type,
...
//        unique_ptr<WritableFile> decoded_out_file;
        WritableFile *decoded_out_file = nullptr;
        ...
        sw.Start();
        if (FLAGS_format == "col") {
          reader.DecodeBlocks(kvp_cd, decoded_out_file, &encoded_blocks);
        } else {
          reader.DecodeBlocksFromRowFormat(decoded_out_file,
                                           &encoded_blocks);
        }
...
int main(int argc, char** argv) {
  ...
  ColumnAwareEncodingExp::Run(sst_file);
  return 0;
}
