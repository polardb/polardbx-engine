...
bool TransactionBaseImpl::IsSnapshotNeeded() {
  return snapshot_needed_;
}
...
Status TransactionBaseImpl::TryLock(ColumnFamilyHandle* column_family,
                                    ...
                                    bool lock_uk, const ReadOptions *options) {
  size_t key_size = 0;
  ...
  return TryLock(column_family, str, read_only, exclusive, untracked, lock_uk, options);
}
...
Status TransactionBaseImpl::RebuildFromWriteBatch(WriteBatch* src_batch) {
  struct IndexedWriteBatchBuilder : public WriteBatch::Handler {
    ...
    IndexedWriteBatchBuilder(Transaction* txn, DBImpl* db)
        : txn_(txn), db_(db), column_family_memtables_(db_->get_version_set()->get_global_ctx()->all_sub_table_->sub_table_map_, db_->get_version_set()->GetColumnFamilySet()) {
    ...
    ColumnFamilyHandle* GetColumnFamilyHandle(uint32_t column_family_id) {
      ...
      return column_family_memtables_.GetColumnFamilyHandle();
    }

    Status PutCF(uint32_t cf, const Slice& key, const Slice& val) override {
      return txn_->Put(GetColumnFamilyHandle(cf), key, val);
    }

    Status DeleteCF(uint32_t cf, const Slice& key) override {
      return txn_->Delete(GetColumnFamilyHandle(cf), key);
    }
    ...
    Status MarkEndPrepare(const Slice&, SequenceNumber) override {
      return Status::InvalidArgument();
    }

    Status MarkCommit(const Slice&, SequenceNumber) override {
      return Status::InvalidArgument();
    }

    Status MarkRollback(const Slice&, SequenceNumber) override {
      return Status::InvalidArgument();
    }
  };
