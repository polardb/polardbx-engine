...
Status TransactionImpl::CommitBatchAsync(WriteBatch* batch,
                                         AsyncCallback* call_back) {
  /// TransactionKeyMap* keys_to_unlock = new TransactionKeyMap();
  TransactionKeyMap* keys_to_unlock = &GetTrackedKeysRef();
  Status s = LockBatch(batch, keys_to_unlock);
  ...
  if (can_commit) {
    txn_state_.store(AWAITING_COMMIT);
    TransactionCallbackOnCommitBatch* trx_call_back =
        new TransactionCallbackOnCommitBatch(this, call_back);

    s = db_->WriteAsync(write_options_, batch, trx_call_back);
...
Status TransactionImpl::CommitAsync(AsyncCallback* call_back) {
  Status s;
  bool commit_single = false;
  bool commit_prepared = false;
  ...
  if (commit_single) {
    ...
    if (WriteBatchInternal::Count(GetCommitTimeWriteBatch()) > 0) {
      ...
      this->call_back_on_commit_.reset(this, call_back);

      s = db_->WriteAsync(write_options_, GetWriteBatch()->GetWriteBatch(),
                          &this->call_back_on_commit_);
      assert(s.ok());
      return s;
    }
  } else if (commit_prepared) {
...
Status TransactionImpl::TryLock(ColumnFamilyHandle* column_family,
                                ...
                                bool lock_uk, const ReadOptions *read_opts) {
  QUERY_TRACE_SCOPE(xengine::monitor::TracePoint::DB_TRY_LOCK);
  ...
  //later get snapshot,set no validateSnapshot flag.
  if (IsSnapshotNeeded()) {
  ...
  // future
  if (untracked || snapshot_ == nullptr || !validate_snapshot ) {
    // Need to remember the earliest sequence number that we know that this
    // key has not been modified after.  This is useful if this same
    // transaction
    // later tries to lock this key again.
    if (!validate_snapshot && snapshot_) {
      new_seqno = snapshot_->GetSequenceNumber();
    }
    else if (current_seqno == kMaxSequenceNumber) {
    ...
    if (s.ok()) {
      s = ValidateSnapshot(column_family, key, current_seqno, &new_seqno, lock_uk, read_opts);
...
Status TransactionImpl::ValidateSnapshot(ColumnFamilyHandle* column_family,
  ...
  assert(snapshot_);
  QUERY_TRACE_SCOPE(TracePoint::VALIDATE_SNAPSHOT);
...
Status TransactionCallbackOnCommit::call_back() {
  Status status;
  TransactionImpl* impl = (TransactionImpl*)(this->trx_impl_);
  impl->Clear();
  if (status.ok()) {
    impl->txn_state_.store(Transaction::TransactionState::COMMITED);
  }
  return status;
}

Status TransactionCallbackOnCommitBatch::call_back() {
  Status status;
  // CallBack for CommitAsync, we will free the trx_impl_;
  trx_impl_->Clear();
  if (status.ok()) {
    trx_impl_->txn_state_.store(Transaction::TransactionState::COMMITED);
  }
  delete trx_impl_;
  return status;
}
