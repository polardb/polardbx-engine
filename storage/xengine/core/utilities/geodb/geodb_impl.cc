...
std::string GeoDBImpl::MakeKey1(const GeoPosition& pos, Slice id,
                                std::string quadkey) {
  std::string lat = util::ToString(pos.latitude);
  std::string lon = util::ToString(pos.longitude);
  std::string key = "p:";
...
double GeoDBImpl::distance(double lat1, double lon1, double lat2, double lon2) {
  ...
  double a =
      (sin(lat / 2) * sin(lat / 2)) +
      cos(radians(lat1)) * cos(radians(lat2)) * (sin(lon / 2) * sin(lon / 2));
