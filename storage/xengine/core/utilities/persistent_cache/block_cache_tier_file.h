...
class BlockCacheFile : public LRUElement<BlockCacheFile> {
 public:
  ...
  explicit BlockCacheFile(util::Env* const env, const std::string& dir,
  ...
  virtual bool Append(const common::Slice& key, const common::Slice& val,
                      LBA* const lba) {
  ...
  virtual bool Read(const LBA& lba, common::Slice* key, common::Slice* block,
                    char* scratch) {
    assert(!"not implemented");
    return false;
  }
  ...
  virtual common::Status Delete(uint64_t* size);

 protected:
  port::RWMutex rwlock_;               // synchronization mutex
  util::Env* const env_ = nullptr;     // env for IO
...
class RandomAccessCacheFile : public BlockCacheFile {
 public:
  explicit RandomAccessCacheFile(util::Env* const env, const std::string& dir,
                                 const uint32_t cache_id)
  ...
  bool Read(const LBA& lba, common::Slice* key, common::Slice* block,
            char* scratch) override;

 private:
  std::unique_ptr<RandomAccessFileReader, memory::ptr_destruct_delete<RandomAccessFileReader>> freader_;

 protected:
  bool OpenImpl(const bool enable_direct_reads);
  bool ParseRec(const LBA& lba, common::Slice* key, common::Slice* val,
                char* scratch);
};
...
class WriteableCacheFile : public RandomAccessCacheFile {
 public:
  explicit WriteableCacheFile(util::Env* const env,
                              CacheWriteBufferAllocator* alloc, Writer* writer,
                              const std::string& dir, const uint32_t cache_id,
                              const uint32_t max_size)
      : RandomAccessCacheFile(env, dir, cache_id),
        alloc_(alloc),
        writer_(writer),
        max_size_(max_size) {}
  ...
  bool Append(const common::Slice&, const common::Slice&, LBA* const) override;
  // End-of-file
 ...
 private:
  ...
  bool ReadBuffer(const LBA& lba, common::Slice* key, common::Slice* block,
                  char* scratch);
  ...
  WritableFile *file_;   // env file abstraction
