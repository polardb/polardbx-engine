...
struct PersistentCacheConfig {
  explicit PersistentCacheConfig(
      util::Env* const _env, const std::string& _path,
  ...
  //
  common::Status ValidateSettings() const {
    // (1) check pre-conditions for variables
    if (!env || path.empty()) {
      return common::Status::InvalidArgument("empty or null args");
    }
    ...
    if (cache_size < cache_file_size || write_buffer_size >= cache_file_size ||
      ...
      return common::Status::InvalidArgument("invalid cache size");
    }
    ...
    if (!writer_qdepth || writer_dispatch_size > write_buffer_size ||
      ...
      return common::Status::InvalidArgument("invalid writer settings");
    }
...
class PersistentCacheTier : public cache::PersistentCache {
 public:
  ...
  virtual common::Status Open();

  // Close the persistent cache tier
  virtual common::Status Close();

  // Reserve space up to 'size' bytes
  virtual bool Reserve(const size_t size);

  // Erase a key from the cache
  virtual bool Erase(const common::Slice& key);

  // Print stats to string recursively
  virtual std::string PrintStats();

  virtual cache::PersistentCache::StatsType Stats();

  // Insert to page cache
  virtual common::Status Insert(const common::Slice& page_key, const char* data,
                                const size_t size) = 0;

  // Lookup page cache by page identifier
  virtual common::Status Lookup(const common::Slice& page_key,
...
class PersistentTieredCache : public PersistentCacheTier {
 public:
  ...
  common::Status Insert(const common::Slice& page_key, const char* data,
                        const size_t size) override;
  common::Status Lookup(const common::Slice& page_key,
                        std::unique_ptr<char[], memory::ptr_delete<char>>* data,
