...
Status VolatileCacheTier::Lookup(
    const Slice& page_key, std::unique_ptr<char[], memory::ptr_delete<char>>* result,
    size_t* size) {
  ...
  if (ok) {
    ...
    char* ptr = static_cast<char*>(
        base_malloc(kv->value.size(), memory::ModId::kVolatileCacheTr));
    if (kv->value.size() > 0 && nullptr == ptr) {
