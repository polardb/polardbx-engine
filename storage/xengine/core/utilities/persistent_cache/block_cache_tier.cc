...
Status BlockCacheTier::Open() {
  ...
  assert(status.ok());
  if (!status.ok()) {
    __XENGINE_LOG(ERROR, "Invalid block cache options");
    return status;
  }
  ...
  if (!status.ok()) {
    __XENGINE_LOG(ERROR, "Error creating directory %s. %s", opt_.path.c_str(),
  ...
  status = NewCacheFile();
  if (!status.ok()) {
    __XENGINE_LOG(ERROR, "Error creating new file %s. %s", opt_.path.c_str(),
                  status.ToString().c_str());
    return status;
  }
...
Status BlockCacheTier::CleanupCacheFolder(const std::string& folder) {
  ...
  if (!status.ok()) {
    __XENGINE_LOG(ERROR, "Error getting files for %s. %s", folder.c_str(),
                  status.ToString().c_str());
    return status;
  }
  ...
  for (auto file : files) {
    if (IsCacheFile(file)) {
      // cache file
      __XENGINE_LOG(INFO, "Removing file %s.", file.c_str());
      status = opt_.env->DeleteFile(folder + "/" + file);
      if (!status.ok()) {
        __XENGINE_LOG(ERROR, "Error deleting file %s. %s", file.c_str(),
                      status.ToString().c_str());
        return status;
      }
    } else {
      __XENGINE_LOG(DEBUG, "Skipping file %s", file.c_str());
    }
  }
  return Status::OK();
}
...
Status BlockCacheTier::InsertImpl(const Slice& key, const Slice& data) {
  ...
  while (!cache_file_->Append(key, data, &lba)) {
    if (!cache_file_->Eof()) {
      __XENGINE_LOG(DEBUG, "Error inserting to cache file %d", cache_file_->cacheid());
...
Status BlockCacheTier::Lookup(const Slice& key,
                              unique_ptr<char[], memory::ptr_delete<char>>* val,
                              size_t* size) {
  StopWatchNano timer(opt_.env, /*auto_start=*/true);
  ...
  assert(blk_key == key);
  char* obj_buf =
      static_cast<char*>(base_malloc(blk_val.size(), memory::ModId::kBlockCacheTier));
  val->reset(obj_buf);
...
Status BlockCacheTier::NewCacheFile() {
  ...
  __XENGINE_LOG(INFO, "Created cache file %d", writer_cache_id_);
  ...
  // assert(status);
  if (!status) {
    __XENGINE_LOG(ERROR, "Error inserting to metadata");
...
Status NewPersistentCache(Env* const env, const std::string& path,
  ...
  auto opt = PersistentCacheConfig(env, path, size);
