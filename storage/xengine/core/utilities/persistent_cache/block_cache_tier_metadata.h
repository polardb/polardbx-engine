...
struct BlockInfo {
  explicit BlockInfo(const common::Slice& key, const LBA& lba = LBA())
...
class BlockCacheTierMetadata {
 public:
  ...
  BlockInfo* Insert(const common::Slice& key, const LBA& lba);
  // bool Insert(BlockInfo* binfo);

  // Lookup block information from block index
  bool Lookup(const common::Slice& key, LBA* lba);

  // Remove a given from the block index
  BlockInfo* Remove(const common::Slice& key);
