 ...
 protected:
  ...
  void InsertImpl() {
    ...
    while (true) {
      ...
      common::Slice key(k);
      while (true) {
        common::Status status = cache_->Insert(key, data, sizeof(data));
        if (status.ok()) {
          break;
        }
        ASSERT_TRUE(status.IsTryAgain());
        util::Env::Default()->SleepForMicroseconds(1 * 1000 * 1000);
      }
    }
  }
  ...
  void VerifyImpl(const bool eviction_enabled = false) {
    ...
    while (true) {
      ...
      auto k = prefix + PaddedNumber(i, /*count=*/8);
      common::Slice key(k);
      unique_ptr<char[], ptr_delete<char>> block;
      size_t block_size;
...
class PersistentCacheDBTest : public db::DBTestBase {
 public:
  PersistentCacheDBTest();

  static uint64_t TestGetTickerCount(const common::Options& options,
                                     monitor::Tickers ticker_type) {
    return static_cast<uint32_t>(
        options.statistics->getTickerCount(ticker_type));
  }

  // insert data to table
  void Insert(const common::Options& options,
              const table::BlockBasedTableOptions& table_options,
              const int num_iter, std::vector<std::string>* values) {
    CreateAndReopenWithCF({"pikachu"}, options);
    // default column family doesn't have block cache
    common::Options no_block_cache_opts;
    no_block_cache_opts.statistics = options.statistics;
    no_block_cache_opts = CurrentOptions(no_block_cache_opts);
    table::BlockBasedTableOptions table_options_no_bc;
    table_options_no_bc.no_block_cache = true;
    no_block_cache_opts.table_factory.reset(
        NewBlockBasedTableFactory(table_options_no_bc));
    ReopenWithColumnFamilies(
        {"default", "pikachu"},
        std::vector<common::Options>({no_block_cache_opts, options}));
