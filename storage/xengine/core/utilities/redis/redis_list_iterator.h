...
class RedisListIterator {
 public:
  ...
  void InsertElement(const common::Slice& elem) {
  ...
  void GetCurrent(common::Slice* curElem) {
    // Ensure we are in a valid state
    CheckErrors();
    ...
    *curElem = common::Slice(data_ + cur_byte_ + sizeof(cur_elem_length_),
                             cur_elem_length_);
  }
  ...
  common::Slice WriteResult() {
    ...
    // Return the result
    return common::Slice(result_.data(), result_.size());
  }
 ...
 public:  // Static public functions
  ...
  static uint32_t SizeOf(const common::Slice& elem) {
    // [Integer Length . Data]
