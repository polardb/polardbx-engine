...
//  std::unique_ptr<RandomAccessFile> file;
  ...
  file_.reset(new RandomAccessFileReader(file));
  ...
  TableReader *tmp_table_reader = nullptr;
  block_table_factory->NewTableReader(
      TableReaderOptions(ioptions_, soptions_, internal_comparator_,
                         ...
                         false /* skip_filters */),
      file_.release(), file_size, tmp_table_reader, /*enable_prefetch=*/false);

  table_reader_.reset(dynamic_cast<BlockBasedTable*>(tmp_table_reader));
}
