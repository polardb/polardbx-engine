...
Status DBWithTTLImpl::CreateColumnFamilyWithTtl(
    CreateSubTableArgs &args, ColumnFamilyHandle** handle, int ttl) {
  //TODO:yuanfeng
  //ColumnFamilyOptions sanitized_options = options;
  //DBWithTTLImpl::SanitizeOptions(ttl, &sanitized_options, GetEnv());

  return DBWithTTL::CreateColumnFamily(args, handle);
}

Status DBWithTTLImpl::CreateColumnFamily(CreateSubTableArgs &args, ColumnFamilyHandle** handle) {
  return CreateColumnFamilyWithTtl(args, handle, 0);
}
