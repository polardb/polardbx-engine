...
#include "xengine/utilities/backupable_db.h"
#include "port/port.h"
...
void BackupableDBOptions::Dump() const {
  __XENGINE_LOG(INFO, "               Options.backup_dir: %s",
                 backup_dir.c_str());
  __XENGINE_LOG(INFO, "               Options.backup_env: %p", backup_env);
  __XENGINE_LOG(INFO, "        Options.share_table_files: %d",
                 static_cast<int>(share_table_files));
  __XENGINE_LOG(INFO, "                     Options.sync: %d",
                 static_cast<int>(sync));
  __XENGINE_LOG(INFO, "         Options.destroy_old_data: %d",
                 static_cast<int>(destroy_old_data));
  __XENGINE_LOG(INFO, "         Options.backup_log_files: %d",
                 static_cast<int>(backup_log_files));
  __XENGINE_LOG(INFO, "        Options.backup_rate_limit: %" PRIu64,
                 backup_rate_limit);
  __XENGINE_LOG(INFO, "       Options.restore_rate_limit: %" PRIu64,
                 restore_rate_limit);
  __XENGINE_LOG(INFO, "Options.max_background_operations: %d",
...
class BackupEngineImpl : public BackupEngine {
 public:
  ...
  ~BackupEngineImpl();
  Status CreateNewBackupWithMetadata(db::DB* db,
 ...
 private:
  ...
  class BackupMeta {
   public:
    ...
    std::shared_ptr<FileInfo> GetFile(const std::string& filename) const {
      ...
      if (it == file_infos_->end()) return nullptr;
      return it->second;
    }

    const std::vector<std::shared_ptr<FileInfo>>& GetFiles() { return files_; }
  ...
  inline std::string GetPrivateFileRel(BackupID backup_id, bool tmp = false,
    ...
    return GetPrivateDirRel() + "/" + util::ToString(backup_id) +
           (tmp ? ".tmp" : "") + "/" + file;
  }
  ...
  inline std::string GetSharedFileWithChecksum(const std::string& file,
    ...
    return file_copy.insert(
        ...
        "_" + util::ToString(checksum_value) + "_" + util::ToString(file_size));
  }
  ...
  inline std::string GetBackupMetaFile(BackupID backup_id) const {
    return GetBackupMetaDir() + "/" + util::ToString(backup_id);
  }
  ...
  std::map<BackupID, std::pair<Status, unique_ptr<BackupMeta>>>
      corrupt_backups_;
  ...
  // directories
  Directory *backup_directory_;
  Directory *shared_directory_;
  Directory *meta_directory_;
  Directory *private_directory_;
...
Status BackupEngineImpl::Initialize() {
  ...
  initialized_ = true;
  if (read_only_) {
    __XENGINE_LOG(INFO, "Starting read_only backup engine");
  }
  options_.Dump();

  if (!read_only_) {
    // gather the list of directories that we need to create
    std::vector<std::pair<std::string, Directory *>> directories;
  ...
  for (auto& file : backup_meta_files) {
    ...
    __XENGINE_LOG(INFO, "Detected backup %s", file.c_str());
    BackupID backup_id = 0;
    sscanf(file.c_str(), "%u", &backup_id);
    if (backup_id == 0 || file != util::ToString(backup_id)) {
      if (!read_only_) {
  ...
  if (options_.destroy_old_data) {  // Destroy old data
    ...
    for (auto backup_iter = backups_.rbegin();
      ...
      Status s = backup_iter->second->LoadFromFile(options_.backup_dir,
                                                   abs_path_to_size);
      if (s.IsCorruption()) {
        __XENGINE_LOG(INFO, "Backup %u corrupted -- %s",
        ...
        __XENGINE_LOG(INFO, "Loading backup %" PRIu32 " OK:\n%s",
                       backup_iter->first,
  ...
  __XENGINE_LOG(INFO, "Latest backup is %u", latest_backup_id_);
  ...
  __XENGINE_LOG(INFO, "Initialized BackupEngine");

  return Status::OK();
}

Status BackupEngineImpl::CreateNewBackupWithMetadata(
    db::DB* db, const std::string& app_metadata, bool flush_before_backup,
    std::function<void()> progress_callback) {
  assert(initialized_);
  assert(!read_only_);
  ...
  __XENGINE_LOG(INFO, "add files for backup done, wait finish.");
  Status item_status;
  for (auto& item : backup_items_to_finish) {
    ...
    if (item_status.ok()) {
      item_status = new_backup.get()->AddFile(std::make_shared<FileInfo>(
          item.dst_relative, result.size, result.checksum_value));
    }
    if (!item_status.ok()) {
      s = item_status;
    }
...
//    unique_ptr<Directory> backup_private_directory;
    Directory *backup_private_directory = nullptr;
    ...
    MOD_DELETE_OBJECT(Directory, backup_private_directory);
  }
  ...
  if (!s.ok()) {
    ...
    __XENGINE_LOG(INFO, "Backup failed -- %s",
                   s.ToString().c_str());
    __XENGINE_LOG(INFO, "Backup Statistics %s\n",
  ...
  __XENGINE_LOG(INFO, "Backup DONE. All is good");

  // backup_speed is in byte/second
  double backup_speed = new_backup->GetSize() / (1.048576 * backup_time);
  __XENGINE_LOG(INFO, "Backup number of files: %u",
                 new_backup->GetNumberFiles());
  char human_size[16];
  AppendHumanBytes(new_backup->GetSize(), human_size, sizeof(human_size));
  __XENGINE_LOG(INFO, "Backup size: %s", human_size);
  __XENGINE_LOG(INFO, "Backup time: %" PRIu64 " microseconds",
                 backup_time);
  __XENGINE_LOG(INFO, "Backup speed: %.3f MB/s", backup_speed);
  __XENGINE_LOG(INFO, "Backup Statistics %s",
                 backup_statistics_.ToString().c_str());
  return s;
}
...
Status BackupEngineImpl::PurgeOldBackups(uint32_t num_backups_to_keep) {
  assert(initialized_);
  assert(!read_only_);
  __XENGINE_LOG(INFO, "Purging old backups, keeping %u",
                 num_backups_to_keep);
...
Status BackupEngineImpl::DeleteBackup(BackupID backup_id) {
  assert(initialized_);
  assert(!read_only_);
  __XENGINE_LOG(INFO, "Deleting backup %u", backup_id);
  ...
  for (auto& itr : backuped_file_infos_) {
    if (itr.second->refs == 0) {
      ...
      __XENGINE_LOG(INFO, "Deleting %s -- %s", itr.first.c_str(),
                     s.ToString().c_str());
      to_delete.push_back(itr.first);
    }
  }
  ...
  __XENGINE_LOG(INFO, "Deleting private dir %s -- %s",
...
Status BackupEngineImpl::RestoreDBFromBackup(
  ...
  __XENGINE_LOG(INFO, "Restoring backup id %u\n", backup_id);
  __XENGINE_LOG(INFO, "keep_log_files: %d\n",
  ...
  for (const auto& file_info : backup->GetFiles()) {
    ...
    __XENGINE_LOG(INFO, "Restoring %s to %s\n", file.c_str(),
                   dst.c_str());
  ...
  __XENGINE_LOG(INFO, "Restoring done -- %s\n",
                 s.ToString().c_str());
  return s;
}
...
Status BackupEngineImpl::VerifyBackup(BackupID backup_id) {
  ...
  __XENGINE_LOG(INFO, "Verifying backup id %u\n", backup_id);
...
//  unique_ptr<SequentialFile> src_file;
  WritableFile *dst_file = nullptr;
  SequentialFile *src_file = nullptr;
...
//      new WritableFileWriter(std::move(dst_file), env_options));
  WritableFileWriter *dest_writer =
      MOD_NEW_OBJECT(memory::ModId::kDefaultMod, WritableFileWriter, dst_file, env_options);
//  unique_ptr<SequentialFileReader> src_reader;
  SequentialFileReader *src_reader = nullptr;
  unique_ptr<char[]> buf; // todo
  if (!src.empty()) {
//    src_reader.reset(new SequentialFileReader(src_file));
    src_reader = MOD_NEW_OBJECT(memory::ModId::kDefaultMod, SequentialFileReader, src_file);
...
Status BackupEngineImpl::AddBackupFileWorkItem(
  ...
  const bool same_path = live_dst_paths.find(dst_path) != live_dst_paths.end();
  ...
  if (!contents.empty() || need_to_copy) {
    __XENGINE_LOG(INFO, "Copying %s to %s", fname.c_str(),
                   dst_path_tmp.c_str());
    CopyOrCreateWorkItem copy_or_create_work_item(
...
Status BackupEngineImpl::CalculateChecksum(const std::string& src, Env* src_env,
  ...
  *checksum_value = 0;
  ...
  std::unique_ptr<SequentialFile, memory::ptr_destruct_delete<SequentialFile>> src_file_ptr;
  SequentialFile *src_file = nullptr;
  Status s = src_env->NewSequentialFile(src, src_file, env_options);
  src_file_ptr.reset(src_file);
  if (!s.ok()) {
    return s;
  }

  unique_ptr<SequentialFileReader> src_reader(new SequentialFileReader(src_file));
...
Status BackupEngineImpl::GarbageCollect() {
  assert(!read_only_);
  __XENGINE_LOG(INFO, "Starting garbage collection");
  ...
  if (options_.share_table_files) {
    ...
    for (auto& child : shared_children) {
      ...
      if (child_itr == backuped_file_infos_.end() ||
        ...
        __XENGINE_LOG(INFO, "Deleting %s -- %s",
  ...
  for (auto& child : private_children) {
    ...
    __XENGINE_LOG(INFO, "Deleting dir %s -- %s",
...
//  unique_ptr<SequentialFile> backup_meta_file;
  SequentialFile *backup_meta_file = nullptr;
  ...
  unique_ptr<SequentialFileReader> backup_meta_reader(
      new SequentialFileReader(backup_meta_file));
  ...
  for (uint32_t i = 0; s.ok() && i < num_files; ++i) {
    ...
    if (line.starts_with(checksum_prefix)) {
      ...
      if (line != util::ToString(checksum_value)) {
...
Status BackupEngineImpl::BackupMeta::StoreToFile(bool sync) {
  Status s;
  unique_ptr<WritableFile> backup_meta_file_ptr;
  WritableFile *backup_meta_file = nullptr;
  EnvOptions env_options;
  env_options.use_mmap_writes = false;
  env_options.use_direct_writes = false;
  s = env_->NewWritableFile(meta_filename_ + ".tmp", backup_meta_file,
                            env_options);
  backup_meta_file_ptr.reset(backup_meta_file);
  if (!s.ok()) {
    return s;
  }
