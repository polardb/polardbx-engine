...
void HistogramStat::to_detail_string(std::string &str) const {
  char buf[1650];
  uint64_t cur_num = num();
  const double mult = 100.0 / cur_num;
  uint64_t cumulative_sum = 0;
...
HistogramStatNonThread::HistogramStatNonThread() :
    num_buckets_(bucketMapper.BucketCount()) {
  assert(num_buckets_ == sizeof(buckets_) / sizeof(*buckets_));
  Clear();
}

void HistogramStatNonThread::Clear() {
...
void HistogramStatNonThread::Add(uint64_t value) {
  const size_t index = bucketMapper.IndexForValue(value);
  assert(index < num_buckets_);
...
void HistogramStatNonThread::add_zero(int32_t count) {
  num_ += count;
  min_ = 0;
  buckets_[0] += count;
}

void HistogramStatNonThread::Merge(const HistogramStatNonThread& other) {
  // This function needs to be performned with the outer lock acquired
  // However, atomic operation on every member is still need, since Add()
  // requires no lock and value update can still happen concurrently
  uint64_t other_min = other.min();
  while (other_min < min_) {
    min_ = other_min;
  }

  uint64_t other_max = other.max();
  while (other_max > max_) {
    max_ = other_max;
  }

  num_ += other.num();
  sum_ += other.sum();
  sum_squares_ += other.sum_squares();
  for (unsigned int b = 0; b < num_buckets_; b++) {
    buckets_[b] += other.bucket_at(b);
  }
}

double HistogramStatNonThread::Median() const { return Percentile(50.0); }

double HistogramStatNonThread::Percentile(double p) const {
  double threshold = num() * (p / 100.0);
  uint64_t cumulative_sum = 0;
  for (unsigned int b = 0; b < num_buckets_; b++) {
    uint64_t bucket_value = bucket_at(b);
...
double HistogramStatNonThread::Average() const {
  uint64_t cur_num = num();
  uint64_t cur_sum = sum();
  if (cur_num == 0) return 0;
  return static_cast<double>(cur_sum) / static_cast<double>(cur_num);
...
double HistogramStatNonThread::StandardDeviation() const {
  uint64_t cur_num = num();
  uint64_t cur_sum = sum();
  uint64_t cur_sum_squares = sum_squares();
  if (cur_num == 0) return 0;
...
std::string HistogramStatNonThread::ToString() const {
  uint64_t cur_num = num();
  std::string r;
  char buf[1650];
  snprintf(buf, sizeof(buf), "Count: %" PRIu64 " Average: %.4f  StdDev: %.2f\n",
...
void HistogramStatNonThread::to_detail_string(std::string &str) const {
  char buf[1650];
  uint64_t cur_num = num();
  const double mult = 100.0 / cur_num;
  uint64_t cumulative_sum = 0;
...
void HistogramStatNonThread::Data(HistogramData* const data) const {
  assert(data);
  data->median = Median();
  data->percentile95 = Percentile(95);
  data->percentile99 = Percentile(99);
...
void HistogramImpl::add_zero(int32_t count) {
  stats_.add_zero(count);
}
...
void HistogramImpl::to_detail_string(std::string &str) const {
  return stats_.to_detail_string(str);
}
