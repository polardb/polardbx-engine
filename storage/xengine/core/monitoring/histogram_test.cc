...
void PopulateHistogram(HistogramImpl& histogram, uint64_t low, uint64_t high,
                       uint64_t loop = 1) {
  for (; loop > 0; loop--) {
    for (uint64_t i = low; i <= high; i++) {
      histogram.Add(i);
...
void BasicOperation(HistogramImpl& histogram) {
  PopulateHistogram(histogram, 1, 100, 10);
...
void MergeHistogram(HistogramImpl& histogram, HistogramImpl& other) {
  PopulateHistogram(histogram, 1, 100);
  PopulateHistogram(other, 101, 200);
  histogram.Merge(other);
...
void EmptyHistogram(HistogramImpl& histogram) {
  ASSERT_EQ(histogram.min(), bucketMapper.LastValue());
  ASSERT_EQ(histogram.max(), 0);
  ASSERT_EQ(histogram.num(), 0);
  ASSERT_EQ(histogram.Median(), 0.0);
...
void ClearHistogram(HistogramImpl& histogram) {
  for (uint64_t i = 1; i <= 100; i++) {
    histogram.Add(i);
  }
  histogram.Clear();
