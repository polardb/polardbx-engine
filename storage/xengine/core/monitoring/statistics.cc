...
const int kTmpSmallStrBufferSize = 50;
  ...
  for (const auto& h : HistogramsNameMap) {
    if (h.first < HISTOGRAM_ENUM_MAX || enable_internal_stats_) {
      ...
      snprintf(buffer, kTmpStrBufferSize,
               "%s statistics Percentiles :=> avg : %f  50 : %f 95 : %f 99 : "
               "%f 100 : %f\n",
               h.second.c_str(), hData.average, hData.median,
               hData.percentile95, hData.percentile99, hData.max);
      res.append(buffer);
    }
  }
  res.shrink_to_fit();
