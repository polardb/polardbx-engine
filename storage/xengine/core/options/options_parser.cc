...
Status PersistRocksDBOptions(const DBOptions& db_opt,
  ...
  std::unique_ptr<WritableFile, memory::ptr_destruct_delete<WritableFile>> writable_ptr;
  WritableFile *writable = nullptr;
  Status s = env->NewWritableFile(file_name, writable, EnvOptions());
  writable_ptr.reset(writable);
  if (!s.ok()) {
    return s;
  }
...
Status RocksDBOptionsParser::Parse(const std::string& file_name, Env* env) {
  ...
  std::unique_ptr<SequentialFile, memory::ptr_destruct_delete<SequentialFile>> seq_file_ptr;
  SequentialFile *seq_file = nullptr;
  Status s = env->NewSequentialFile(file_name, seq_file, EnvOptions());
  seq_file_ptr.reset(seq_file);
  if (!s.ok()) {
    return s;
  }
  ...
  for (int line_num = 1;
       ReadOneLine(&iss, seq_file, &line, &has_data, &s); ++line_num) {
    if (!s.ok()) {
      return s;
    }
...
Status RocksDBOptionsParser::CheckSection(const OptionSection section,
  ...
  if (section == kOptionSectionDBOptions) {
    ...
    if (cf_opts_.size() == 0 && !is_default_cf) {
      return InvalidArgument(
          line_num,
          "Default sub table must be the first CFOptions section "
    ...
    if (GetCFOptions(section_arg) == nullptr) {
      return InvalidArgument(
          line_num, std::string("Does not find a matched sub table name in "
                                "TableOptions section.  sub table Name:") +
                        section_arg);
    }
...
Status RocksDBOptionsParser::EndSection(
  ...
  if (section == kOptionSectionDBOptions) {
    ...
    if (cf_opt == nullptr) {
      return Status::InvalidArgument(
          "The specified sub table must be defined before the "
          "TableOptions section:",
          section_arg);
    }
...
Status RocksDBOptionsParser::ValidityCheck() {
  if (!has_db_options_) {
    return Status::Corruption(
        "An Option file must have a single DBOptions section");
  }
  if (!has_default_cf_options_) {
    return Status::Corruption(
        "An Option file must have a single CFOptions:default section");
  }
  ...
  if (cf_names.size() != parser.cf_names()->size()) {
    if (sanity_check_level >= kSanityLevelLooselyCompatible) {
      return Status::InvalidArgument(
          ...
          "the same number of sub table names as the db instance.");
    } else if (cf_opts.size() > parser.cf_opts()->size()) {
      return Status::InvalidArgument(
          "[RocksDBOptionsParser Error]",
          "The persisted options file has less number of sub table "
  ...
  for (size_t i = 0; i < cf_names.size(); ++i) {
    if (cf_names[i] != parser.cf_names()->at(i)) {
      return Status::InvalidArgument(
          ...
          "instance does not have the same name for sub table ",
          ToString(i));
    }
  }
