...
#include "xengine/utilities/leveldb_options.h"
...
TEST_F(OptionsTest, GetOptionsFromMapTest) {
  ...
  ASSERT_EQ(new_cf_opt.level0_layer_num_compaction_trigger, 16);
  ASSERT_EQ(new_cf_opt.level1_extents_major_compaction_trigger, 1000);
  ASSERT_EQ(new_cf_opt.level2_usage_percent, 70);
...
TEST_F(OptionsTest, GetMemTableRepFactoryFromString) {
  std::unique_ptr<memtable::MemTableRepFactory> new_mem_factory = nullptr;
...
class OptionsParserTest : public testing::Test {
 public:
  OptionsParserTest() { env_.reset(new test::StringEnv(util::Env::Default())); }
...
TEST_F(OptionsParserTest, Comment) {
  ...
  std::string options_file_content =
      ...
      "[CFOptions   \"default\"]  # sub table must be specified\n"
...
TEST_F(OptionsSanityCheckTest, SanityCheck) {
  ...
  // prefix_extractor
  {
    ...
    // default prefix_extractor has been set for bloom filter
