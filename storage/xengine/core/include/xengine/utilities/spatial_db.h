...
class FeatureSet {
 ...
 public:
  ...
  bool Deserialize(const common::Slice& input);
...
class Cursor {
 public:
  ...
  // REQUIRES: Valid()
  virtual const common::Slice blob() = 0;
  // Lifetime of the underlying storage until the next call to Next()
  // REQUIRES: Valid()
...
class SpatialDB : public util::StackableDB {
 public:
  ...
  static common::Status Open(const SpatialDBOptions& options,
                             const std::string& name, SpatialDB** db,
                             bool read_only = false);

  explicit SpatialDB(db::DB* db) : StackableDB(db) {}
  ...
  virtual common::Status Insert(
      ...
      const BoundingBox<double>& bbox, const common::Slice& blob,
  ...
  virtual common::Status Compact(int num_threads = 1) = 0;
