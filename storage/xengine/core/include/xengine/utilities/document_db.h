...
#include "xengine/utilities/json_document.h"
...
class DocumentDB : public StackableDB {
 public:
  ...
  static common::Status Open(const DocumentDBOptions& options,
  ...
  // are created
  virtual common::Status CreateIndex(const common::WriteOptions& write_options,
  ...
  virtual common::Status DropIndex(const std::string& name) = 0;
  ...
  virtual common::Status Insert(const common::WriteOptions& options,
                                const JSONDocument& document) = 0;

  // Deletes all documents matching a filter atomically
  virtual common::Status Remove(const common::ReadOptions& read_options,
  ...
  virtual common::Status Update(const common::ReadOptions& read_options,
                                const common::WriteOptions& write_options,
  ...
  virtual Cursor* Query(const common::ReadOptions& read_options,
                        const JSONDocument& query) = 0;
};
