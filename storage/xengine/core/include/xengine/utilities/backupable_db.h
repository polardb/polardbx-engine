...
struct BackupableDBOptions {
  ...
  // Default: nullptr
  std::shared_ptr<util::RateLimiter> backup_rate_limiter{nullptr};
  ...
  // Default: nullptr
  std::shared_ptr<util::RateLimiter> restore_rate_limiter{nullptr};
  ...
  explicit BackupableDBOptions(
      const std::string& _backup_dir, util::Env* _backup_env = nullptr,
...
class BackupEngineReadOnly {
 public:
  ...
  static common::Status Open(util::Env* db_env,
  ...
  virtual common::Status RestoreDBFromBackup(
      BackupID backup_id, const std::string& db_dir, const std::string& wal_dir,
      const RestoreOptions& restore_options = RestoreOptions()) = 0;

  // See the corresponding doc in BackupEngine
  virtual common::Status RestoreDBFromLatestBackup(
  ...
  //
  // Returns common::Status::OK() if all checks are good
  virtual common::Status VerifyBackup(BackupID backup_id) = 0;
};
...
class BackupEngine {
 public:
  ...
  virtual common::Status CreateNewBackupWithMetadata(
      db::DB* db, const std::string& app_metadata,
      bool flush_before_backup = false,
      std::function<void()> progress_callback = []() {}) = 0;

  // Captures the state of the database in the latest backup
  // NOT a thread safe call
  virtual common::Status CreateNewBackup(
      db::DB* db, bool flush_before_backup = false,
  ...
  virtual common::Status PurgeOldBackups(uint32_t num_backups_to_keep) = 0;

  // deletes a specific backup
  virtual common::Status DeleteBackup(BackupID backup_id) = 0;

  // Call this from another thread if you want to stop the backup
  // that is currently happening. It will return immediatelly, will
  // not wait for the backup to stop.
  // The backup will stop ASAP and the call to CreateNewBackup will
  // return common::Status::Incomplete(). It will not clean up after itself, but
  ...
  virtual common::Status GarbageCollect() = 0;
};
