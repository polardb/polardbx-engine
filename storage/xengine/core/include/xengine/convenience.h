...
Status GetBlockBasedTableOptionsFromMap(
    ...
    const std::unordered_map<std::string, std::string>& opts_map,
    table::BlockBasedTableOptions* new_table_options,
...
Status GetPlainTableOptionsFromMap(
    const table::PlainTableOptions& table_options,
    const std::unordered_map<std::string, std::string>& opts_map,
...
void CancelAllBackgroundWork(db::DB* db, bool wait = false);

// Delete files which are entirely in the given range
// Could leave some keys in the range which are in files which are not
// entirely in the range.
// Snapshots before the delete might not see the data in the given range.
Status DeleteFilesInRange(db::DB* db, db::ColumnFamilyHandle* column_family,
