...
struct TableFileCreationInfo : public TableFileCreationBriefInfo {
  ...
  explicit TableFileCreationInfo(table::TableProperties&& prop)
...
struct FlushJobInfo {
  ...
  common::SequenceNumber smallest_seqno;
  // The largest sequence number in the newly created file
  common::SequenceNumber largest_seqno;
...
struct CompactionJobInfo {
  ...
  explicit CompactionJobInfo(const storage::CompactionJobStats& _stats)
      : stats(_stats) {}
  ...
  storage::CompactionJobStats stats;
};
...
// the current thread holding any db::DB mutex. This is to prevent potential
...
class EventListener {
 public:
  ...
  virtual void OnFlushCompleted(db::DB* /*db*/,
  ...
  // it should not run for an extended period of time before the function
  // returns.  Otherwise, RocksDB may be blocked.
  virtual void OnFlushBegin(db::DB* /*db*/,
                            const FlushJobInfo& /*flush_job_info*/) {}
  ...
  virtual void OnCompactionCompleted(db::DB* /*db*/,
