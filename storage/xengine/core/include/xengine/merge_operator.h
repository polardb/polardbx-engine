...
class MergeOperator {
 public:
  ...
  virtual bool FullMerge(const common::Slice& key,
  ...
  struct MergeOperationInput {
    explicit MergeOperationInput(
        const common::Slice& _key, const common::Slice* _existing_value,
        const std::vector<common::Slice>& _operand_list)
        : key(_key),
  ...
  // always return false.
  virtual bool PartialMerge(const common::Slice& key,
                            const common::Slice& left_operand,
                            const common::Slice& right_operand,
                            std::string* new_value) const {
    return false;
  }
