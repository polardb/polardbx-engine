...
class FilterBitsBuilder {
 public:
  ...
  virtual void AddKey(const common::Slice& key) = 0;

  // Generate the filter using the keys that are added
  // The return value of this function would be the filter bits,
  // The ownership of actual data is set to buf
  virtual common::Slice Finish(std::unique_ptr<char[],
                               memory::ptr_delete<char>>* buf) = 0;
};

// A class that checks if a key can be in filter
// It should be initialized by common::Slice generated by BitsBuilder
class FilterBitsReader {
 public:
  virtual ~FilterBitsReader() {}

  // Check if the entry match the bits in filter
  virtual bool MayMatch(const common::Slice& entry) = 0;
};
...
class FilterPolicy {
 public:
  ...
  virtual void CreateFilter(const common::Slice* keys, int n,
  ...
  virtual bool KeyMayMatch(const common::Slice& key,
                           const common::Slice& filter) const = 0;
  ...
  virtual FilterBitsReader* GetFilterBitsReader(
      const common::Slice& contents) const {
    return nullptr;
  }
};
