...
struct EnvOptions {
  ...
  // write_buffered_number buffer number
  size_t concurrent_writable_file_buffer_num = 4;

  // concurrent_direct_file_writer
  size_t concurrent_writable_file_single_buffer_size = 4 * 1024U * 1024U;

  size_t concurrent_writable_file_buffer_switch_limit = 512 * 1024U;
...
class Env {
 public:
  ...
  //
  // The returned file will only be accessed by one thread at a time.
  virtual common::Status ReopenWritableFile(const std::string& fname,
                                            WritableFile *&result,
  ...
  virtual common::Status FileExists(const std::string& fname) = 0;
  ...
  //                  permission to access "dir", or if "dir" is invalid.
  //         IOError if an IO Error was encountered
  virtual common::Status GetChildrenFileAttributes(
      const std::string& dir, std::vector<FileAttributes>* result);

  // Delete the named file.
  virtual common::Status DeleteFile(const std::string& fname) = 0;

  // Create the specified directory. Returns error if directory exists.
  virtual common::Status CreateDir(const std::string& dirname) = 0;

  // Creates directory if missing. Return Ok if it exists, or successful in
  // Creating.
  virtual common::Status CreateDirIfMissing(const std::string& dirname) = 0;

  // Delete the specified directory.
  virtual common::Status DeleteDir(const std::string& dirname) = 0;
  ...
  virtual common::Status GetHostName(char* name, uint64_t len) = 0;

  // Get the number of seconds since the Epoch, 1970-01-01 00:00:00 (UTC).
  virtual common::Status GetCurrentTime(int64_t* unix_time) = 0;

  // Get full directory name for this db.
  virtual common::Status GetAbsolutePath(const std::string& db_path,
  ...
  virtual EnvOptions OptimizeForLogWrite(
      const EnvOptions& env_options, const common::DBOptions& db_options) const;
...
class SequentialFile {
 public:
  ...
  //
  // REQUIRES: External synchronization
  virtual common::Status Skip(uint64_t n) = 0;
  ...
  virtual common::Status InvalidateCache(size_t offset, size_t length) {
    return common::Status::NotSupported("InvalidateCache not supported.");
  }

  // Positioned Read for direct I/O
  // If Direct I/O enabled, offset, n, and scratch should be properly aligned
  virtual common::Status PositionedRead(uint64_t offset, size_t n,
                                        common::Slice* result, char* scratch) {
    return common::Status::NotSupported();
  }
  ...
  virtual int get_file_size() const { return 0; }
...
class RandomAccessFile {
 public:
  ...
  virtual common::Status Prefetch(uint64_t offset, size_t n) {
    return common::Status::OK();
  }

  // Used by the file_reader_writer to decide if the ReadAhead wrapper
  // should simply forward the call and do not enact buffering or locking.
  virtual bool ShouldForwardRawRequest() const { return false; }
  ...
  virtual int fill_aio_info(const int64_t offset, const int64_t size, AIOInfo &aio_info) const
  {
    return common::Status::kNotSupported;
  }
};
...
class WritableFile {
 public:
  ...
  virtual common::Status Append(const common::Slice& data) = 0;
  ...
  virtual common::Status Truncate(uint64_t size) {
    return common::Status::OK();
  }
  virtual common::Status Close() = 0;
  virtual common::Status Flush() = 0;
  virtual common::Status Sync() = 0;  // sync data
   ...
   * metadata as well.
   */
  virtual common::Status Fsync() { return Sync(); }
  ...
  virtual common::Status RangeSync(uint64_t offset, uint64_t nbytes) {
    return common::Status::OK();
  }
 ...
 protected:
   ...
   */
  virtual common::Status Allocate(uint64_t offset, uint64_t len) {
    return common::Status::OK();
  }
...
class RandomRWFile {
 public:
  ...
  // Write bytes in `data` at  offset `offset`, Returns common::Status::OK() on
  // success.
  // Pass aligned buffer when use_direct_io() returns true.
  virtual common::Status Write(uint64_t offset, const common::Slice& data) = 0;
  ...
  virtual common::Status Read(uint64_t offset, size_t n, common::Slice* result,
                              char* scratch) const = 0;
  virtual int fallocate(int mode, int64_t offset, int64_t length) { return common::Status::kOk; }
  virtual int ftruncate(int64_t length) { return common::Status::kOk; }
  virtual common::Status Flush() = 0;
...
extern common::Status ReadFileToString(Env* env, const std::string& fname,
                                       std::string* data);
...
class EnvWrapper : public Env {
 public:
  ...
  common::Status ReopenWritableFile(const std::string& fname,
                                    WritableFile *&result,
                                    const EnvOptions& options) override {
  ...
  common::Status GetChildrenFileAttributes(
  ...
  common::Status GetHostName(char* name, uint64_t len) override {
    return target_->GetHostName(name, len);
  }
  common::Status GetCurrentTime(int64_t* unix_time) override {
...
class WritableFileWrapper : public WritableFile {
 public:
  ...
  common::Status Close() override { return target_->Close(); }
  common::Status Flush() override { return target_->Flush(); }
  common::Status Sync() override { return target_->Sync(); }
  common::Status Fsync() override { return target_->Fsync(); }
 ...
 protected:
  common::Status Allocate(uint64_t offset, uint64_t len) override {
    return target_->Allocate(offset, len);
  }
  common::Status RangeSync(uint64_t offset, uint64_t nbytes) override {
