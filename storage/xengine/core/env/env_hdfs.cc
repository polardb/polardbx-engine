...
//  HdfsReadableFile* f = new HdfsReadableFile(fileSys_, fname);
  HdfsReadableFile *f = MOD_NEW_OBJECT(memory::ModId::kDefaultMod, HdfsReadableFile, fileSys_, fname);
  if (f == nullptr || !f->isValid()) {
//    delete f;
    MOD_DELETE_OBJECT(HdfsReadableFile, f);
    *result = nullptr;
...
//  HdfsWritableFile* f = new HdfsWritableFile(fileSys_, fname);
  HdfsWritableFile *f = MOD_NEW_OBJECT(memory::ModId::kDefaultMod, HdfsWritableFile, fileSys_, fname);
  if (f == nullptr || !f->isValid()) {
//    delete f;
    MOD_DELETE_OBJECT(HdfsWritableFile, f);
    *result = nullptr;
    return IOError(fname, errno);
  }
...
//      result->reset(new HdfsDirectory(0));
      result = MOD_NEW_OBJECT(memory::ModId::kDefaultMod, HdfsDirectory, 0);
      return Status::OK();
