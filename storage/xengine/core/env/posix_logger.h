...
class PosixLogger : public Logger {
 ...
 public:
  ...
  virtual void print_log_buffer(const char *buf, const int64_t buf_len) override
  {
    //reuse the rocksdb's control log flush logic
    if (nullptr != buf && buf_len > 0)  { 
      struct timeval now_tv;
      gettimeofday(&now_tv, nullptr);
      uint64_t now_micros = static_cast<uint64_t>(now_tv.tv_sec) * 1000000 + now_tv.tv_usec;
      size_t size = fwrite(buf, 1, buf_len, file_);
      flush_pending_ = true;
      if (size > 0 && size == static_cast<size_t>(buf_len))  {
        log_size_ += size;
      }
      if (now_micros - last_flush_micros_ >= flush_every_seconds_ * 1000000) {
        Flush();
