...
class RangeDelAggregator {
 public:
  ...
  RangeDelAggregator(const InternalKeyComparator& icmp,
                     common::SequenceNumber upper_bound,
  ...
  bool ShouldDelete(const common::Slice& internal_key,
                    RangePositioningMode mode = kFullScan);
  ...
  common::Status AddTombstones(table::InternalIterator *input);
  ...
  //    compaction.
  void AddToBuilder(table::TableBuilder* builder,
                    const common::Slice* lower_bound,
                    const common::Slice* upper_bound, FileMetaData* meta,
                    bool bottommost_level = false);
  bool IsEmpty();

 private:
  // Maps tombstone user start key -> tombstone object
  typedef std::multimap<common::Slice, RangeTombstone,
                        util::stl_wrappers::LessOfComparator>
      TombstoneMap;
  ...
  typedef std::map<common::SequenceNumber, PositionalTombstoneMap> StripeMap;
  ...
  void InitRep(const std::vector<common::SequenceNumber>& snapshots);

  PositionalTombstoneMap& GetPositionalTombstoneMap(common::SequenceNumber seq);
  common::Status AddTombstone(RangeTombstone tombstone);

  common::SequenceNumber upper_bound_;
