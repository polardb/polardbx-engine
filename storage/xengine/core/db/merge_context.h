...
const std::vector<common::Slice> empty_operand_list;
...
class MergeContext {
 public:
  ...
  void PushOperand(const common::Slice& operand_slice,
  ...
  void PushOperandBack(const common::Slice& operand_slice,
                       bool operand_pinned = false) {
    Initialize();
    SetDirectionForward();
  ...
  common::Slice GetOperand(int index) {
  ...
  const std::vector<common::Slice>& GetOperands() {
    if (!operand_list_) {
 ...
 private:
  void Initialize() {
    if (!operand_list_) {
      operand_list_.reset(new std::vector<common::Slice>());
  ...
  // List of operands
  std::unique_ptr<std::vector<common::Slice>> operand_list_;
