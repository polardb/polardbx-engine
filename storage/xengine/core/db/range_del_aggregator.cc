...
bool RangeDelAggregator::ShouldDelete(
  ...
  switch (mode) {
    ...
    case kBinarySearch:
      assert(collapse_deletions_);
      tombstone_map_iter = tombstone_map.upper_bound(parsed.user_key);
      if (tombstone_map_iter == tombstone_map.begin()) {
        // before start of deletion intervals
        return false;
      }
...
Status RangeDelAggregator::AddTombstones(InternalIterator *input) {
  if (input == nullptr) {
    return Status::OK();
  }
  input->SeekToFirst();
  ...
  if (!first_iter) {
    // no use now
//    rep_->pinned_iters_mgr_.PinIterator(input, false /* arena */);
  }
  return Status::OK();
}
  ...
  while (stripe_map_iter != rep_->stripe_map_.end()) {
    for (auto tombstone_map_iter = stripe_map_iter->second.raw_map.begin();
      ...
      if (lower_bound != nullptr &&
          icmp_.user_comparator()->Compare(tombstone.end_key_, *lower_bound) <=
              0) {
