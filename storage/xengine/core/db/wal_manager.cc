...
Status WalManager::GetSortedWalFiles(VectorLogPtr& files) {
  ...
  if (!files.empty()) {
    ...
    __XENGINE_LOG(INFO, "Latest Archived log: %" PRIu64,
  ...
  for (auto& log : logs) {
    if (log->LogNumber() > latest_archived_log_number) {
      ...
      __XENGINE_LOG(WARN, "%s already moved to archive",
                     log->PathName().c_str());
    }
  }
...
Status WalManager::GetUpdatesSince(
  ...
  std::unique_ptr<VectorLogPtr, memory::ptr_destruct_delete<VectorLogPtr>> wal_files(MOD_NEW_OBJECT(memory::ModId::kDefaultMod, VectorLogPtr));
...
void WalManager::PurgeObsoleteWALFiles() {
  ...
  if (!s.ok()) {
    __XENGINE_LOG(ERROR, "Can't get current time: %s",
                    s.ToString().c_str());
    assert(false);
    return;
  }
  ...
  if (!s.ok()) {
    __XENGINE_LOG(ERROR, "Can't get archive files: %s", s.ToString().c_str());
    assert(false);
    return;
  }
  ...
  for (auto& f : files) {
    ...
    if (ParseFileName(f, &number, &type) && type == kLogFile) {
      ...
      if (ttl_enabled) {
        ...
        if (!s.ok()) {
          __XENGINE_LOG(WARN, "Can't get file mod time: %s: %s", file_path.c_str(), s.ToString().c_str());
          continue;
        }
        if (now_seconds - file_m_time > db_options_.wal_ttl_seconds) {
          s = env_->DeleteFile(file_path);
          if (!s.ok()) {
            __XENGINE_LOG(WARN, "Can't delete file: %s: %s", file_path.c_str(), s.ToString().c_str());
            continue;
          } else {
            XENGINE_LOG(INFO, "success to delete wal file", K(file_path), K(number));
      ...
      if (size_limit_enabled) {
        ...
        if (!s.ok()) {
          __XENGINE_LOG(ERROR, "Unable to get file size: %s: %s", file_path.c_str(), s.ToString().c_str());
          return;
        } else {
          if (file_size > 0) {
            ...
            s = env_->DeleteFile(file_path);
            if (!s.ok()) {
              __XENGINE_LOG(WARN, "Unable to delete file: %s: %s", file_path.c_str(), s.ToString().c_str());
              continue;
            } else {
              XENGINE_LOG(INFO, "success to delete wal file", K(file_path));
              MutexLock l(&read_first_record_cache_mutex_);
              read_first_record_cache_.erase(number);
            }
          }
  ...
  if (files_del_num > archived_logs.size()) {
    __XENGINE_LOG(WARN, "Trying to delete more archived log files than exist. Deleting all");
...
Status WalManager::ReadFirstRecord(const WalFileType type,
  ...
  *sequence = 0;
  if (type != kAliveLogFile && type != kArchivedLogFile) {
    __XENGINE_LOG(ERROR, "[WalManger] Unknown file type %s", ToString(type).c_str());
...
Status WalManager::ReadFirstLine(const std::string& fname,
  ...
  struct LogReporter : public log::Reader::Reporter {
    ...
    virtual void Corruption(size_t bytes, const Status& s) override {
      __XENGINE_LOG(WARN, "[WalManager] %s%s: dropping %d bytes; %s",
  ...
  SequentialFileReader *file_reader = MOD_NEW_OBJECT(memory::ModId::kDefaultMod, SequentialFileReader, file);
  if (!status.ok()) {
    return status;
  }
