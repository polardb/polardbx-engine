...
Status TransactionLogIteratorImpl::OpenLogFile(
    const LogFile* logFile, SequentialFileReader *&file_reader) {
...
//  unique_ptr<SequentialFile> file;
  ...
  if (s.ok()) {
    file_reader = MOD_NEW_OBJECT(memory::ModId::kDefaultMod, SequentialFileReader, file);
  }
  return s;
}
...
Status TransactionLogIteratorImpl::status() { return currentStatus_; }

bool TransactionLogIteratorImpl::Valid() { return started_ && isValid_; }
...
void TransactionLogIteratorImpl::SeekToStartSequence(uint64_t startFileIndex,
                                                     bool strict) {
  std::string scratch;
  Slice record;
  started_ = false;
...
void TransactionLogIteratorImpl::Next() { return NextImpl(false); }
...
bool TransactionLogIteratorImpl::IsBatchExpected(
    const WriteBatch* batch, const SequenceNumber expectedSeq) {
  assert(batch);
...
void TransactionLogIteratorImpl::UpdateCurrentWriteBatch(const Slice& record) {
  std::unique_ptr<WriteBatch, memory::ptr_destruct_delete<WriteBatch>> batch(MOD_NEW_OBJECT(memory::ModId::kTransaction, WriteBatch));
...
Status TransactionLogIteratorImpl::OpenLogReader(const LogFile* logFile) {
  ...
  assert(file);
  currentLogReader_.reset(MOD_NEW_OBJECT(memory::ModId::kTransaction, log::Reader,
      file, &reporter_, read_options_.verify_checksums_, 0 /* initial_offset*/,
      logFile->LogNumber()));
