...
class FileIndexer {
 public:
  explicit FileIndexer(const util::Comparator* ucmp);
  ...
  void UpdateIndex(util::Arena* arena, const size_t num_levels,
                   std::vector<FileMetaData*>* const files);

  enum {
    // MSVC version 1800 still does not have constexpr for ::max()
    kLevelMaxIndex = port::kMaxInt32
  };
 ...
 private:
  ...
  util::autovector<IndexLevel> next_level_index_;
  int32_t* level_rb_;
};
}
}  // namespace xengine
