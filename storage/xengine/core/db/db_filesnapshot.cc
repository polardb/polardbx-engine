...
Status DBImpl::DisableFileDeletions() {
  ...
  if (disable_delete_obsolete_files_ == 1) {
    __XENGINE_LOG(INFO, "File Deletions Disabled");
  } else {
    __XENGINE_LOG(WARN, "File Deletions Disabled, but already disabled. Counter: %d", disable_delete_obsolete_files_);
  }
  return Status::OK();
}
...
Status DBImpl::EnableFileDeletions(bool force) {
  ...
  bool should_purge_files = false;
  {
    ...
    if (disable_delete_obsolete_files_ == 0) {
      __XENGINE_LOG(INFO, "File Deletions Enabled");
      should_purge_files = true;
      FindObsoleteFiles(&job_context, true);
    } else {
      __XENGINE_LOG(WARN, "File Deletions Enable, but not really enabled. Counter: %d", disable_delete_obsolete_files_);
    }
  }
...
Status DBImpl::GetLiveFiles(std::vector<std::string>& ret,
  ...
  *manifest_file_size = 0;
  ...
  if (flush_memtable) {
    ...
    if (!status.ok()) {
      mutex_.Unlock();
      __XENGINE_LOG(ERROR, "Cannot Flush data %s\n", status.ToString().c_str());
      return status;
    }
    return Status::kOk;
  }
