  ...
  Repairer(const std::string& dbname, const DBOptions& db_options,
      ...
      : dbname_(dbname),
        ...
        table_cache_(new TableCache(default_cf_iopts_, env_options_,
                                    raw_table_cache_.get(), nullptr)),  // TODO
        wb_(db_options_.db_write_buffer_size, db_options_.db_total_write_buffer_size),
 ...
 private:
  ...
  void ConvertLogFilesToTables() {
    for (size_t i = 0; i < logs_.size(); i++) {
      ...
      if (!status.ok()) {
        __XENGINE_LOG(WARN, "Log #%" PRIu64 ": ignoring conversion error: %s", logs_[i], status.ToString().c_str());
      }
      ArchiveFile(logname);
    }
  }
  ...
  Status ConvertLogToTable(uint64_t log) {
    struct LogReporter : public log::Reader::Reporter {
      ...
      virtual void Corruption(size_t bytes, const Status& s) override {
        ...
        __XENGINE_LOG(ERROR, "Log #%" PRIu64 ": dropping %d bytes; %s", lognum, static_cast<int>(bytes), s.ToString().c_str());
      }
    };

    // Open the log file
    std::string logname = LogFileName(dbname_, log);
    unique_ptr<SequentialFile, memory::ptr_destruct_delete<SequentialFile>> lfile_ptr;
    SequentialFile *lfile = nullptr;
    Status status = env_->NewSequentialFile(logname, lfile, env_options_);
    lfile_ptr.reset(lfile);
    if (!status.ok()) {
      return status;
    }
//    unique_ptr<SequentialFileReader> lfile_reader(new SequentialFileReader(lfile));
    SequentialFileReader *lfile_reader = MOD_NEW_OBJECT(memory::ModId::kEnv, SequentialFileReader, lfile);
    ...
    */
    ...
    while (reader.ReadRecord(&record, &scratch)) {
      ...
      if (status.ok()) {
        ...
        __XENGINE_LOG(WARN, "Log #%" PRIu64 ": ignoring %s", log, status.ToString().c_str());
...
//          env_options_, immutable_db_options_);
      ...
      status = BuildTable(
          ...
          *cfd->GetLatestMutableCFOptions(),
          ...
          0);
      __XENGINE_LOG(INFO, "Log #%" PRIu64 ": %d ops saved to Table #%" PRIu64 " %s",
    ...
    delete cf_mems;
*/
    MOD_DELETE_OBJECT(SequentialFileReader, lfile_reader);
    return status;
  }
  ...
  void ExtractMetaData() {
    for (size_t i = 0; i < table_fds_.size(); i++) {
      ...
      if (!status.ok()) {
        ...
        __XENGINE_LOG(WARN, "Table #%s: ignoring %s", file_num_buf, status.ToString().c_str());
        ArchiveFile(fname);
      } else {
        tables_.push_back(t);
      }
    ...
    if (status.ok()) {
      ...
      for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
        ...
        if (!ParseInternalKey(key, &parsed)) {
          __XENGINE_LOG(ERROR, "Table #%" PRIu64 ": unparsable key %s", t->meta.fd.GetNumber(), EscapeString(key).c_str());
          continue;
        }
      ...
      __XENGINE_LOG(INFO, "Table #%" PRIu64 ": %d entries %s", t->meta.fd.GetNumber(), counter, status.ToString().c_str());
    }
*/
    return status;
  }
  ...
  void ArchiveFile(const std::string& fname) {
    ...
    __XENGINE_LOG(INFO, "Archiving %s: %s\n", fname.c_str(), s.ToString().c_str());
  }
};
