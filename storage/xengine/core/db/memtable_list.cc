    ...
    if (batch_count > 0) {
        ...
        for (VersionEdit *edit : edit_list) {
          ...
          for (auto& extent_meta : edit->get_change_info()->extent_meta_) {
            if (mutable_cf_options.compaction_delete_percent > 0 &&
              ...
              XENGINE_LOG(INFO, "Level-0 extent delete triggered ",
      ...
      if (s.ok()) {         // commit new state
        while (batch_count-- > 0) {
          ...
          __XENGINE_LOG(INFO, "[%s] Level-0 commit table #%" PRIu64
          ...
          if (m->delete_triggered_) {
            __XENGINE_LOG(INFO, "[%s] Level-0 delete triggered #%"
        ...
        for (auto it = current_->memlist_.rbegin(); batch_count-- > 0; it++) {
          ...
          __XENGINE_LOG(INFO, "Level-0 commit table #%" PRIu64
                                         ...
                                         util::autovector<MemTable *> *to_delete)
{
  ...
  if (0 >= flushed_memtables.size()) {
    ...
    XENGINE_LOG(WARN, "invalid argument", K(ret), "memtable_count", flushed_memtables.size());
  } else {
    InstallNewVersion();
