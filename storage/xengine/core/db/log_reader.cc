  ...
  if (!eof_ && !read_error_) {
    ...
    if (use_aio_ && !aio_error_) {
      ...
      if (FAILED(check_and_prefetch())) {
        XENGINE_LOG(WARN, "prefetch file failed, will try sync read", K(ret), K(end_of_buffer_offset_));
        aio_error_ = true;
      } else if (FAILED(file_->read(kBlockSize, &buffer_, backing_store_,
                          &aio_handles_[(aio_read_index_++) % MAX_PREFETCH_LOG_BLOCK_NUM]))) {
        aio_error_ = true;
        XENGINE_LOG(WARN, "read file by aio failed, will try sync read", K(ret), K(end_of_buffer_offset_), K(aio_read_index_));
      }
      if (ret != Status::kOk) {
        status = file_->Skip(end_of_buffer_offset_);
        if (!status.ok()) {
          XENGINE_LOG(ERROR, "fail to skip already read part of current log file", K(end_of_buffer_offset_));
        }
      }
    }
    ...
    if (!status.ok()) {
      ...
      *error = kEof;
      ...
      XENGINE_LOG(DEBUG, "reach file end", K(end_of_buffer_offset_), K(eof_offset_), K(file_size_));
    }
    return true;
  } else {
...
int Reader::check_and_prefetch() {
  ...
  for (int i = 0; i < MAX_PREFETCH_LOG_BLOCK_NUM && !aio_error_; ++i) {
    ...
    if (FAILED(file_->prefetch(kBlockSize, &aio_handles_[i]))) {
      XENGINE_LOG(WARN, "prefetch file failed", K(ret), K(i));
      aio_error_ = true;
    }
  }
  return ret;
