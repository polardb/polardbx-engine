...
class IntTblPropCollector {
 public:
  ...
  virtual common::Status Finish(table::UserCollectedProperties* properties) = 0;
  ...
  virtual common::Status InternalAddBlock(const BlockStats& block_stats,
                                          uint64_t file_size) {
    return common::Status::OK();
  }

  virtual table::UserCollectedProperties GetReadableProperties() const = 0;

  virtual bool NeedCompact() const { return false; }

  virtual common::Status internal_add_extent(const storage::ExtentMeta&) {
    return common::Status::OK();
  }

  virtual common::Status internal_del_extent(const storage::ExtentMeta&) {
    return common::Status::OK();
  }
};
...
class InternalKeyPropertiesCollector : public IntTblPropCollector {
 public:
  ...
  virtual common::Status Finish(
      table::UserCollectedProperties* properties) override;
...
class UserKeyTablePropertiesCollector : public IntTblPropCollector {
 public:
  ...
  explicit UserKeyTablePropertiesCollector(
      table::TablePropertiesCollector* collector)
  ...
  virtual bool SupportAddBlock() const override {
    return collector_->SupportAddBlock();
  }
  ...
  virtual common::Status internal_add_extent(
      const storage::ExtentMeta&) override;

  virtual common::Status internal_del_extent(
      const storage::ExtentMeta&) override;

 protected:
  std::unique_ptr<table::TablePropertiesCollector> collector_;
};
...
class UserKeyTablePropertiesCollectorFactory
 ...
 public:
  explicit UserKeyTablePropertiesCollectorFactory(
      std::shared_ptr<table::TablePropertiesCollectorFactory>
          user_collector_factory)
      : user_collector_factory_(user_collector_factory) {}
  virtual IntTblPropCollector* CreateIntTblPropCollector(
      uint32_t column_family_id) override {
    table::TablePropertiesCollectorFactory::Context context;
