...
class Compaction {
 public:
  Compaction(VersionStorageInfo* input_version,
             const common::ImmutableCFOptions& immutable_cf_options,
             const common::MutableCFOptions& mutable_cf_options,
             std::vector<CompactionInputFiles> inputs, int output_level,
             uint64_t target_file_size, uint64_t max_compaction_bytes,
             uint32_t output_path_id, common::CompressionType compression,
             std::vector<FileMetaData*> grandparents,
             bool manual_compaction = false, double score = -1,
             bool deletion_compaction = false,
             common::CompactionReason compaction_reason =
                 common::CompactionReason::kUnknown);
  ...
  common::CompressionType output_compression() const {
  ...
  bool KeyNotExistsBeyondOutputLevel(const common::Slice& user_key,
  ...
  void ReleaseCompactionFiles(common::Status status);
  ...
  // Return the common::ImmutableCFOptions that should be used throughout the
  // compaction
  // procedure
  const common::ImmutableCFOptions* immutable_cf_options() const {
    return &immutable_cf_options_;
  }

  // Return the common::MutableCFOptions that should be used throughout the
  // compaction
  // procedure
  const common::MutableCFOptions* mutable_cf_options() const {
  ...
  std::unique_ptr<storage::CompactionFilter> CreateCompactionFilter() const;
  ...
  common::Slice GetSmallestUserKey() const { return smallest_user_key_; }

  common::Slice GetLargestUserKey() const { return largest_user_key_; }

  common::CompactionReason compaction_reason() { return compaction_reason_; }
 ...
 private:
  ...
  const common::ImmutableCFOptions immutable_cf_options_;
  const common::MutableCFOptions mutable_cf_options_;
  const int number_levels_;
  ColumnFamilyData* cfd_;
  util::Arena arena_;  // util::Arena used to allocate space for file_levels_
  ...
  util::autovector<LevelFilesBrief, 2> input_levels_;
  ...
  common::Slice smallest_user_key_;

  // largest user keys in compaction
  common::Slice largest_user_key_;

  // Reason for compaction
  common::CompactionReason compaction_reason_;
};
