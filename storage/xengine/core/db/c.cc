...
#include "xengine/universal_compaction.h"
...
struct rocksdb_compactionfilter_t : public CompactionFilter {
  ...
  virtual ~rocksdb_compactionfilter_t() { (*destructor_)(state_); }
...
struct rocksdb_comparator_t : public Comparator {
  ...
  virtual ~rocksdb_comparator_t() { (*destructor_)(state_); }
...
struct rocksdb_filterpolicy_t : public FilterPolicy {
  ...
  virtual ~rocksdb_filterpolicy_t() { (*destructor_)(state_); }
...
struct rocksdb_mergeoperator_t : public MergeOperator {
  ...
  virtual ~rocksdb_mergeoperator_t() { (*destructor_)(state_); }
...
struct rocksdb_slicetransform_t : public SliceTransform {
  ...
  virtual ~rocksdb_slicetransform_t() { (*destructor_)(state_); }
...
rocksdb_backup_engine_t* rocksdb_backup_engine_open(
  ...
  if (SaveError(errptr, BackupEngine::Open(
                            options->rep.env,
                            BackupableDBOptions(path, nullptr, true),
                            &be))) {
    return nullptr;
  }
...
void rocksdb_options_set_uint64add_merge_operator(rocksdb_options_t* opt) {
  opt->rep.merge_operator = MergeOperators::CreateUInt64AddOperator();
}
...
rocksdb_writebatch_t* rocksdb_writebatch_create_from(const char* rep,
  ...
  b->rep = WriteBatch(memory::xstring(rep, size));
  return b;
}
...
void rocksdb_options_set_block_based_table_factory(
  ...
  if (table_options) {
    opt->rep.table_factory.reset(NewBlockBasedTableFactory(table_options->rep));
  }
}
...
void rocksdb_options_set_cuckoo_table_factory(
  ...
  if (table_options) {
    opt->rep.table_factory.reset(NewCuckooTableFactory(table_options->rep));
  }
}
...
void rocksdb_options_enable_statistics(rocksdb_options_t* opt) {
  opt->rep.statistics = CreateDBStatistics();
}
...
void rocksdb_options_set_parallel_wal_recovery(rocksdb_options_t* opt,
                                               unsigned char v) {
  opt->rep.parallel_wal_recovery = v;
}
...
void rocksdb_options_set_skip_log_error_on_recovery(rocksdb_options_t* opt,
                                                    unsigned char v) {
...
void rocksdb_options_set_access_hint_on_compaction_start(rocksdb_options_t* opt,
  ...
  switch (v) {
    case 0:
      opt->rep.access_hint_on_compaction_start = Options::NONE;
      break;
    case 1:
      opt->rep.access_hint_on_compaction_start = Options::NORMAL;
      break;
    case 2:
      opt->rep.access_hint_on_compaction_start = Options::SEQUENTIAL;
      break;
    case 3:
      opt->rep.access_hint_on_compaction_start = Options::WILLNEED;
      break;
  }
}
...
void rocksdb_options_set_memtable_vector_rep(rocksdb_options_t* opt) {
  opt->rep.memtable_factory.reset(new VectorRepFactory);
}
...
void rocksdb_options_set_hash_skip_list_rep(rocksdb_options_t* opt,
  ...
  MemTableRepFactory* factory = NewHashSkipListRepFactory(
...
void rocksdb_options_set_hash_link_list_rep(rocksdb_options_t* opt,
                                            size_t bucket_count) {
  opt->rep.memtable_factory.reset(NewHashLinkListRepFactory(bucket_count));
}
...
void rocksdb_options_set_plain_table_factory(rocksdb_options_t* opt,
  ...
  TableFactory* factory = NewPlainTableFactory(options);
...
void rocksdb_options_set_compaction_style(rocksdb_options_t* opt, int style) {
  opt->rep.compaction_style = static_cast<CompactionStyle>(style);
}
...
char* rocksdb_options_statistics_get_string(rocksdb_options_t* opt) {
  Statistics* statistics = opt->rep.statistics.get();
  if (statistics) {
...
void rocksdb_readoptions_set_read_tier(rocksdb_readoptions_t* opt, int v) {
  opt->rep.read_tier = static_cast<ReadTier>(v);
}
...
rocksdb_env_t* rocksdb_create_mem_env() {
  rocksdb_env_t* result = new rocksdb_env_t;
  result->rep = NewMemEnv(Env::Default());
...
rocksdb_slicetransform_t* rocksdb_slicetransform_create_fixed_prefix(
  ...
  Wrapper* wrapper = new Wrapper;
  wrapper->rep_ = NewFixedPrefixTransform(prefixLen);
  wrapper->state_ = nullptr;
  wrapper->destructor_ = &Wrapper::DoNothing;
  return wrapper;
}
...
void rocksdb_options_set_min_level_to_compress(rocksdb_options_t* opt,
  ...
  if (level >= 0) {
    ...
    for (int i = 0; i < level; i++) {
      opt->rep.compression_per_level[i] = kNoCompression;
    }
