...
// not needed anymore.
class PinnedIteratorsManager : public common::Cleanable {
 public:
  ...
  void PinIterator(table::InternalIterator* iter, bool arena = false) {
    if (arena) {
  ...
  inline void ReleasePinnedData() {
    ...
    pinned_ptrs_.clear();
    // Also do cleanups from the base common::Cleanable
    common::Cleanable::Reset();
  }
...
//    delete reinterpret_cast<table::InternalIterator*>(ptr);
  ...
  static void ReleaseArenaInternalIterator(void* ptr) {
    reinterpret_cast<table::InternalIterator*>(ptr)->~InternalIterator();
  }
