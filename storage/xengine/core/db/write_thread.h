 ...
 public:
  ...
  struct Writer {
    ...
    common::SequenceNumber
        sequence;           // the sequence number to use for the first key
    common::Status status;  // status of memtable inserter
    common::Status callback_status;  // status returned by callback->Callback()
    ...
    Writer(const common::WriteOptions& write_options, WriteBatch* _batch,
  ...
  // util::autovector<WriteBatch*>* write_batch_group: Out-param of group
  // members
  // returns:                Total batch group byte size
  size_t EnterAsBatchGroupLeader(
      Writer* leader, Writer** last_writer,
      util::autovector<WriteThread::Writer*>* write_batch_group);
  ...
  // common::SequenceNumber sequence: Starting sequence number to assign to
  // Writer-s
  void LaunchParallelFollowers(ParallelGroup* pg,
  ...
  // common::Status status:          common::Status of write operation
  ...
  // monitor::InstrumentedMutex* mu:  The db mutex, to unlock while waiting
  // REQUIRES: db mutex held
  void EnterUnbatched(Writer* w, monitor::InstrumentedMutex* mu);
