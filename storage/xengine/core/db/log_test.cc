...
class LogTest : public ::testing::TestWithParam<int> {
 private:
  ...
  void reset_source_contents() {
    auto src = dynamic_cast<StringSource*>(reader_.internal_get_file()->file());
    assert(src);
    src->contents_ = dest_contents();
  }

  Slice reader_contents_;
  unique_ptr<ConcurrentDirectFileWriter, memory::ptr_destruct_delete<ConcurrentDirectFileWriter>> dest_holder_;
  unique_ptr<SequentialFileReader, memory::ptr_destruct_delete<SequentialFileReader>> source_holder_;
 ...
 public:
  LogTest()
      : reader_contents_(),
        dest_holder_(test::GetConcurrentDirectFileWriter(
//            new test::StringSink(&reader_contents_)
            MOD_NEW_OBJECT(memory::ModId::kDefaultMod, test::StringSink, &reader_contents_))),
        source_holder_(
            test::GetSequentialFileReader(MOD_NEW_OBJECT(memory::ModId::kDefaultMod, StringSource, reader_contents_))),
        writer_(dest_holder_.release(), 123, GetParam(), true),
        reader_(source_holder_.release(), &report_, true /*checksum*/,
                0 /*initial_offset*/, 123, true) {
...
//        test::GetSequentialFileReader(new StringSource(reader_contents_)));
    SequentialFileReader *file_reader = test::GetSequentialFileReader(
        MOD_NEW_OBJECT(memory::ModId::kDefaultMod, StringSource, reader_contents_));
    unique_ptr<Reader> offset_reader(
        new Reader(file_reader, &report_, true /*checksum*/,
                   WrittenBytes() + offset_past_end, 123, true));
    Slice record;
    std::string scratch;
...
//        test::GetSequentialFileReader(new StringSource(reader_contents_)));
    SequentialFileReader *file_reader = test::GetSequentialFileReader(
        MOD_NEW_OBJECT(memory::ModId::kDefaultMod, StringSource, reader_contents_));
    unique_ptr<Reader> offset_reader(new Reader(file_reader,
                                                &report_, true /*checksum*/,
                                                initial_offset, 123, true));
    Slice record;
    std::string scratch;
    ASSERT_TRUE(offset_reader->ReadRecord(&record, &scratch));
...
//  unique_ptr<ConcurrentDirectFileWriter> dest_holder(
//      test::GetConcurrentDirectFileWriter(
//          new test::OverwritingStringSink(get_reader_contents())));
  ConcurrentDirectFileWriter *dest_holder = test::GetConcurrentDirectFileWriter(
      MOD_NEW_OBJECT(memory::ModId::kDefaultMod, test::OverwritingStringSink, get_reader_contents()));
  dest_holder->init_multi_buffer();
  Writer recycle_writer(dest_holder, 123, true, true);
  recycle_writer.AddRecord(Slice("foooo"));
  recycle_writer.AddRecord(Slice("bar"));
  recycle_writer.file()->flush();
