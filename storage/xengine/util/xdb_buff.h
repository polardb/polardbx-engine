...
inline void xdb_netstr_append_uint64(my_core::String *const out_netstr,
                                     const uint64 &val) {
...
inline void xdb_netstr_append_uint32(my_core::String *const out_netstr,
                                     const uint32 &val) {
  DBUG_ASSERT(out_netstr != nullptr);
...
inline void xdb_netstr_append_uint16(my_core::String *const out_netstr,
                                     const uint16 &val) {
  DBUG_ASSERT(out_netstr != nullptr);
...
inline void xdb_netbuf_store_uint64(uchar *const dst_netbuf, const uint64 &n) {
...
inline void xdb_netbuf_store_uint32(uchar *const dst_netbuf, const uint32 &n) {
  DBUG_ASSERT(dst_netbuf != nullptr);
...
inline void xdb_netbuf_store_uint16(uchar *const dst_netbuf, const uint16 &n) {
  DBUG_ASSERT(dst_netbuf != nullptr);
...
inline void xdb_netbuf_store_byte(uchar *const dst_netbuf, const uchar &c) {
  DBUG_ASSERT(dst_netbuf != nullptr);

  *dst_netbuf = c;
}

inline void xdb_netbuf_store_index(uchar *const dst_netbuf,
                                   const uint32 &number) {
  DBUG_ASSERT(dst_netbuf != nullptr);

  xdb_netbuf_store_uint32(dst_netbuf, number);
}
...
inline uint64 xdb_netbuf_to_uint64(const uchar *const netbuf) {
...
inline uint32 xdb_netbuf_to_uint32(const uchar *const netbuf) {
  DBUG_ASSERT(netbuf != nullptr);
...
inline uint16 xdb_netbuf_to_uint16(const uchar *const netbuf) {
  DBUG_ASSERT(netbuf != nullptr);
...
inline uchar xdb_netbuf_to_byte(const uchar *const netbuf) {
  DBUG_ASSERT(netbuf != nullptr);
...
inline uint64 xdb_netbuf_read_uint64(const uchar **netbuf_ptr) {
  DBUG_ASSERT(netbuf_ptr != nullptr);

  // Convert from network byte order (Big Endian) to host machine byte order
  // (usually Little Endian).
  const uint64 host_val = xdb_netbuf_to_uint64(*netbuf_ptr);
...
inline uint32 xdb_netbuf_read_uint32(const uchar **netbuf_ptr) {
  DBUG_ASSERT(netbuf_ptr != nullptr);

  // Convert from network byte order (Big Endian) to host machine byte order
  // (usually Little Endian).
  const uint32 host_val = xdb_netbuf_to_uint32(*netbuf_ptr);
...
inline uint16 xdb_netbuf_read_uint16(const uchar **netbuf_ptr) {
  DBUG_ASSERT(netbuf_ptr != nullptr);

  // Convert from network byte order (Big Endian) to host machine byte order
  // (usually Little Endian).
  const uint16 host_val = xdb_netbuf_to_uint16(*netbuf_ptr);
...
inline void xdb_netbuf_read_gl_index(const uchar **netbuf_ptr,
                                     GL_INDEX_ID *const gl_index_id) {
  DBUG_ASSERT(gl_index_id != nullptr);
  DBUG_ASSERT(netbuf_ptr != nullptr);

  gl_index_id->cf_id = xdb_netbuf_read_uint32(netbuf_ptr);
  gl_index_id->index_id = xdb_netbuf_read_uint32(netbuf_ptr);
}
...
class Xdb_string_reader {
  const char *m_ptr;
  uint m_len;

private:
  Xdb_string_reader &operator=(const Xdb_string_reader &) = default;

public:
  Xdb_string_reader(const Xdb_string_reader &) = default;
  /* named constructor */
  static Xdb_string_reader read_or_empty(const xengine::common::Slice *const slice) {
    if (!slice) {
      return Xdb_string_reader("");
    } else {
      return Xdb_string_reader(slice);
    }
  }

  explicit Xdb_string_reader(const std::string &str) {
    m_len = str.length();
    if (m_len) {
      m_ptr = &str.at(0);
    } else {
      /*
        One can a create a Xdb_string_reader for reading from an empty string
  ...
  explicit Xdb_string_reader(const xengine::common::Slice *const slice) {
    m_ptr = slice->data();
    m_len = slice->size();
  }
...
class Xdb_string_writer {
  std::vector<uchar> m_data;

public:
  Xdb_string_writer(const Xdb_string_writer &) = delete;
  Xdb_string_writer &operator=(const Xdb_string_writer &) = delete;
  Xdb_string_writer() = default;
  ...
  void write_uint16(const uint &val) {
    ...
    xdb_netbuf_store_uint16(m_data.data() + size, val);
  }

  void write_uint32(const uint &val) {
    const auto size = m_data.size();
    m_data.resize(size + 4);
    xdb_netbuf_store_uint32(m_data.data() + size, val);
  }
  ...
  void write_uint16_at(const size_t &pos, const uint &new_val) {
    ...
    xdb_netbuf_store_uint16(m_data.data() + pos, new_val);
  }
};

/*
   A helper class for writing bits into Xdb_string_writer.

   The class assumes (but doesn't check) that nobody tries to write
   anything to the Xdb_string_writer that it is writing to.
*/
class Xdb_bit_writer {
  Xdb_string_writer *m_writer;
  uchar m_offset;

public:
  Xdb_bit_writer(const Xdb_bit_writer &) = delete;
  Xdb_bit_writer &operator=(const Xdb_bit_writer &) = delete;

  explicit Xdb_bit_writer(Xdb_string_writer *writer_arg)
...
class Xdb_bit_reader {
  const uchar *m_cur;
  uchar m_offset;
  uint m_ret;
  Xdb_string_reader *const m_reader;

public:
  Xdb_bit_reader(const Xdb_bit_reader &) = delete;
  Xdb_bit_reader &operator=(const Xdb_bit_reader &) = delete;

  explicit Xdb_bit_reader(Xdb_string_reader *const reader)
