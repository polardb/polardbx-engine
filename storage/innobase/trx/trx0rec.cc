    ...
    mtr_t *mtr)           /*!< in: mtr */
{
  ...
  *ptr++ = (byte)rec_get_info_bits(rec, dict_table_is_comp(table));
  ...
  field = rec_get_nth_field(rec, offsets, index->get_sys_col_pos(DATA_SCN_ID),
                            &flen);

  ut_ad(flen == DATA_SCN_ID_LEN);
  ptr += mach_u64_write_compressed(ptr, lizard::trx_read_scn(field));

  field = rec_get_nth_field(rec, offsets, index->get_sys_col_pos(DATA_UNDO_PTR),
                            &flen);
...
byte *trx_undo_update_rec_get_update(
    ...
    type_cmpl_t &type_cmpl,     /*!< out: type compilation info */
    txn_info_t txn_info) /*!< in: Lizard txn information */
{
  DBUG_TRACE;
  ...
  lizard::trx_undo_update_rec_by_lizard_fields(trx, index, update, n_fields + 2,
                                               txn_info, heap);
...
/** Copies an undo record to heap.
 ...
 @param[in/out]	txn_rec		txn_info of record, Liard;
                                 trx_id of the trx that generated
