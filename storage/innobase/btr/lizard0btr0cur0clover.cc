...
/**
  ...
  @param[in]      mtr        mtr
*/
void btr_cur_upd_clover_fields_clust_rec_log(const rec_t *rec,
  ...
  if (!mlog_open_and_write_index(mtr, rec, index, MLOG_REC_CLUST_CLOVER_UPDATE,
                                 REDO_CLOVER_FIELDS_LEN + 2, log_ptr)) {
    return;
  }

  log_ptr = row_upd_write_clover_vals_to_log(index, txn_rec, log_ptr, mtr);
...
byte *btr_cur_parse_clover_fields_upd_clust_rec(byte *ptr, byte *end_ptr,
  ...
  ptr = row_upd_parse_clover_vals(ptr, end_ptr, &pos, &scn, &undo_ptr, &gcn);
  ...
  if (page) {
    ...
    row_upd_rec_clover_fields_in_recovery(rec, page_zip, index, pos, offsets,
                                          scn, undo_ptr, gcn);
