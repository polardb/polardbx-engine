...
void btr_cur_upd_lizard_fields_clust_rec_log(const rec_t *rec,
  ...
  log_ptr = mlog_open_and_write_index(
      mtr, rec, index,
      page_rec_is_comp(rec) ? MLOG_COMP_REC_CLUST_LIZARD_UPDATE
                            : MLOG_REC_CLUST_LIZARD_UPDATE,
      1 + 1 + DATA_SCN_ID_LEN + DATA_UNDO_PTR_LEN + DATA_GCN_ID_LEN + 14 + 2);
...
Page &Page::operator=(const Page &page) {
  if (this != &page) {
    m_page_id.copy_from(page.m_page_id);
...
bool Cursor::restore_position(mtr_t *mtr) const {
  ...
  if (!buf_pool_is_obsolete(m_withdraw_clock) &&
      buf_page_optimistic_get(latch_mode, m_block, m_modify_clock,
                              Page_fetch::SCAN, __FILE__, __LINE__, mtr)) {
    return true;
  }
