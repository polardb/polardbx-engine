...
/**
 * @brief clear all files generated by changeset
 * file name such as "table_name_number.pkcs"
 */
static void clear_pre_changeset_files(const char *dir_path) {
  struct dirent *dir_p;
  ...
  while ((dir_p = readdir(dir)) != nullptr) {
    std::string name = dir_p->d_name;
    std::string path = dir_path;
    std::string suffix = ".pkcs";

    if (dir_p->d_type == DT_REG && has_suffix(name, suffix)) {
      remove((path + name).c_str());
    } else if (dir_p->d_type == DT_DIR) {
      if (name != "." && name != "..") {
        clear_pre_changeset_files((path + name + "/").c_str());
      }
    }
  }
...
static const Thread_to_stop threads_to_stop[]{
    ...
    {"gp_wait_timeout", srv_threads.m_gp_wait_timeout, lock_set_timeout_event,
...
dberr_t srv_start(bool create_new_db) {
  ...
  /* Clear changeset remaining files for polarx */
  clear_pre_changeset_files(mysql_real_data_home_ptr);
