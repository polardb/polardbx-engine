...
dberr_t Fil_shard::space_delete(space_id_t space_id, buf_remove_t buf_remove,
  ...
  if (const fil_space_t *s = get_space_by_id(space_id)) {
    ...
    if (new_filepath) {
      ...
      DBUG_EXECUTE_IF("Data_file_purge_rename_fail", exists = false;);

      if (!exists) {
        err = DB_TABLESPACE_NOT_FOUND;
      } else if (!os_file_rename(innodb_data_file_key, path, new_filepath)) {
...
bool fil_replace_tablespace(space_id_t old_space_id, space_id_t new_space_id,
  ...
  dberr_t err =
      fil_delete_tablespace(old_space_id, BUF_REMOVE_ALL_NO_WRITE, nullptr);
  if (err != DB_SUCCESS) {
    return (false);
  }
...
dberr_t fil_discard_tablespace(space_id_t space_id) {
  ...
  err = fil_delete_tablespace(space_id, BUF_REMOVE_ALL_NO_WRITE, nullptr);
