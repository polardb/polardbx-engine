...
void srv_scn_history_thread(void) {
  ...
  while (!scn_history_start_shutdown) {
    os_event_wait_time(scn_history_event, 1000000);
...
CRing<Item>::CRing(PSI_memory_key key, size_t size)
  ...
  m_items = UT_NEW_ARRAY(Item, m_capacity, key);
  m_head = -1;
  m_tail = -1;
}
...
void CRing<Item>::rebuild(size_t size) {
  ...
  m_items = UT_NEW_ARRAY(Item, size, m_key);
  m_capacity = size;
  m_head = -1;
  m_tail = -1;
}
...
CSnapshot_mgr::CSnapshot_mgr(PSI_memory_key key, size_t part, size_t buff_size)
  ...
  m_csnapshot_buffers = static_cast<CSnapshot_buffer *>(
      ut_malloc(sizeof(CSnapshot_buffer) * m_part, key));
  m_latches =
      static_cast<rw_lock_t *>(ut_malloc(sizeof(rw_lock_t) * m_part, key));
...
trx_id_t CSnapshot_mgr::search_up_limit_tid(const Item &lhs) {
  ...
  size_t partition_num = os_thread_get_curr_id() % m_part;

  rw_lock_s_lock(m_latches + partition_num);
