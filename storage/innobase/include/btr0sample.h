...
struct btr_sample_t {
  ...
  bool move_to_next_via_rec(mtr_t *mtr);
  bool move_to_next_via_blk(mtr_t *mtr);
  bool move_to_next_page(mtr_t *mtr);
  bool restore_leaf_pcur(mtr_t *mtr);
  bool restore_pcur_optimistic(btr_pcur_t *pcur, bool leaf, mtr_t *mtr,
                               bool &succ);
  bool restore_leaf_pessimistic(mtr_t *mtr);
  void search_to_leaf(const dtuple_t *tuple, page_cur_mode_t mode, mtr_t *mtr);
  bool get_one_leaf_with_sampling(page_cur_t *page_cursor, mtr_t *mtr);
