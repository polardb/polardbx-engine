...
polarx_rpc::chrono::Time_point Command::m_start_measure;
...
Command::Result Command::cmd_switchsid(std::istream &input,
  ...
  context->session()->set_session_id(sid);
...
Command::Result Command::cmd_do_ssl_handshake(std::istream &input,
                                              Execution_context *context,
                                              const std::string &args) {
  xcl::XError error(CR_SSL_CONNECTION_ERROR, "SSL not supported.");
  context->print_error_red(context->m_script_stack, error, '\n');
  return Result::Stop_with_failure;
}
...
Command::Result Command::cmd_sleep(std::istream &input,
  ...
  std::this_thread::sleep_for(std::chrono::microseconds(delay_in_microseconds));
#endif
  return Result::Continue;
}
...
Command::Result Command::cmd_repeat(std::istream &input,
  ...
  if (variable_name.length())
    context->m_variables->set(variable_name, polarx_rpc::to_string(loop.value));
...
Command::Result Command::cmd_peerdisc(std::istream &input,
  ...
  polarx_rpc::chrono::Time_point start_time = polarx_rpc::chrono::now();
  try {
    xcl::XProtocol::Server_message_type_id msgid;
...
Command::Result Command::cmd_measure(std::istream &input,
                                     Execution_context *context,
                                     const std::string &args) {
  m_start_measure = polarx_rpc::chrono::now();
  return Result::Continue;
}
...
Command::Result Command::cmd_endmeasure(std::istream &input,
                                        Execution_context *context,
                                        const std::string &args) {
  if (!polarx_rpc::chrono::is_valid(m_start_measure)) {
  ...
  m_start_measure = polarx_rpc::chrono::Time_point();
  return Result::Continue;
}
...
Command::Result Command::cmd_varinc(std::istream &input,
  ...
  int_val += int_n;
  val = polarx_rpc::to_string(int_val);
...
Command::Result Command::cmd_assert_eq(std::istream &input,
                                       Execution_context *context,
                                       const std::string &args) {
  return cmd_assert_generic<std::equal_to<std::string>>(input, context, args);
}
...
Command::Result Command::cmd_assert_le(std::istream &input,
                                       Execution_context *context,
                                       const std::string &args) {
  return cmd_assert_generic<Numeric_values<std::less_equal<long>>>(
      input, context, args);
}
...
Command::Result Command::cmd_assert_ge(std::istream &input,
                                       Execution_context *context,
                                       const std::string &args) {
  return cmd_assert_generic<Numeric_values<std::greater_equal<long>>>(
      input, context, args);
}
