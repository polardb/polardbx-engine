...
#include "../../client/mysqlxclient/xrow.h"
...
inline std::string get_flags(const xcl::Column_type &field, uint32_t flags) {
  ...
  if (flags & POLARX_COLUMN_FLAGS_UINT_ZEROFILL) {  // and other equal 1
    switch (field) {
      case xcl::Column_type::SINT:
      case xcl::Column_type::UINT:
        r += " ZEROFILL";
  ...
  if (flags & POLARX_COLUMN_FLAGS_NOT_NULL) r += " NOT_NULL";

  if (flags & POLARX_COLUMN_FLAGS_PRIMARY_KEY) r += " PRIMARY_KEY";

  if (flags & POLARX_COLUMN_FLAGS_UNIQUE_KEY) r += " UNIQUE_KEY";

  if (flags & POLARX_COLUMN_FLAGS_MULTIPLE_KEY) r += " MULTIPLE_KEY";

  if (flags & POLARX_COLUMN_FLAGS_AUTO_INCREMENT) r += " AUTO_INCREMENT";
