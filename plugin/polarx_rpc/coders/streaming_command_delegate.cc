...
int CstreamingCommandDelegate::field_metadata(struct st_send_field *field,
  ...
  if (compact_metadata_)
    ...
    msg_enc().encode_full_metadata(
        field->col_name, field->org_col_name, field->table_name,
        field->org_table_name, field->db_name, "def", xtype, type,
...
void CstreamingCommandDelegate::abort_row() { row_enc_.abort_row(); }
...
int CstreamingCommandDelegate::get_integer(longlong value) {
  const bool unsigned_flag =
      (field_types_[row_enc_.get_num_fields()].flags & UNSIGNED_FLAG) != 0;
...
int CstreamingCommandDelegate::get_longlong(longlong value,
  ...
  if (unsigned_flag &&
      (field_types_[row_enc_.get_num_fields()].type == MYSQL_TYPE_TINY)) {
    value &= 0xff;
  }
...
int CstreamingCommandDelegate::get_string(const char *const value,
  ...
  const enum_field_types type = field_types_[row_enc_.get_num_fields()].type;
  const unsigned int flags = field_types_[row_enc_.get_num_fields()].flags;
...
void CstreamingCommandDelegate::handle_error(uint sql_errno,
  ...
  CcommandDelegate::handle_error(sql_errno, err_msg, sqlstate);
}
