...
public:
  ...
  int init(KEY *key_info, uint32_t key_idx) {
    ...
    if (!key_info || key_idx >= MAX_INDEXES) {
      ...
      log_exec_error("ExecKeyMeta init failed, ret: %d, key_ptr: %p, key_idx: %u",
                     ret, key_info, key_idx);
    } else {
      key_info_ = key_info;
      key_idx_ = key_idx;
      init_ = true;
  ...
  const char *name() {
    return key_info_->name;
  }

  uint32_t parts() {
    return key_info_->user_defined_key_parts;
  }

  uint32_t length() {
    return key_info_->key_length;
  }
...
public:
  ...
  int init(ExecKeyMeta *exec_key,
    ...
    if ((ret = expr.init(mysqlx_key))) {
      ...
      log_exec_error("store key to table->record[0] failed, "
                     "ret: %d, part_map: %lu",
                     ret, used_part_map_);
    }
    return ret;
  ...
  int store_to_buffer(ExecKeyMeta *exec_key,
                      const ExecKeyMap &expr);

  uchar *key_buffer() const {
    return key_buffer_.get();
  }

  uint64_t used_length() const {
    return used_length_;
  }

  KeyPartMap used_part_map() const {
    return used_part_map_;
  }

  bool using_full_key() const {
    return using_full_key_;
  }

  bool is_impossible_where() const {
    return impossible_where_;
  }
  ...
  int init(ExecKeyMeta *exec_key, const RangeInfo &range_info) {
    ...
    if (range_info.begin_key_) {
      ...
      if (!begin_range_) {
        ...
        log_exec_error("SearchKey of begin key init failed, "
                       "ret: %d, parts: %lu",
        ...
        begin_range_->flag = range_info.include_begin_ ? HA_READ_KEY_OR_NEXT :
                                                         HA_READ_AFTER_KEY;
      }
    }
    ...
    if (!ret && range_info.end_key_) {
      ...
      if (!end_range_) {
        ...
        log_exec_error("SearchKey of end key init failed, "
                       "ret: %d, parts: %lu",
        ...
        end_range_->flag = range_info.include_end_ ? HA_READ_AFTER_KEY :
                                                     HA_READ_BEFORE_KEY;
      }
    }
    return ret;
...
public:
  ...
  int init(TABLE *table) {
    ...
    if (!table || !(table->file) || !(table->s)) {
      ret = HA_ERR_TABLE_CORRUPT;
      log_exec_error("ExecTable init failed, ret: %d, "
      ...
      for (uint32_t i = 0; i < table->s->keys ; ++i) {
        ...
        std::unique_ptr<ExecKeyMeta> new_exec_key(new (std::nothrow) ExecKeyMeta());
        if (!new_exec_key) {
          ret = HA_ERR_OUT_OF_MEM;
          log_exec_error("ExecTable init failed, new ExecKeyMeta failed, ret: %d",
                         ret);
...
public:
  ...
  void defer_delete_item(ExprItem *item) {
    item_free_list_.emplace_back(item);
  }
  ...
  int do_project() {
    ...
    // use get_all_fields to send metadata(See @executor::Protocol::write_metadata)
    // and row(See @executor::Protocol::write_row).
    //
