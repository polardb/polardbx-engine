...
polarx_rpc::err_t Physical_backfill::read_buffer(
  ...
  if (msg.has_file_info()) {
    ...
    if (!exists) {
      ...
      if (msg.offset() < 0) {
        log_exec_error("ER_X_NOT_ALLOW_NEGATIVE_OFFSET offset must be positive "
                       "number， but got:%ld",
                       msg.offset());
        m = "offset must be positive number， but got:";
        m += msg.offset();
        return polarx_rpc::err_t(ER_X_NOT_ALLOW_NEGATIVE_OFFSET, m);
      }
      ...
      if ((file_desc_info.file = my_open(full_file_name.c_str(),
        ...
        if (ret != 0) {
          ...
          log_exec_error(
              ...
              "desired size %d Operating system error number %d. "
  ...
  if (msg.has_table_info()) {
    ...
    if (file_infos.size() % 2 != 0 || file_infos.size() == 0) {
      ...
      for (int i = 0; i < file_infos.size(); i = i + 2) {
        ...
        handler_begin_read(thd, info.c_str(), info.length());
