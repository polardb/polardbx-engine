...
ngs::Error_code Galaxy_stmt_command_handler::execute(
...
ngs::Error_code Galaxy_stmt_command_handler::sql_stmt_execute(
    const Mysqlx::Sql::GalaxyStmtExecute &msg) {
  ...
  /** Galaxy X-protocol */
  if (msg.has_db_name()) {
    std::string db_name = msg.db_name();

    auto &da = m_session->data_context();

    Empty_resultset empty_resultset;
    ngs::Error_code err =
        da.init_db(db_name.data(), db_name.length(), &empty_resultset);
    if (err) return err;
  }
...
ngs::Error_code Galaxy_stmt_command_handler::deprecated_admin_stmt_execute(
    const Mysqlx::Sql::GalaxyStmtExecute &msg) {
...
ngs::Error_code Galaxy_stmt_command_handler::admin_stmt_execute(
    const Mysqlx::Sql::GalaxyStmtExecute &msg) {
  m_session->update_status(
